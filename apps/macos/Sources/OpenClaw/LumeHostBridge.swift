...
final class LumeHostBridge: NSObject {
    static let messageHandlerName = "lumeHost"
    ...
    (() => {
      if (globalThis.lumeHost?.request) return;
      let nextId = 1;
      ...
      globalThis.__lumeHostReject = (id, message) => {
        ...
        if (!entry) return;
        pending.delete(id);
        entry.reject(new Error(message || "Native Lume host request failed"));
      };
      ...
      globalThis.lumeHost = {
        request(method, params = {}) {
          return new Promise((resolve, reject) => {
            ...
            const handler = globalThis.webkit?.messageHandlers?.lumeHost;
            if (!handler?.postMessage) {
              pending.delete(id);
              reject(new Error("Native Lume host bridge unavailable"));
              return;
            }
...
extension LumeHostBridge: WKScriptMessageHandler {
    ...
    private func dispatch(method: String, params: [String: Any]) async throws -> Any {
        ...
        case "revealLogs":
            ...
            return ["ok": true]
        default:
            throw LumeHostBridgeError.unknownMethod(method)
        }
    }
    ...
    private func requestPermission(_ params: [String: Any]) async throws -> Any {
        guard let raw = params["permission"] as? String,
            ...
            throw LumeHostBridgeError.invalidParams("Missing or invalid permission")
        }
    ...
    private func setLaunchAtLogin(_ params: [String: Any]) async throws -> Any {
        guard let enabled = params["enabled"] as? Bool else {
            throw LumeHostBridgeError.invalidParams("Missing launch-at-login value")
        }
    ...
    private func openExternal(_ params: [String: Any]) throws {
        guard let raw = params["url"] as? String,
            ...
            throw LumeHostBridgeError.invalidParams("Missing URL")
        }
...
private enum LumeHostBridgeError: LocalizedError {
