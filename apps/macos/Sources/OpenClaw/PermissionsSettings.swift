    ...
    private var guidanceText: String {
        ...
        case .whileUsing:
            if PermissionManager.isLocationAuthorized(status: self.authorizationStatus, mode: .whileUsing) {
                return self.locationPreciseEnabled
                    ? "OpenClaw can use your location while the app is active."
                    : "OpenClaw can share an approximate location while the app is active."
            }
        ...
        case .always:
            if PermissionManager.isLocationAuthorized(status: self.authorizationStatus, mode: .always) {
                return "OpenClaw can keep using location even when the app is not frontmost."
            }
            if PermissionManager.isLocationAuthorized(status: self.authorizationStatus, mode: .whileUsing) {
