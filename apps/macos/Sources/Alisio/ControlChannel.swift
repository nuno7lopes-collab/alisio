...
final class ControlChannel {
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: AlisioKit.AnyCodable]()) {
                $0[$1.key] = AlisioKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: AlisioProtocol.AnyCodable?) -> [String: AlisioProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: AlisioProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: AlisioKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: AlisioProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("alisio.control.heartbeat")
    static let controlAgentEvent = Notification.Name("alisio.control.agent")
}
