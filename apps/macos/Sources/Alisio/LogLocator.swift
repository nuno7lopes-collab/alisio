...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["ALISIO_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/alisio")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("alisio-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("alisio-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/alisio/ (rolling or stdout), or nil if none exist.
