...
actor GatewayEndpointStore {
    ...
    private static let staticLogger = Logger(subsystem: AlisioBrand.logSubsystem, category: "gateway-endpoint")
    ...
    {
        let raw = env["ALISIO_GATEWAY_PASSWORD"] ?? ""
        ...
        if !trimmed.isEmpty {
               ...
               !configPassword.isEmpty
            {
                self.warnEnvOverrideOnce(
                    kind: .password,
                    envVar: "ALISIO_GATEWAY_PASSWORD",
    ...
    private static func resolveGatewayToken(
        ...
        launchdSnapshot: LaunchAgentPlistSnapshot?) -> String?
    {
        let raw = env["ALISIO_GATEWAY_TOKEN"] ?? ""
        let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
        if !trimmed.isEmpty {
    ...
    private let logger = Logger(subsystem: AlisioBrand.logSubsystem, category: "gateway-endpoint")
    ...
    init(deps: Deps = .live) {
        ...
        let bind = GatewayEndpointStore.resolveGatewayBindMode(
            root: AlisioConfigFile.loadDict(),
            env: ProcessInfo.processInfo.environment)
        let customBindHost = GatewayEndpointStore.resolveGatewayCustomBindHost(root: AlisioConfigFile.loadDict())
    ...
    {
        if let envBind = env["ALISIO_GATEWAY_BIND"] {
    ...
    private static func resolveGatewayScheme(
        ...
        env: [String: String]) -> String
    {
        if let envValue = env["ALISIO_GATEWAY_TLS"]?.trimmingCharacters(in: .whitespacesAndNewlines),
           !envValue.isEmpty
        {
