...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.alisio/alisio.json. Prefer ALISIO_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.alisio/alisio.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.alisio/alisio.json. Restart the gateway to apply."
        }
    ...
    private func restartGatewayIfNeeded() {
        ...
        Task { await GatewayLaunchAgentManager.restart() }
    }
