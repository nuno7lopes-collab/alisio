...
enum LaunchAgentPlist {
    ...
    static func snapshot(url: URL) -> LaunchAgentPlistSnapshot? {
        ...
        let token = env["ALISIO_GATEWAY_TOKEN"]?.trimmingCharacters(in: .whitespacesAndNewlines).nonEmpty
        let password = env["ALISIO_GATEWAY_PASSWORD"]?.trimmingCharacters(in: .whitespacesAndNewlines).nonEmpty
