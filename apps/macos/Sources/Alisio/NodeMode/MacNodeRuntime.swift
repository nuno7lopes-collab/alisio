...
actor MacNodeRuntime {
    ...
    func handleInvoke(_ req: BridgeInvokeRequest) async -> BridgeInvokeResponse {
        ...
        do {
            ...
            case AlisioBrowserCommand.proxy.rawValue:
            ...
            case AlisioSystemCommand.which.rawValue:
                return try await self.handleSystemWhich(req)
            case AlisioSystemCommand.notify.rawValue:
                return try await self.handleSystemNotify(req)
            case AlisioSystemCommand.execApprovalsGet.rawValue:
                return try await self.handleSystemExecApprovalsGet(req)
            case AlisioSystemCommand.execApprovalsSet.rawValue:
    ...
    private func handleA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case AlisioCanvasA2UICommand.push.rawValue,
             AlisioCanvasA2UICommand.pushJSONL.rawValue:
    ...
    private func handleBrowserProxyInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        guard AlisioConfigFile.browserControlEnabled() else {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: AlisioNodeError(
        ...
        if mode == .whileUsing, !isApplicationActive {
            return BridgeInvokeResponse(
                ...
                error: AlisioNodeError(
                    ...
                    message: "LOCATION_BACKGROUND_UNAVAILABLE: while using location requires Alisio to stay active"))
        }
    ...
    private func resolveA2UIHostUrl() async -> String? {
        ...
        return baseUrl.appendingPathComponent("__alisio__/a2ui/").absoluteString + "?platform=macos"
    }
    ...
    private func handleSystemRun(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(AlisioSystemRunParams.self, from: req.paramsJSON)
    ...
    private func handleSystemWhich(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(AlisioSystemWhichParams.self, from: req.paramsJSON)
...
extension MacNodeRuntime {
    ...
    private nonisolated static func locationMode() -> AlisioLocationMode {
    ...
    private static func errorResponse(
        ...
        code: AlisioNodeErrorCode,
        message: String) -> BridgeInvokeResponse
    {
        BridgeInvokeResponse(
            id: req.id,
            ok: false,
            error: AlisioNodeError(code: code, message: message))
    }

    private static func encodeCanvasSnapshot(
        image: NSImage,
        format: AlisioCanvasSnapshotFormat,
        maxWidth: Int?,
