...
enum SessionMenuPreviewLoader {
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> AlisioSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: AlisioSessionPreviewEntry,
    ...
    private static func previewItems(
        from payload: AlisioChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [AlisioKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [AlisioKit.AnyCodable]) -> [AlisioChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(AlisioChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: AlisioChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: AlisioChatMessage) -> Bool {
    ...
    private static func toolNames(for message: AlisioChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: AlisioChatMessage) -> String? {
