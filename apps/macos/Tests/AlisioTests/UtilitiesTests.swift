...
@Suite(.serialized) struct UtilitiesTests {
    ...
    @Test func `gateway entrypoint prefers bin over dist`() throws {
        ...
        let bin = tmp.appendingPathComponent("bin/alisio.js")
    ...
    @Test func `log locator picks newest log file`() throws {
        let fm = FileManager()
        let dir = URL(fileURLWithPath: "/tmp/alisio", isDirectory: true)
        try? fm.createDirectory(at: dir, withIntermediateDirectories: true)

        let older = dir.appendingPathComponent("alisio-old-\(UUID().uuidString).log")
        let newer = dir.appendingPathComponent("alisio-new-\(UUID().uuidString).log")
