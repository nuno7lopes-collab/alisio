...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.alisio.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to Alisio", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to Alisio gateway…")
        ...
        do {
            ...
            self.showStatus("Sent to Alisio.")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "AlisioShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "Alisio is not connected to a gateway yet."])
        }
        ...
        let makeOptions: (String) -> GatewayConnectOptions = { clientId in
            GatewayConnectOptions(
                ...
                clientId: clientId,
                clientMode: "node",
                clientDisplayName: "Alisio Share",
