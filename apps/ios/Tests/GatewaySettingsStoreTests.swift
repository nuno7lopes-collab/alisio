...
@Suite(.serialized) struct GatewaySettingsStoreTests {
    ...
    @Test func bootstrapKeepsKeychainValuesStable() {
        withBootstrapSnapshots {
            applyDefaults([
                "node.instanceId": nil,
                "gateway.preferredStableID": nil,
            ...
            #expect(KeychainStore.loadString(service: nodeService, account: "instanceId") == "node-from-keychain")
            #expect(UserDefaults.standard.string(forKey: "node.instanceId") == "node-from-keychain")
        }
    }
    ...
    @Test func lastGateway_discoveredOverwritesManual() {
        withLastGatewaySnapshot {
            GatewaySettingsStore.saveLastGatewayConnectionManual(
                ...
                port: 40705,
                useTLS: true,
                stableID: "manual|10.0.0.99|40705")
    ...
    @Test func lastGateway_migratesFromUserDefaults() {
        withLastGatewaySnapshot {
            ...
            applyDefaults([
                ...
                "gateway.last.port": 40705,
                "gateway.last.tls": false,
                "gateway.last.stableID": "manual|example.org|40705",
            ])

            let loaded = GatewaySettingsStore.loadLastGatewayConnection()
            #expect(loaded == .manual(host: "example.org", port: 40705, useTLS: false, stableID: "manual|example.org|40705"))
