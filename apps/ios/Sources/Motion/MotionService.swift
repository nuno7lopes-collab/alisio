...
final class MotionService: MotionServicing {
    func activities(params: AlisioMotionActivityParams) async throws -> AlisioMotionActivityPayload {
        ...
        let mapped: [AlisioMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return AlisioMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: AlisioPedometerParams) async throws -> AlisioPedometerPayload {
        ...
        let payload: AlisioPedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = AlisioPedometerPayload(
