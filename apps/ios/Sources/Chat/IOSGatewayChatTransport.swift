...
struct IOSGatewayChatTransport: AlisioChatTransport, Sendable {
    private static let logger = Logger(subsystem: "ai.alisio", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> AlisioChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(AlisioChatSessionsListResponse.self, from: res)
    }
        ...
        attachments: [AlisioChatAttachmentPayload]) async throws -> AlisioChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [AlisioChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(AlisioChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(AlisioGatewayHealthOK.self, from: res))?.ok ?? true
    }
