...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__ALISIO_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<AlisioTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.alisio", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = AlisioTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> AlisioTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return AlisioTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> AlisioTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> AlisioTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return AlisioTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> AlisioTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = AlisioTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: AlisioTalkPTTStopPayload) {
