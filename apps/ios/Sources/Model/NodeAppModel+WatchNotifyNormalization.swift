...
extension NodeAppModel {
    static func normalizeWatchNotifyParams(_ params: AlisioWatchNotifyParams) -> AlisioWatchNotifyParams {
    ...
    static func normalizeWatchActions(
        _ actions: [AlisioWatchAction]?,
        kind: String?,
        promptId: String?) -> [AlisioWatchAction]
    {
        let provided = (actions ?? []).compactMap { action -> AlisioWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return AlisioWatchAction(
                id: id,
                label: label,
        ...
        if normalizedKind.contains("approval") || normalizedKind.contains("approve") {
            return [
                AlisioWatchAction(id: "approve", label: "Approve"),
                AlisioWatchAction(id: "decline", label: "Decline", style: "destructive"),
                AlisioWatchAction(id: "open_phone", label: "Open iPhone"),
                AlisioWatchAction(id: "escalate", label: "Escalate"),
            ]
        }

        return [
            AlisioWatchAction(id: "done", label: "Done"),
            AlisioWatchAction(id: "snooze_10m", label: "Snooze 10m"),
            AlisioWatchAction(id: "open_phone", label: "Open iPhone"),
            AlisioWatchAction(id: "escalate", label: "Escalate"),
        ]
    }
    ...
    static func normalizedWatchRisk(
        _ risk: AlisioWatchRisk?,
        priority: AlisioNotificationPriority?) -> AlisioWatchRisk?
    {
    ...
    static func normalizedWatchPriority(
        _ priority: AlisioNotificationPriority?,
        risk: AlisioWatchRisk?) -> AlisioNotificationPriority?
    {
