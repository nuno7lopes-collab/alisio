...
final class RemindersService: RemindersServicing {
    func list(params: AlisioRemindersListParams) async throws -> AlisioRemindersListPayload {
        ...
        let payload: [AlisioReminderPayload] = try await withCheckedThrowingContinuation { cont in
        ...
        return AlisioRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: AlisioRemindersAddParams) async throws -> AlisioRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = AlisioReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return AlisioRemindersAddPayload(reminder: payload)
    }
