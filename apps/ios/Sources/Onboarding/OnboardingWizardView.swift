...
struct OnboardingWizardView: View {
    ...
    @State private var manualPort: Int = GatewayDefaults.defaultPort
    @State private var manualPortText: String = GatewayDefaults.defaultPortText
    @State private var manualTLS: Bool = true
    @State private var gatewayToken: String = ""
    @State private var gatewayPassword: String = ""
    @State private var connectMessage: String?
    @State private var statusLine: String = "In your Alisio chat, run /pair qr, then scan the code here."
    ...
    private var introStep: some View {
        VStack(spacing: 0) {
            ...
            Text("Turn this iPhone into a secure Alisio node for chat, voice, camera, and device tools.")
                .font(.subheadline)
                .foregroundStyle(.secondary)
                .multilineTextAlignment(.center)
            ...
            VStack(alignment: .leading, spacing: 14) {
                ...
                Label("Use Alisio from your phone", systemImage: "message.fill")
            }
            ...
            HStack(alignment: .top, spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    Text("Security notice")
                        .font(.headline)
                    Text(
                        "The connected Alisio agent can use device capabilities you enable, such as camera, microphone, photos, contacts, calendar, and location. Continue only if you trust the gateway and agent you connect to.")
                        .font(.footnote)
                        .foregroundStyle(.secondary)
    ...
    private var welcomeStep: some View {
        VStack(spacing: 0) {
            ...
            Text("Scan a QR code from your Alisio gateway or continue with manual setup.")
                .font(.subheadline)
                .foregroundStyle(.secondary)
                .multilineTextAlignment(.center)
                .padding(.horizontal, 32)
            ...
            if self.issue.needsPairing {
                Section {
                    ...
                    let requestLine: String = {
                        ...
                        return "Request ID: check `alisio devices list`."
                    }()
                    Text(
                        "Approve this device on the gateway.\n"
                            + "1) `alisio devices approve` (or `alisio devices approve <requestId>`)\n"
                            + "2) `/pair approve` in your Alisio chat\n"
                            + "\(requestLine)\n"
                            + "Alisio will also retry automatically when you return to this app.")
                }
            }
    ...
    private func advanceFromIntro() {
        ...
        self.statusLine = "In your Alisio chat, run /pair qr, then scan the code here."
        self.step = .welcome
    }
    ...
    private func initializeState() {
        if self.manualHost.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            if let last = GatewaySettingsStore.loadLastGatewayConnection() {
                ...
                case .discovered:
                    self.manualHost = "alisio.local"
                    self.manualPort = GatewayDefaults.defaultPort
                    self.manualTLS = true
                }
            } else {
                self.manualHost = "alisio.local"
        ...
        if self.selectedMode == .developerLocal && self.manualHost == "alisio.local" {
        ...
        if !hasSavedGateway, !hasToken, !hasPassword {
            self.statusLine = "No saved pairing found. In your Alisio chat, run /pair qr, then scan the code here."
        }
    }
    ...
    private func applyModeDefaults(_ mode: OnboardingConnectionMode) {
        ...
        let hostIsDefaultLike = host.isEmpty || host == "alisio.local" || host == "localhost"

        switch mode {
        case .homeNetwork:
            if hostIsDefaultLike { self.manualHost = "alisio.local" }
            self.manualTLS = true
            if self.manualPort <= 0 || self.manualPort > 65535 {
                self.manualPort = GatewayDefaults.defaultPort
            }
        case .remoteDomain:
            if host == "alisio.local" || host == "localhost" { self.manualHost = "" }
            self.manualTLS = true
            if self.manualPort <= 0 || self.manualPort > 65535 {
                self.manualPort = GatewayDefaults.defaultPort
            }
