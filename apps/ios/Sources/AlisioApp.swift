...
@MainActor
final class AlisioAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.alisio.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.alisio.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.alisio.ios.bgrefresh"
...
enum WatchPromptNotificationBridge {
    static let typeKey = "alisio.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "alisio.watch.promptId"
    static let sessionKeyKey = "alisio.watch.sessionKey"
    static let actionPrimaryIDKey = "alisio.watch.action.primary.id"
    static let actionPrimaryLabelKey = "alisio.watch.action.primary.label"
    static let actionSecondaryIDKey = "alisio.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "alisio.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "alisio.watch.action.primary"
    static let actionSecondaryIdentifier = "alisio.watch.action.secondary"
    static let actionIdentifierPrefix = "alisio.watch.action."
    static let actionIDKeyPrefix = "alisio.watch.action.id."
    static let actionLabelKeyPrefix = "alisio.watch.action.label."
    static let categoryPrefix = "alisio.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: AlisioWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> AlisioWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return AlisioWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "Alisio" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [AlisioWatchAction]) -> [UNNotificationAction] {
...
struct AlisioApp: App {
    ...
    @UIApplicationDelegateAdaptor(AlisioAppDelegate.self) private var appDelegate
...
extension AlisioApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("Alisio: installing uncaught exception handler")
