...
final class ContactsService: ContactsServicing {
    ...
    func search(params: AlisioContactsSearchParams) async throws -> AlisioContactsSearchPayload {
        ...
        return AlisioContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: AlisioContactsAddParams) async throws -> AlisioContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return AlisioContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return AlisioContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> AlisioContactPayload {
        AlisioContactPayload(
