# Alisio iOS (Super Alpha)

This iPhone app is super-alpha and internal-use only. It connects to an Alisio Gateway as a `role: node`.
...
- `ALISIO_PUSH_RELAY_BASE_URL=https://relay.example.com`
...
- `apps/ios/Sources/Alisio.entitlements` sets `aps-environment` to `development`.
- APNs token registration to gateway happens only after gateway connection (`push.apns.register`).
- Local/manual builds default to `AlisioPushTransport=direct` and `AlisioPushDistribution=local`.
- Your selected team/profile must support Push Notifications for the app bundle ID you are signing.
- If push capability or provisioning is wrong, APNs registration fails at runtime (check Xcode logs for `APNs registration failed`).
- Debug builds default to `AlisioPushAPNsEnvironment=sandbox`; Release builds default to `production`.
...
- Gateway-side relay sending is configured through `gateway.push.apns.relay.baseUrl` in `alisio.json`. `ALISIO_APNS_RELAY_BASE_URL` remains a temporary env override only.
...
This exists to keep the hosted relay limited to genuine Alisio official builds and to ensure a
