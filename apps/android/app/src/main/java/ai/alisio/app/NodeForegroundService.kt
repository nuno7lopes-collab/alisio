...
class NodeForegroundService : Service() {
  ...
  override fun onCreate() {
    super.onCreate()
    ensureChannel()
    val initial = buildNotification(title = "Alisio Node", text = "Starting…")
    ...
    notificationJob =
      scope.launch {
        combine(
          ...
          val title = if (connected) "Alisio Node · Connected" else "Alisio Node"
          val micSuffix =
            if (micEnabled) {
  ...
  private fun ensureChannel() {
    ...
    val channel =
      NotificationChannel(
        ...
        description = "Alisio node connection status"
        setShowBadge(false)
      }
  ...
  companion object {
    ...
    private const val ACTION_STOP = "ai.alisio.app.action.STOP"
