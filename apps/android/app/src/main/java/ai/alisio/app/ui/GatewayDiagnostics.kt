...
internal fun alisioAndroidVersionLabel(): String {
...
internal fun buildGatewayDiagnosticsReport(
  ...
  return """
    Help diagnose this Alisio Android gateway connection failure.

    Please:
    - pick one route only: same machine, same LAN, Tailscale, or public URL
    - classify this as pairing/auth, TLS trust, wrong advertised route, wrong address/port, or gateway down
    - quote the exact app status/error below
    - tell me whether `alisio devices list` should show a pending pairing request
    - if more signal is needed, ask for `alisio qr --json`, `alisio devices list`, and `alisio nodes status`
    - give the next exact command or tap

    Debug info:
    - screen: $screen
    - app version: ${alisioAndroidVersionLabel()}
    - device: $device
...
internal fun copyGatewayDiagnosticsReport(
  ...
  clipboard.setPrimaryClip(ClipData.newPlainText("Alisio gateway diagnostics", report))
