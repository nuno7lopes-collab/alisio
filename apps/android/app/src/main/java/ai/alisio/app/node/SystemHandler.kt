...
private const val NOTIFICATION_CHANNEL_BASE_ID = "alisio.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "Alisio Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "Alisio Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "Alisio Active")
      }
