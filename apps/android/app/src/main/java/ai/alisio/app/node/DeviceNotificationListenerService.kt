...
import ai.alisio.app.NotificationBurstLimiter
import ai.alisio.app.SecurePrefs
import ai.alisio.app.allowsPackage
import ai.alisio.app.isWithinQuietHours
...
class DeviceNotificationListenerService : NotificationListenerService() {
  ...
  companion object {
    ...
    private fun migrateLegacyRecentPackagesIfNeeded(context: Context) {
      ...
      val current = prefs.getString(recentPackagesPref, null)?.trim().orEmpty()
      if (current.isEmpty()) {
        val legacyPrefs = legacyRecentPackagesPrefs(context)
        val migrated =
          sequenceOf(
            prefs.getString(legacyRecentPackagesPref, null),
            legacyPrefs.getString(recentPackagesPref, null),
            legacyPrefs.getString(legacyRecentPackagesPref, null),
          ).map { it?.trim().orEmpty() }
            .firstOrNull { it.isNotEmpty() }

        if (!migrated.isNullOrEmpty()) {
          prefs.edit().putString(recentPackagesPref, migrated).remove(legacyRecentPackagesPref).apply()
          return
        }
      }

      if (prefs.contains(legacyRecentPackagesPref)) {
