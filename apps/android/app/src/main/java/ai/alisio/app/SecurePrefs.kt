...
class SecurePrefs(
  ...
  companion object {
    ...
    private const val plainPrefsName = "alisio.node"
    private const val securePrefsName = "alisio.node.secure"
    private val legacyPlainPrefsName = "$legacyBrandName.node"
    private val legacySecurePrefsName = "$legacyBrandName.node.secure"
  ...
  private val legacyPlainPrefs: SharedPreferences =
    appContext.getSharedPreferences(legacyPlainPrefsName, Context.MODE_PRIVATE)
  ...
  private val legacySecurePrefs: SharedPreferences by lazy {
    securePrefsOverride ?: createSecurePrefs(appContext, legacySecurePrefsName)
  }
  ...
  private fun migrateLegacyPrefsIfNeeded() {
    copyMissingEntries(source = legacyPlainPrefs, destination = plainPrefs)
    copyMissingEntries(source = legacySecurePrefs, destination = securePrefs)
  }

  private fun copyMissingEntries(
    source: SharedPreferences,
    destination: SharedPreferences,
  ) {
    val entries = runCatching { source.all }.getOrDefault(emptyMap())
    if (entries.isEmpty()) {
      return
    }
  ...
  private fun loadWakeWords(): List<String> {
    ...
    return try {
      ...
      val sanitized = WakeWords.sanitize(decoded, defaultWakeWords)
