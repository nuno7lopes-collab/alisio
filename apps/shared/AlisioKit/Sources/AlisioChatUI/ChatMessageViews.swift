...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        .padding(.vertical, self.style == .alisio ? (self.isUser ? 12 : 2) : 10)
        .padding(.horizontal, self.style == .alisio ? (self.isUser ? 14 : 0) : 12)
    ...
    private var inlineAttachments: [AlisioChatMessageContent] {
    ...
    private var toolCalls: [AlisioChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [AlisioChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            ...
            return self.userAccent ?? AlisioChatTheme.userBubble
        }
        if self.style == .alisio {
            return .clear
        }
        if self.style == .onboarding {
            return AlisioChatTheme.onboardingAssistantBubble
        }
        return AlisioChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return AlisioChatTheme.onboardingAssistantBorder
        }
        return Color.white.opacity(0.08)
    }
    ...
    private var bubbleShape: ChatBubbleShape {
        ChatBubbleShape(
            cornerRadius: self.style == .alisio ? (self.isUser ? 22 : 0) : ChatUIConstants.bubbleCorner,
    ...
    private var bubbleShadowColor: Color {
        if self.style == .alisio && self.isUser {
...
private struct AttachmentRow: View {
    let att: AlisioChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? AlisioChatTheme.userText : AlisioChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: AlisioChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(self.style == .alisio ? Color.clear : AlisioChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(self.style == .alisio ? Color.clear : Color.white.opacity(0.08), lineWidth: 1))
...
private extension View {
    func assistantBubbleContainerStyle() -> some View {
        self
            .background(
                RoundedRectangle(cornerRadius: 16, style: .continuous)
                    .fill(AlisioChatTheme.assistantBubble))
            .overlay(
                RoundedRectangle(cornerRadius: 16, style: .continuous)
                    .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
            .frame(maxWidth: ChatUIConstants.bubbleMaxWidth, alignment: .leading)
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: AlisioChatTheme.assistantText)
            }
        }
    }
}
