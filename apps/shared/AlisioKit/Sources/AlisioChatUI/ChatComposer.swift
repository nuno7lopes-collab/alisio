...
@MainActor
struct AlisioChatComposer: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: self.style == .alisio ? 8 : 4) {
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(AlisioChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(AlisioChatTheme.composerBorder, lineWidth: 1))
                    .shadow(color: .black.opacity(0.12), radius: 12, y: 6)
            } else {
                let shape = RoundedRectangle(cornerRadius: self.style == .alisio ? 24 : cornerRadius, style: .continuous)
                shape
                    .fill(self.style == .alisio ? AnyShapeStyle(Color(chatHex: 0x111215)) : AlisioChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(self.style == .alisio ? Color.white.opacity(0.09) : AlisioChatTheme.composerBorder, lineWidth: 1))
                    .shadow(color: .black.opacity(self.style == .alisio ? 0.24 : 0.12), radius: self.style == .alisio ? 20 : 12, y: self.style == .alisio ? 10 : 6)
            }
            #else
    ...
    private var modelPicker: some View {
        ...
        {
            Text(self.viewModel.defaultModelLabel).tag(AlisioChatViewModel.defaultModelSelectionID)
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \AlisioPendingAttachment.id)
                { (att: AlisioPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            AlisioPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
                    ...
                    .padding(.vertical, 5)
                    .background(self.style == .alisio ? Color(chatHex: 0x181A1F) : Color.accentColor.opacity(0.08))
                    .overlay(
                        Capsule()
                            .strokeBorder(self.style == .alisio ? Color.white.opacity(0.08) : Color.clear, lineWidth: 1))
                    .clipShape(Capsule())
                }
            }
        }
    ...
    private var editor: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if !self.isComposerCompacted && self.style != .alisio {
                Rectangle()
                    .fill(AlisioChatTheme.divider)
                    .frame(height: 1)
        ...
        .padding(.horizontal, self.style == .alisio ? 14 : 10)
        .padding(.vertical, self.style == .alisio ? 12 : 8)
        .background(
            RoundedRectangle(cornerRadius: self.style == .alisio ? 22 : 12, style: .continuous)
                .fill(self.style == .alisio ? AnyShapeStyle(Color.clear) : AlisioChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: self.style == .alisio ? 22 : 12, style: .continuous)
                        .strokeBorder(self.style == .alisio ? Color.clear : AlisioChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(AlisioChatTheme.subtleCard)
        .clipShape(Capsule())
    }
    ...
    private var sendButton: some View {
        Group {
            if self.viewModel.pendingRunCount > 0 {
                ...
                .padding(6)
                .background(Circle().fill(self.style == .alisio ? Color(chatHex: 0xE05757) : Color.red))
    ...
    private var textMinHeight: CGFloat {
        self.style == .alisio ? 48 : (self.style == .onboarding ? 24 : 28)
    }

    private var textMaxHeight: CGFloat {
        self.style == .alisio ? 140 : (self.style == .onboarding ? 52 : 64)
    }
    ...
    @ViewBuilder
    private var alisioAttachmentButton: some View {
        #if os(macOS)
        Button {
            self.pickFilesMac()
        } label: {
