...
public struct GatewayConnectDeepLink: Codable, Sendable, Equatable {
    ...
    public static func fromSetupCode(_ code: String) -> GatewayConnectDeepLink? {
        ...
        let port = parsed.port ?? (tls ? 443 : 40705)
...
public enum DeepLinkParser {
    public static func parse(_ url: URL) -> DeepLinkRoute? {
        guard let scheme = url.scheme?.lowercased(),
              scheme == AlisioBranding.legacyDeepLinkScheme || scheme == AlisioBranding.canonicalDeepLinkScheme
        else {
            return nil
        }
        ...
        case "gateway":
            ...
            let port = query["port"].flatMap { Int($0) } ?? 40705
