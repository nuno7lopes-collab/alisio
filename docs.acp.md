...
This document describes how the Alisio ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`alisio acp` exposes an ACP agent over stdio and forwards prompts to a running
Alisio Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
runtime. It is designed to route IDE prompts into an existing Alisio Gateway
...
- ACP client spawns `alisio acp` and speaks ACP messages over stdio.
...
`alisio acp` resolves the Gateway URL and auth from CLI flags or config:
