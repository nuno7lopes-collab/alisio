...
elif [[ -n "${ALISIO_LIVE_GATEWAY_PROVIDERS:-}" ]]; then
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
  done < <(alisio_live_collect_auth_dirs_from_csv "${ALISIO_LIVE_GATEWAY_PROVIDERS:-}")
else
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
...
docker run --rm -t \
  ...
  -e ALISIO_LIVE_GATEWAY_MODELS="${ALISIO_LIVE_GATEWAY_MODELS:-modern}" \
  -e ALISIO_LIVE_GATEWAY_PROVIDERS="${ALISIO_LIVE_GATEWAY_PROVIDERS:-}" \
  -e ALISIO_LIVE_GATEWAY_MAX_MODELS="${ALISIO_LIVE_GATEWAY_MAX_MODELS:-24}" \
  -e ALISIO_LIVE_GATEWAY_MODEL_TIMEOUT_MS="${ALISIO_LIVE_GATEWAY_MODEL_TIMEOUT_MS:-}" \
  -v "$ROOT_DIR":/src:ro \
