...
import { parseReleaseVersion, resolveNpmCommandInvocation } from "./alisio-npm-release-check.ts";
...
export function buildPublishedInstallScenarios(version: string): PublishedInstallScenario[] {
  ...
  const exactSpec = `alisio@${version}`;
  ...
  if (parsed.channel === "stable" && parsed.correctionNumber !== undefined) {
    scenarios.push({
      ...
      installSpecs: [`alisio@${parsed.baseVersion}`, exactSpec],
      expectedVersion: version,
    });
  }
...
function verifyScenario(version: string, scenario: PublishedInstallScenario): void {
  const workingDir = mkdtempSync(join(tmpdir(), `alisio-postpublish-${scenario.name}.`));
  ...
  try {
    ...
    const packageRoot = join(globalRoot, "alisio");
    ...
    console.log(`alisio-npm-postpublish-verify: ${scenario.name} OK (${version})`);
  } finally {
...
function main(): void {
  ...
  if (!version) {
    throw new Error("Usage: node --import tsx scripts/alisio-npm-postpublish-verify.ts <version>");
  }
  ...
  console.log(
    `alisio-npm-postpublish-verify: verified published npm install paths for ${version}.`,
  );
}
...
if (entrypoint !== null && import.meta.url === entrypoint) {
  try {
    ...
    console.error(
      `alisio-npm-postpublish-verify: ${error instanceof Error ? error.message : String(error)}`,
    );
    process.exitCode = 1;
  }
}
