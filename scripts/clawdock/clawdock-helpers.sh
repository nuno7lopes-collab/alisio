#!/usr/bin/env bash
# ClawDock - Docker helpers for Alisio
...
# Common paths to check for Alisio
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    ...
    if [[ "$response" =~ ^[Nn] ]]; then
      ...
      echo "  export CLAWDOCK_DIR=/path/to/alisio"
      return 1
    fi
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^ALISIO_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d alisio-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart alisio-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f alisio-gateway
}
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec alisio-gateway \
...
clawdock-cli() {
  _clawdock_compose run --rm alisio-cli "$@"
}
...
clawdock-update() {
  ...
  _clawdock_compose build alisio-gateway || { echo "❌ Build failed"; return 1; }

  echo ""
  echo "♻️  Recreating container with new image..."
  _clawdock_compose down 2>&1 | _clawdock_filter_warnings
  _clawdock_compose up -d alisio-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-rebuild() {
  _clawdock_compose build alisio-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "ALISIO_GATEWAY_TOKEN=$token" alisio-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" alisio-gateway \
    bash -c './alisio.mjs config set gateway.remote.token "$TOKEN" && ./alisio.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec alisio-gateway \
    bash -c "./alisio.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart alisio-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm alisio-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec alisio-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for Alisio${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (alisio alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to alisio project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.alisio)${_CLR_RESET}"
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.alisio.pt${_CLR_RESET}"
  echo ""
}
