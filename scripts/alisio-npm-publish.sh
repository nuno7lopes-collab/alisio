...
if [[ "${mode}" != "--publish" ]]; then
  echo "usage: bash scripts/alisio-npm-publish.sh --publish" >&2
  exit 2
fi

package_version="$(node -p "require('./package.json').version")"
current_beta_version="$(npm view alisio dist-tags.beta 2>/dev/null || true)"
...
import { resolveNpmPublishPlan } from "./scripts/alisio-npm-release-check.ts";
...
if [[ -n "${mirror_dist_tags_csv}" ]]; then
  ...
  for dist_tag in "${mirror_dist_tags[@]}"; do
    [[ -n "${dist_tag}" ]] || continue
    echo "Mirroring alisio@${package_version} onto dist-tag ${dist_tag}"
    npm dist-tag add "alisio@${package_version}" "${dist_tag}"
  done
fi
