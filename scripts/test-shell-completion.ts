 ...
 * `alisio update`. Use it to verify the completion installation flow
...
\`alisio update\`, \`alisio doctor\`, and \`alisio onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(theme.muted("  1. Remove the '# Alisio Completion' block from your shell profile"));
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`alisio completion --install\` later to enable.`));
    return;
  }
