...
function parsePluginSdkSubpath(specifier) {
  ...
  const subpath = specifier.slice("alisio/plugin-sdk/".length);
  return subpath || null;
}
...
async function main() {
  ...
  if (violations.length === 0) {
    console.log("OK: all referenced alisio/plugin-sdk/<subpath> imports are exported.");
    return;
  }

  console.error(
    "Rule: every referenced alisio/plugin-sdk/<subpath> must exist in the public package exports.",
  );
