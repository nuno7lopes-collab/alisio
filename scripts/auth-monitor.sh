...
#
# Suggested cron: */30 * * * * /home/admin/alisio/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Alisio notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., alisio-alerts)
...
STATE_FILE="$HOME/.alisio/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Alisio if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use alisio
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Alisio to $NOTIFY_PHONE..."
            alisio send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Alisio is down. Run: ssh l36 '~/alisio/scripts/mobile-reauth.sh'" "urgent"
    exit 1
