...
const EXPECTED_REPOSITORY_URL = "https://github.com/alisio/alisio";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "alisio") {
    errors.push(`package.json name must be "alisio"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.alisio !== "alisio.mjs") {
    errors.push(`package.json bin.alisio must be "alisio.mjs"; found "${pkg.bin?.alisio ?? ""}".`);
  }
...
function main(): number {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`alisio-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `alisio-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta).`,
  );
  return 0;
}
