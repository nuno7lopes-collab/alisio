...
BASE_IMAGE="${BASE_IMAGE:-alisio-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-alisio-sandbox-common:bookworm-slim}"
...
ALISIO_DOCKER_BUILD_USE_BUILDX="${ALISIO_DOCKER_BUILD_USE_BUILDX:-0}"
ALISIO_DOCKER_BUILD_CACHE_FROM="${ALISIO_DOCKER_BUILD_CACHE_FROM:-}"
ALISIO_DOCKER_BUILD_CACHE_TO="${ALISIO_DOCKER_BUILD_CACHE_TO:-}"
...
if [ "${ALISIO_DOCKER_BUILD_USE_BUILDX}" = "1" ]; then
  build_cmd=(docker buildx build --load)
  if [ -n "${ALISIO_DOCKER_BUILD_CACHE_FROM}" ]; then
    build_cmd+=(--cache-from "${ALISIO_DOCKER_BUILD_CACHE_FROM}")
  fi
  if [ -n "${ALISIO_DOCKER_BUILD_CACHE_TO}" ]; then
    build_cmd+=(--cache-to "${ALISIO_DOCKER_BUILD_CACHE_TO}")
  fi
fi
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=alisio.sandbox=1)
NOTE
