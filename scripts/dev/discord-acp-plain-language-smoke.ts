...
type DriverMode = "token" | "webhook" | "alisio";
...
function resolveStateDir(): string {
  const override = process.env.ALISIO_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.ALISIO_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".alisio");
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver alisio] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) Alisio spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|alisio> Driver transport mode (default: token)\n" +
    ...
    "  --alisio-bin <path>        Alisio CLI binary for driver=alisio (default: alisio)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  ALISIO_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  ALISIO_DISCORD_SMOKE_DRIVER\n" +
    "  ALISIO_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  ALISIO_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  ALISIO_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  ALISIO_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  ALISIO_DISCORD_SMOKE_AGENT\n" +
    "  ALISIO_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  ALISIO_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  ALISIO_DISCORD_SMOKE_POLL_MS\n" +
    "  ALISIO_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  ALISIO_DISCORD_SMOKE_ALISIO_BIN"
  );
}

function parseArgs(): Args {
  const channelId = resolveArg("--channel") || process.env.ALISIO_DISCORD_SMOKE_CHANNEL_ID || "";
  const driverModeRaw =
    resolveArg("--driver") || process.env.ALISIO_DISCORD_SMOKE_DRIVER || "token";
  const normalizedDriverMode = driverModeRaw.trim().toLowerCase();
  const driverMode: DriverMode =
    normalizedDriverMode === "webhook"
      ? "webhook"
      : normalizedDriverMode === "alisio"
        ? "alisio"
        : normalizedDriverMode === "token"
          ? "token"
          : "token";
  const driverToken = resolveArg("--token") || process.env.ALISIO_DISCORD_SMOKE_DRIVER_TOKEN || "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.ALISIO_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.ALISIO_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") || process.env.ALISIO_DISCORD_SMOKE_BOT_TOKEN_PREFIX || "Bot";
  const targetAgent = resolveArg("--agent") || process.env.ALISIO_DISCORD_SMOKE_AGENT || "codex";
  const mentionUserId =
    resolveArg("--mention") || process.env.ALISIO_DISCORD_SMOKE_MENTION_USER_ID || undefined;
  const instruction =
    resolveArg("--instruction") || process.env.ALISIO_DISCORD_SMOKE_INSTRUCTION || undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.ALISIO_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.ALISIO_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.ALISIO_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const alisioBin =
    resolveArg("--alisio-bin") || process.env.ALISIO_DISCORD_SMOKE_ALISIO_BIN || "alisio";
...
async function alisioCliJson<T>(params: { alisioBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.alisioBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`alisio ${params.args.join(" ")} returned empty stdout`);
  }
  return JSON.parse(stdout) as T;
}

async function readMessagesWithCli(params: {
  alisioBin: string;
  target: string;
  limit: number;
}): Promise<DiscordMessage[]> {
  const response = await alisioCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    alisioBin: params.alisioBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "alisio") {
    return await readMessagesWithCli({
      alisioBin: params.args.alisioBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `alisio-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      const sent = await alisioCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
      ...
      if (!sentMessageId) {
        throw new Error("alisio message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    while (Date.now() < deadline && !ackMessage) {
      try {
        const threadMessages =
          args.driverMode === "alisio"
            ? await readMessagesWithCli({
                alisioBin: args.alisioBin,
                target: threadId,
                limit: 50,
              })
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from Alisio.`,
        diagnostics: {
          bindingCandidates: [
            {
