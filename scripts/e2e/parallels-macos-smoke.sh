...
AUTH_KEY_FLAG=""
MODEL_ID=""
INSTALL_URL="https://alisio.pt/install.sh"
HOST_PORT="18425"
...
SNAPSHOT_STATE=""
SNAPSHOT_NAME=""
GUEST_ALISIO_BIN="/opt/homebrew/bin/alisio"
GUEST_ALISIO_ENTRY="/opt/homebrew/lib/node_modules/alisio/alisio.mjs"
...
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/alisio-parallels-smoke.XXXXXX)"
...
Options:
  ...
  --install-url <url>        Installer URL for latest release. Default: https://alisio.pt/install.sh
...
spawn {*}$cmd
send -- "printf '__ALISIO_READY__\\n'\r"
expect "__ALISIO_READY__"
log_user 0
send -- "export PS1='' PROMPT='' PROMPT2='' RPROMPT=''\r"
send -- "stty -echo\r"

send -- "cat >/tmp/alisio-prl.sh <<'__ALISIO_SCRIPT__'\r"
send -- $script
if {![string match "*\n" $script]} {
  send -- "\r"
}
send -- "__ALISIO_SCRIPT__\r"
send -- "/bin/bash /tmp/alisio-prl.sh; rc=\$?; rm -f /tmp/alisio-prl.sh; printf '__ALISIO_RC__:%s\\n' \"\$rc\"; exit \"\$rc\"\r"
log_user 1

set rc 1
expect {
  -re {__ALISIO_RC__:(-?[0-9]+)} {
...
export ALISIO_NO_ONBOARD=1
curl -fsSL $install_url_q -o /tmp/alisio-install.sh
bash /tmp/alisio-install.sh${version_arg_q}
$GUEST_ALISIO_BIN --version
EOF
)"
}
...
verify_version_contains() {
  ...
  local version
  version="$(
    guest_current_user_exec "$GUEST_ALISIO_BIN" --version
  )"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
...
start_server() {
  ...
  ) >/tmp/alisio-parallels-http.log 2>&1 &
  SERVER_PID=$!
  sleep 1
...
verify_bundle_permissions() {
  ...
  cmd="$(cat <<EOF
root=\$($npm_q root -g); check_path() { local path="\$1"; [ -e "\$path" ] || return 0; local perm perm_oct; perm=\$(/usr/bin/stat -f '%OLp' "\$path"); perm_oct=\$((8#\$perm)); if (( perm_oct & 0002 )); then echo "world-writable install artifact: \$path (\$perm)" >&2; exit 1; fi; }; check_path "\$root/alisio"; check_path "\$root/alisio/extensions"; if [ -d "\$root/alisio/extensions" ]; then while IFS= read -r -d '' extension_dir; do check_path "\$extension_dir"; done < <(/usr/bin/find "\$root/alisio/extensions" -mindepth 1 -maxdepth 1 -type d -print0); fi
EOF
)"
...
verify_gateway() {
  guest_current_user_cli "$GUEST_ALISIO_BIN" gateway status --deep --require-rpc
}

show_gateway_status_compat() {
  if guest_current_user_cli "$GUEST_ALISIO_BIN" gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_current_user_cli "$GUEST_ALISIO_BIN" gateway status --deep --require-rpc
    return
  fi
  guest_current_user_cli "$GUEST_ALISIO_BIN" gateway status --deep
}

verify_turn() {
  guest_current_user_cli "$GUEST_ALISIO_BIN" models set "$MODEL_ID"
  guest_current_user_cli \
    /usr/bin/env "$API_KEY_ENV=$API_KEY_VALUE" \
    "$GUEST_ALISIO_BIN" agent \
    --agent main \
...
resolve_dashboard_url() {
  local dashboard_url
  dashboard_url="$(
    guest_current_user_cli "$GUEST_ALISIO_BIN" dashboard --no-open \
      | awk '/^Dashboard URL: / { sub(/^Dashboard URL: /, ""); print; exit }'
  )"
  dashboard_url="${dashboard_url//$'\r'/}"
  dashboard_url="${dashboard_url//$'\n'/}"
  [[ -n "$dashboard_url" ]] || {
    echo "failed to resolve dashboard URL from alisio dashboard --no-open" >&2
    return 1
  }
  printf '%s\n' "$dashboard_url"
}
...
while [ \$SECONDS -lt \$deadline ]; do
  if curl -fsSL "\$dashboard_http_url" >/tmp/alisio-dashboard-smoke.html 2>/dev/null; then
    if grep -F '<title>Alisio Control</title>' /tmp/alisio-dashboard-smoke.html >/dev/null; then
      if grep -F '<alisio-app></alisio-app>' /tmp/alisio-dashboard-smoke.html >/dev/null; then
        dashboard_ready=1
        break
      fi
    fi
...
grep -F '<title>Alisio Control</title>' /tmp/alisio-dashboard-smoke.html >/dev/null
grep -F '<alisio-app></alisio-app>' /tmp/alisio-dashboard-smoke.html >/dev/null
...
PY
  ...
  script="$(cat <<EOF
cat >/tmp/alisio-discord-token <<'__ALISIO_TOKEN__'
$DISCORD_TOKEN_VALUE
__ALISIO_TOKEN__
cat >/tmp/alisio-discord-guilds.json <<'__ALISIO_GUILDS__'
$guilds_json
__ALISIO_GUILDS__
token="\$(tr -d '\n' </tmp/alisio-discord-token)"
guilds_json="\$(cat /tmp/alisio-discord-guilds.json)"
$GUEST_NODE_BIN $GUEST_ALISIO_ENTRY config set channels.discord.token "\$token"
$GUEST_NODE_BIN $GUEST_ALISIO_ENTRY config set channels.discord.enabled true
$GUEST_NODE_BIN $GUEST_ALISIO_ENTRY config set channels.discord.groupPolicy allowlist
$GUEST_NODE_BIN $GUEST_ALISIO_ENTRY config set channels.discord.guilds "\$guilds_json" --strict-json
$GUEST_NODE_BIN $GUEST_ALISIO_ENTRY gateway restart
for _ in 1 2 3 4 5 6 7 8; do
  if $GUEST_NODE_BIN $GUEST_ALISIO_ENTRY gateway status --deep --require-rpc >/dev/null 2>&1; then
    break
  fi
  sleep 2
done
$GUEST_NODE_BIN $GUEST_ALISIO_ENTRY channels status --probe --json
rm -f /tmp/alisio-discord-token /tmp/alisio-discord-guilds.json
EOF
)"
  prlctl exec "$VM_NAME" --current-user /usr/bin/env \
    PATH=/opt/homebrew/bin:/opt/homebrew/opt/node/bin:/opt/homebrew/sbin:/usr/bin:/bin:/usr/sbin:/sbin \
...
run_fresh_main_lane() {
  ...
  phase_run "fresh.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "alisio-main-fresh.tgz"
...
run_upgrade_lane() {
  ...
  phase_run "upgrade.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "alisio-main-upgrade.tgz"
