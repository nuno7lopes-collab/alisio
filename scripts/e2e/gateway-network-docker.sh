...
docker run -d \
  ...
  "$IMAGE_NAME" \
  bash -lc "set -euo pipefail; entry=dist/index.mjs; [ -f \"\$entry\" ] || entry=dist/index.js; node \"\$entry\" config set gateway.controlUi.enabled false >/dev/null; node \"\$entry\" gateway run --port $PORT --bind lan --allow-unconfigured > /tmp/gateway-net-e2e.log 2>&1"
