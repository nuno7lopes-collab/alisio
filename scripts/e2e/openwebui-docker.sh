...
IMAGE_NAME="alisio-openwebui-e2e"
OPENWEBUI_IMAGE="${OPENWEBUI_IMAGE:-ghcr.io/open-webui/open-webui:v0.8.10}"
PROFILE_FILE="${ALISIO_PROFILE_FILE:-$HOME/.profile}"
MODEL="${ALISIO_OPENWEBUI_MODEL:-openai/gpt-5.4}"
PROMPT_NONCE="OPENWEBUI_DOCKER_E2E_$(date +%s)_$$"
PROMPT="${ALISIO_OPENWEBUI_PROMPT:-Reply with exactly this token and nothing else: ${PROMPT_NONCE}}"
PORT="${ALISIO_OPENWEBUI_GATEWAY_PORT:-18789}"
WEBUI_PORT="${ALISIO_OPENWEBUI_PORT:-8080}"
TOKEN="openwebui-e2e-$(date +%s)-$$"
ADMIN_EMAIL="${ALISIO_OPENWEBUI_ADMIN_EMAIL:-openwebui-e2e@example.com}"
ADMIN_PASSWORD="${ALISIO_OPENWEBUI_ADMIN_PASSWORD:-OpenWebUI-E2E-Password-$(date +%s)-$$}"
NET_NAME="alisio-openwebui-e2e-$$"
GW_NAME="alisio-openwebui-gateway-$$"
OW_NAME="alisio-openwebui-$$"
...
docker run -d \
  ...
  -e "ALISIO_GATEWAY_TOKEN=$TOKEN" \
  -e "ALISIO_OPENWEBUI_MODEL=$MODEL" \
  -e "ALISIO_SKIP_CHANNELS=1" \
  -e "ALISIO_SKIP_GMAIL_WATCHER=1" \
  -e "ALISIO_SKIP_CRON=1" \
  -e "ALISIO_SKIP_CANVAS_HOST=1" \
  "${EXTERNAL_AUTH_MOUNTS[@]}" \
  "${PROFILE_MOUNT[@]}" \
  "$IMAGE_NAME" \
  bash -lc '
    ...
    node "$entry" config set gateway.auth.token "$ALISIO_GATEWAY_TOKEN" >/dev/null
...
docker run -d \
  ...
  --network "$NET_NAME" \
  -e ENV=prod \
  -e WEBUI_NAME="Alisio E2E" \
  -e WEBUI_SECRET_KEY="alisio-openwebui-e2e-secret" \
  -e OFFLINE_MODE=True \
  ...
  -e WEBUI_ADMIN_NAME="Alisio E2E" \
  -e ENABLE_SIGNUP=False \
  -e DEFAULT_MODELS="alisio/default" \
...
echo "Running Open WebUI -> Alisio smoke..."
docker exec \
