...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/alisio-bin:$PATH"
  mkdir -p /tmp/alisio-bin
  cat > /tmp/alisio-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  ...
  run_wizard_cmd() {
    ...
    set_isolated_alisio_env "$home_dir"

    input_fifo="$(mktemp -u "/tmp/alisio-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/alisio-onboard-${case_name}.log"
  ...
  run_wizard() {
    ...
    local validate_fn="${4:-}"
	    ...
	    run_wizard_cmd "$case_name" "$home_dir" "node \"$ALISIO_ENTRY\" onboard $ONBOARD_FLAGS" "$send_fn" true "$validate_fn"
	  }

  make_home() {
    mktemp -d "/tmp/alisio-e2e-$1.XXXXXX"
  }

  set_isolated_alisio_env() {
    local home_dir="$1"
    export HOME="$home_dir"
    export ALISIO_HOME="$home_dir"
    export ALISIO_STATE_DIR="$home_dir/.alisio"
    export ALISIO_CONFIG_PATH="$ALISIO_STATE_DIR/alisio.json"
    mkdir -p "$ALISIO_STATE_DIR"
  }
  ...
  run_case_local_basic() {
    ...
    workspace_dir="$ALISIO_STATE_DIR/workspace"
    config_path="$ALISIO_CONFIG_PATH"
    sessions_dir="$ALISIO_STATE_DIR/agents/main/sessions"
...
NODE
  ...
  run_case_channels() {
	    ...
	    run_wizard_cmd channels "$home_dir" "node \"$ALISIO_ENTRY\" configure --section channels" send_channels_flow
...
JSON
	    ...
	    run_wizard_cmd skills "$home_dir" "node \"$ALISIO_ENTRY\" configure --section skills" send_skills_flow
