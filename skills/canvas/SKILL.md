...
```
http://<tailscale-hostname>:40709/__alisio__/canvas/<file>.html
```
...
```bash
cat ~/.alisio/alisio.json | jq '.gateway.bind'
```

Then construct the URL:

- **loopback**: `http://127.0.0.1:40709/__alisio__/canvas/<file>.html`
- **lan/tailnet/auto**: `http://<hostname>:40709/__alisio__/canvas/<file>.html`
...
```
canvas action:present node:mac-63599bc4-b54d-4392-9048-b97abd58343a target:http://peters-mac-studio-1.sheep-coho.ts.net:40709/__alisio__/canvas/snake.html
```
...
1. Check server bind: `cat ~/.alisio/alisio.json | jq '.gateway.bind'`
2. Check what port canvas is on: `lsof -i :40709`
3. Test URL directly: `curl http://<hostname>:40709/__alisio__/canvas/<file>.html`
...
Node is offline. Use `alisio nodes list` to find online nodes.
...
The canvas host serves from `/__alisio__/canvas/` prefix:

```
http://<host>:40709/__alisio__/canvas/index.html  → ~/clawd/canvas/index.html
http://<host>:40709/__alisio__/canvas/games/snake.html → ~/clawd/canvas/games/snake.html
```

The `/__alisio__/canvas/` prefix is defined by `CANVAS_HOST_PATH` constant.
