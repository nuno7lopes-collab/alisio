---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Alisio deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Alisio cron scheduling for periodic checks, or version status checks on a machine running Alisio (laptop, workstation, Pi, VPS).
---

# Alisio Host Hardening

## Overview

Assess and harden the host running Alisio, then align it to a user-defined risk tolerance without breaking access. Use Alisio security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Alisio changes the host firewall, SSH, or OS updates; it does not.
...
5. Alisio gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Alisio can access sensitive data.
...
- Credential hygiene notes (location of Alisio creds, prefer disk encryption)
...
Follow the durable-memory prompt format used by Alisio compaction:
