...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `alisio-e2e-${name}-`));
  const configDir = path.join(homeDir, ".alisio");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          ALISIO_SKIP_PROVIDERS: "1",
          ALISIO_SKIP_GMAIL_WATCHER: "1",
          ALISIO_SKIP_CRON: "1",
          ALISIO_SKIP_BROWSER_CONTROL_SERVER: "1",
          ALISIO_SKIP_CANVAS_HOST: "1",
          ALISIO_TEST_MINIMAL_GATEWAY: "1",
          VITEST: "1",
        },
