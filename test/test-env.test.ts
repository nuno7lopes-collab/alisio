...
function createTempHome(): string {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "alisio-test-env-real-home-"));
...
describe("installTestEnv", () => {
  it("keeps live tests on a temp HOME while copying config and auth state", () => {
    ...
    writeFile(
      path.join(realHome, "custom-alisio.json5"),
      `{
        // Preserve provider config, strip host-bound paths.
        agents: {
          defaults: {
            workspace: "/Users/peter/Projects",
            agentDir: "/Users/peter/.alisio/agents/main/agent",
          },
          list: [
            {
              id: "dev",
              workspace: "/Users/peter/dev-workspace",
              agentDir: "/Users/peter/.alisio/agents/dev/agent",
            },
          ],
        },
        models: {
    ...
    writeFile(path.join(realHome, ".alisio", "credentials", "token.txt"), "secret\n");
    writeFile(
      path.join(realHome, ".alisio", "agents", "main", "agent", "auth-profiles.json"),
    ...
    process.env.ALISIO_CONFIG_PATH = "~/custom-alisio.json5";
    process.env.ALISIO_TEST_HOME = priorIsolatedHome;
    process.env.ALISIO_STATE_DIR = path.join(priorIsolatedHome, ".alisio");
    ...
    expect(process.env.ALISIO_TEST_HOME).toBe(testEnv.tempHome);
    expect(process.env.TEST_PROFILE_ONLY).toBe("from-profile");

    const copiedConfigPath = path.join(testEnv.tempHome, ".alisio", "alisio.json");
    ...
    expect(fs.existsSync(path.join(testEnv.tempHome, ".alisio", "credentials", "token.txt"))).toBe(
      true,
    );
    expect(
      fs.existsSync(
        path.join(testEnv.tempHome, ".alisio", "agents", "main", "agent", "auth-profiles.json"),
      ),
    ).toBe(true);
  ...
  it("accepts legacy Alisio env aliases when staging live state", () => {
    ...
    expect(fs.existsSync(path.join(testEnv.tempHome, ".alisio", "alisio.json"))).toBe(true);
  });
  ...
  it("allows explicit live runs against the real HOME", () => {
    ...
    process.env.USERPROFILE = realHome;
    process.env.ALISIO_LIVE_TEST = "1";
    process.env.ALISIO_LIVE_USE_REAL_HOME = "1";
    process.env.ALISIO_LIVE_TEST_QUIET = "1";
