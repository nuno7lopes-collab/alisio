...
describe("loadVitestExperimentalConfig", () => {
  ...
  it("passes through the filesystem module cache path when provided", () => {
    expect(
      loadVitestExperimentalConfig(
        {
          ALISIO_VITEST_FS_MODULE_CACHE_PATH: "/tmp/alisio-vitest-cache",
        },
        "linux",
      ),
    ).toEqual({
      experimental: {
        fsModuleCache: true,
        fsModuleCachePath: "/tmp/alisio-vitest-cache",
      },
    });
  });
  ...
  it("enables import timing output and import breakdown reporting", () => {
    expect(
      loadVitestExperimentalConfig(
        {
          ALISIO_VITEST_IMPORT_DURATIONS: "true",
          ALISIO_VITEST_PRINT_IMPORT_BREAKDOWN: "1",
        },
        "linux",
      ),
    ).toEqual({
