...
function normalizeDenyList(cfg: AlisioPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: AlisioPluginApi["config"]): string[] {
...
export default definePluginEntry({
  ...
  register(api: AlisioPluginApi) {
    ...
    const timerService: AlisioPluginService = {
