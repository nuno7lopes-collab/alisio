...
import type { AlisioPluginCommandDefinition, PluginCommandContext } from "alisio/plugin-sdk/core";
import { afterEach, beforeEach, describe, expect, it, vi } from "vitest";
import { createTestPluginApi } from "../../test/helpers/plugins/plugin-api.js";
import type { AlisioPluginApi } from "./api.js";
...
const pluginApiMocks = vi.hoisted(() => ({
  ...
  resolvePreferredAlisioTmpDir: vi.fn(() => path.join(os.tmpdir(), "alisio-device-pair-tests")),
}));
...
vi.mock("./api.js", () => {
  return {
    ...
    resolvePreferredAlisioTmpDir: pluginApiMocks.resolvePreferredAlisioTmpDir,
...
function createApi(params?: {
  runtime?: AlisioPluginApi["runtime"];
  ...
  return createTestPluginApi({
    ...
    runtime: (params?.runtime ?? {}) as AlisioPluginApi["runtime"],
...
function createChannelRuntime(
  ...
  } as unknown as AlisioPluginApi["runtime"];
}
...
describe("device-pair /pair qr", () => {
  beforeEach(async () => {
    ...
    await fs.mkdir(pluginApiMocks.resolvePreferredAlisioTmpDir(), { recursive: true });
  });

  afterEach(async () => {
    await fs.rm(pluginApiMocks.resolvePreferredAlisioTmpDir(), { recursive: true, force: true });
  });
