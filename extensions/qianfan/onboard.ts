...
function resolveQianfanPreset(cfg: AlisioConfig): {
  api: ModelApi;
  baseUrl: string;
...
const qianfanPresetAppliers = createDefaultModelsPresetAppliers({
  ...
  resolveParams: (cfg: AlisioConfig) => {
...
export function applyQianfanProviderConfig(cfg: AlisioConfig): AlisioConfig {
  return qianfanPresetAppliers.applyProviderConfig(cfg);
}

export function applyQianfanConfig(cfg: AlisioConfig): AlisioConfig {
