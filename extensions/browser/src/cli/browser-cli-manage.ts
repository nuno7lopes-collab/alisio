...
async function runBrowserToggle(
  ...
  const name = status.profile ?? "alisio";
...
export function registerBrowserManageCommands(
        ...
        defaultRuntime.log(
          profiles
            .map((p) => {
              ...
              const driver = p.driver !== "alisio" ? ` [${p.driver}]` : "";
  ...
  browser
    ...
    .option("--driver <driver>", "Profile driver (alisio|existing-session). Default: alisio")
    .action(
      async (
        opts: {
          name: string;
