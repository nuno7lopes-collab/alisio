  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          alisio: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  ...
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.ALISIO_GATEWAY_PORT;
    process.env.ALISIO_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.ALISIO_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.ALISIO_GATEWAY_PASSWORD;
