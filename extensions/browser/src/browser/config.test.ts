...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    expect(profile?.name).toBe("alisio");
    expect(profile?.driver).toBe("alisio");
    expect(profile?.cdpPort).toBe(40716);
    expect(profile?.cdpUrl).toBe("http://127.0.0.1:40716");

    const alisio = resolveProfile(resolved, "alisio");
    expect(alisio?.driver).toBe("alisio");
    expect(alisio?.cdpPort).toBe(40716);
    expect(alisio?.cdpUrl).toBe("http://127.0.0.1:40716");
  ...
  it("derives default ports from ALISIO_GATEWAY_PORT when unset", () => {
    withEnv({ ALISIO_GATEWAY_PORT: "19001" }, () => {
      const resolved = resolveBrowserConfig(undefined);
      expect(resolved.controlPort).toBe(19003);
      expect(resolveProfile(resolved, "chrome-relay")).toBe(null);

      const alisio = resolveProfile(resolved, "alisio");
      expect(alisio?.cdpPort).toBe(19012);
      expect(alisio?.cdpUrl).toBe("http://127.0.0.1:19012");
    });
  });

  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ ALISIO_GATEWAY_PORT: undefined }, () => {
      const resolved = resolveBrowserConfig(undefined, { gateway: { port: 19011 } });
      expect(resolved.controlPort).toBe(19013);
      expect(resolveProfile(resolved, "chrome-relay")).toBe(null);

      const alisio = resolveProfile(resolved, "alisio");
      expect(alisio?.cdpPort).toBe(19022);
      expect(alisio?.cdpUrl).toBe("http://127.0.0.1:19022");
    });
  });
  ...
  it("supports overriding the local CDP auto-allocation range start", () => {
    ...
    expect(alisio?.cdpPort).toBe(19000);
    expect(alisio?.cdpUrl).toBe("http://127.0.0.1:19000");
  });
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "alisio");
  ...
  it("sets usesChromeMcp only for existing-session profiles", () => {
    ...
    const managed = resolveProfile(resolved, "alisio")!;
  ...
  describe("default profile preference", () => {
    it("defaults to alisio profile when defaultProfile is not configured", () => {
      ...
      expect(resolved.defaultProfile).toBe("alisio");
    });

    it("keeps alisio default when headless=true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
      });
      expect(resolved.defaultProfile).toBe("alisio");
    ...
    it("keeps alisio default when noSandbox=true", () => {
      const resolved = resolveBrowserConfig({
        noSandbox: true,
      });
      expect(resolved.defaultProfile).toBe("alisio");
    ...
    it("keeps alisio default when both headless and noSandbox are true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
        noSandbox: true,
      });
