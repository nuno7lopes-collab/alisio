...
export function resolveAlisioUserDataDir(profileName = DEFAULT_ALISIO_BROWSER_PROFILE_NAME) {
...
export function buildAlisioChromeLaunchArgs(params: {
...
export async function launchAlisioChrome(
  resolved: ResolvedBrowserConfig,
  profile: ResolvedBrowserProfile,
  ...
  const userDataDir = resolveAlisioUserDataDir(profile.name);
  fs.mkdirSync(userDataDir, { recursive: true });

  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_ALISIO_BROWSER_COLOR).toUpperCase(),
  );
  ...
  if (needsDecorate) {
    try {
      decorateAlisioProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
      log.info(`🦞 alisio browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`alisio browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`alisio browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  log.info(
    `🦞 alisio browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
