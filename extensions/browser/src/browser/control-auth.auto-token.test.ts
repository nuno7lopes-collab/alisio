...
const mocks = vi.hoisted(() => ({
  loadConfig: vi.fn<() => AlisioConfig>(),
  resolveGatewayAuth: vi.fn(
    ({
      authConfig,
    }: {
      authConfig?: NonNullable<NonNullable<AlisioConfig["gateway"]>["auth"]> | undefined;
    }) => {
      const token =
  ...
  ensureGatewayStartupAuth: vi.fn(async ({ cfg }: { cfg: AlisioConfig }) => ({
    cfg: {
      ...cfg,
      gateway: {
        ...cfg.gateway,
