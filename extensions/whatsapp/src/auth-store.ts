...
import { info, success } from "alisio/plugin-sdk/runtime-env";
import { getChildLogger } from "alisio/plugin-sdk/runtime-env";
import { defaultRuntime, type RuntimeEnv } from "alisio/plugin-sdk/runtime-env";
import { resolveOAuthDir } from "alisio/plugin-sdk/state-paths";
import type { WebChannel } from "alisio/plugin-sdk/text-runtime";
import { resolveUserPath } from "alisio/plugin-sdk/text-runtime";
...
export async function pickWebChannel(
  ...
  if (!hasWeb) {
    throw new Error(
      `No WhatsApp Web session found. Run \`${formatCliCommand("alisio channels login --channel whatsapp --verbose")}\` to link.`,
    );
  }
  return choice;
}
