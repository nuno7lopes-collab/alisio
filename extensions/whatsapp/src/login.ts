...
import { danger, info, success } from "alisio/plugin-sdk/runtime-env";
import { defaultRuntime, type RuntimeEnv } from "alisio/plugin-sdk/runtime-env";
import { logInfo } from "alisio/plugin-sdk/text-runtime";
...
export async function loginWeb(
  ...
  try {
    ...
    if (code === LOGGED_OUT_STATUS) {
      ...
      console.error(
        danger(
          `WhatsApp reported the session is logged out. Cleared cached web session; please rerun ${formatCliCommand("alisio channels login")} and scan the QR again.`,
        ),
      );
