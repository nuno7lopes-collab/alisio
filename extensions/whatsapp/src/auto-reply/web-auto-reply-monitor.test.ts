...
beforeEach(async () => {
  sessionDir = await fs.mkdtemp(path.join(os.tmpdir(), "alisio-group-gating-"));
...
const makeConfig = (overrides: Record<string, unknown>) =>
  ({
    ...
    ...overrides,
  }) as unknown as ReturnType<typeof import("alisio/plugin-sdk/config-runtime").loadConfig>;

function runGroupGating(params: {
  cfg: ReturnType<typeof import("alisio/plugin-sdk/config-runtime").loadConfig>;
...
function makeInboundCfg(messagePrefix = "") {
  return {
    agents: { defaults: { workspace: "/tmp/alisio" } },
