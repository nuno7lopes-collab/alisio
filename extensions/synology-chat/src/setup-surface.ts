...
function getChannelConfig(cfg: AlisioConfig): SynologyChatChannelConfig {
  return (cfg.channels?.[channel] as SynologyChatChannelConfig | undefined) ?? {};
}

function getRawAccountConfig(cfg: AlisioConfig, accountId: string): SynologyChatAccountRaw {
...
function isSynologyChatConfigured(cfg: AlisioConfig, accountId: string): boolean {
...
function resolveExistingAllowedUserIds(cfg: AlisioConfig, accountId: string): string[] {
  ...
  textInputs: [
    {
      ...
      helpLines: [
        ...
        "This is the URL Alisio uses to send replies back to Chat.",
      ],
