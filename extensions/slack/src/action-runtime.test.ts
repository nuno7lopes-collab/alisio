...
describe("handleSlackAction", () => {
  function slackConfig(overrides?: Record<string, unknown>): AlisioConfig {
    return {
      channels: {
        slack: {
          botToken: "tok",
  ...
  function createReplyToFirstScenario() {
    const cfg = { channels: { slack: { botToken: "tok" } } } as AlisioConfig;
  ...
  async function resolveReadToken(cfg: AlisioConfig): Promise<string | undefined> {
  ...
  async function resolveSendToken(cfg: AlisioConfig): Promise<string | undefined> {
    sendSlackMessage.mockClear();
