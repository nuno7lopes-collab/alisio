...
// Prefix for Alisio-generated action IDs to scope our handler
const ALISIO_ACTION_PREFIX = "alisio:";
...
export function registerSlackInteractionEvents(params: { ctx: SlackMonitorContext }) {
  ...
  const modalMatcher = new RegExp(`^${ALISIO_ACTION_PREFIX}`);

  // Handle Alisio modal submissions with callback_ids scoped by our prefix.
  registerModalLifecycleHandler({
