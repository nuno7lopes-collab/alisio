...
let resetInboundDedupe: typeof import("alisio/plugin-sdk/reply-runtime").resetInboundDedupe;
...
beforeEach(async () => {
  vi.resetModules();
  ({ resetInboundDedupe } = await import("alisio/plugin-sdk/reply-runtime"));
...
describe("monitorSlackProvider tool results", () => {
  ...
  async function expectMentionPatternMessageAccepted(text: string): Promise<void> {
    setRequireMentionChannelConfig(["\\balisio\\b"]);
  ...
  it("accepts channel messages when mentionPatterns match", async () => {
    await expectMentionPatternMessageAccepted("alisio: hello");
  });

  it("accepts channel messages when mentionPatterns match even if another user is mentioned", async () => {
    await expectMentionPatternMessageAccepted("alisio: hello <@U2>");
  });
