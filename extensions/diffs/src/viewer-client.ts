...
function getCards(): Array<{ host: HTMLElement; payload: DiffViewerPayload }> {
  ...
  for (const card of document.querySelectorAll<HTMLElement>(".oc-diff-card")) {
    const host = card.querySelector<HTMLElement>("[data-alisio-diff-host]");
    const payloadNode = card.querySelector<HTMLScriptElement>("[data-alisio-diff-payload]");
    if (!host || !payloadNode) {
      continue;
    }
...
async function main(): Promise<void> {
  try {
    await hydrateViewer();
    document.documentElement.dataset.alisioDiffsReady = "true";
  } catch (error) {
    document.documentElement.dataset.alisioDiffsError = "true";
