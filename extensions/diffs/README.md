...
Read-only diff viewer plugin for **Alisio** agents.
...
Set plugin-wide defaults in `~/.alisio/alisio.json`:
...
Before:
Alisio supports plugins.

After:
Alisio supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/alisio-diffs`).
