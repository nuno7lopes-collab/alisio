...
const ALISIO_OPENSHELL_E2E = process.env.ALISIO_E2E_OPENSHELL === "1";
const ALISIO_OPENSHELL_E2E_TIMEOUT_MS = 12 * 60_000;
const ALISIO_OPENSHELL_COMMAND = process.env.ALISIO_E2E_OPENSHELL_COMMAND?.trim() || "openshell";
...
RUN echo "alisio-openshell-e2e" > /opt/openshell-e2e-marker.txt
...
describe("openshell sandbox backend e2e", () => {
  it.runIf(process.platform !== "win32" && ALISIO_OPENSHELL_E2E)(
    "creates a remote-canonical sandbox through OpenShell and executes over SSH",
    { timeout: ALISIO_OPENSHELL_E2E_TIMEOUT_MS },
    async () => {
      if (!(await dockerReady())) {
        return;
      }
      if (!(await commandAvailable(ALISIO_OPENSHELL_COMMAND))) {
        return;
      }

      const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "alisio-openshell-e2e-"));
      ...
      const gatewayName = `alisio-e2e-${scopeSuffix}`;
      const scopeKey = `session:openshell-e2e-deny:${scopeSuffix}`;
      const allowSandboxName = `alisio-policy-allow-${scopeSuffix}`;
      ...
      const sandboxCfg = {
        ...
        ssh: createSandboxSshConfig("/tmp/alisio-sandboxes"),
      ...
      const pluginConfig = resolveOpenShellPluginConfig({
        command: ALISIO_OPENSHELL_COMMAND,
        gateway: gatewayName,
        from: dockerfilePath,
        mode: "remote",
        autoProviders: false,
      ...
      try {
        ...
        expect(stdout).toContain("alisio-openshell-e2e");
