...
export async function handleNextcloudTalkInbound(params: {
  ...
  const defaultGroupPolicy = resolveDefaultGroupPolicy(config as AlisioConfig);
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as AlisioConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as AlisioConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as AlisioConfig);
