...
const ALISIO_PLUGIN_SDK_PREFIX = ["alisio", "plugin-sdk"].join("/");
...
function findAlisioPackageRoot(startDir) {
  let cursor = path.resolve(startDir);
  for (let i = 0; i < 12; i += 1) {
    const pkg = readPackageJson(cursor);
    if (pkg?.name === "alisio" && pkg.exports?.["./plugin-sdk"]) {
...
function buildPluginSdkAliasMap(moduleUrl) {
  const location = findAlisioPackageRoot(path.dirname(fileURLToPath(moduleUrl)));
  if (!location) {
    return {};
  }
  ...
  if (rootAlias) {
    aliasMap[ALISIO_PLUGIN_SDK_PREFIX] = rootAlias;
  }
  ...
  for (const exportKey of Object.keys(packageJson.exports ?? {})) {
    ...
    if (resolvedPath) {
      aliasMap[`${ALISIO_PLUGIN_SDK_PREFIX}/${subpath}`] = resolvedPath;
    }
  }
  ...
  if (extensionApi) {
    aliasMap["alisio/extension-api"] = extensionApi;
  }
