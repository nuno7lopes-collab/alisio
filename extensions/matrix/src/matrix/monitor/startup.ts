...
export async function runMatrixStartupMaintenance(params: {
  ...
  try {
    ...
    if (deviceHealth.staleAlisioDevices.length > 0) {
      params.logger.warn(
        `matrix: stale Alisio devices detected for ${params.auth.userId}: ${deviceHealth.staleAlisioDevices.map((device) => device.deviceId).join(", ")}. Run 'alisio matrix devices prune-stale --account ${params.effectiveAccountId}' to keep encrypted-room trust healthy.`,
      );
    }
  } catch (err) {
