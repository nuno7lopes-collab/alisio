...
function formatMatrixSelfDecryptionHint(accountId: string): string {
  return (
    ...
    "This usually means another Matrix device did not share the room key, or another Alisio runtime is using the same account. " +
    `Check 'alisio matrix verify status --verbose --account ${accountId}' and 'alisio matrix devices list --account ${accountId}'.`
  );
}
