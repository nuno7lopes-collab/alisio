...
metadata: { "alisio": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Alisio `sessions_spawn`
- **File I/O** == Alisio `read`/`write`
- **Remote fetch** == Alisio `web_fetch` (or `exec` with curl when POST is required)
