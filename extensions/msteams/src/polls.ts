...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["alisioPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["alisio", "pollId"]) ??
    readNestedString(value, ["alisio", "poll", "id"]) ??
    readNestedString(value, ["data", "alisioPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "alisio", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        msteams: {
          type: "messageBack",
          text: "alisio poll vote",
          displayText: "Vote recorded",
          value: { alisioPollId: pollId, pollId },
        },
      },
    ],
  };
