...
`alisio.invoke --each`) without adding new Alisio code per workflow.
...
This extension depends on Alisio internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Alisio extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.alisio/extensions` as a standalone plugin directory.
