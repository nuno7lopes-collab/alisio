...
description: Triage GitHub security advisories for Alisio with high-confidence close/keep decisions, exact tag and commit verification, trust-model checks, optional hardening notes, and a final reply ready to post and copy to clipboard.
---

# Security Triage

Use when reviewing Alisio security advisories, drafts, or GHSA reports.
...
1. Read `SECURITY.md`.
2. Read the GHSA body with `gh api /repos/alisio/alisio/security-advisories/<GHSA>`.
...
```bash
gh api /repos/alisio/alisio/security-advisories/<GHSA>
gh api /repos/alisio/alisio/security-advisories --paginate
git tag --sort=-creatordate | head -n 20
npm view alisio version --userconfig "$(mktemp)"
git tag --contains <commit>
git show <tag>:<path>
gh search issues --repo alisio/alisio --match title,body,comments -- "<terms>"
gh search prs --repo alisio/alisio --match title,body,comments -- "<terms>"
```
