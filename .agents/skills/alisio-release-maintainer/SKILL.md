---
name: alisio-release-maintainer
description: Maintainer workflow for Alisio releases, prereleases, changelog release notes, and publish validation. Use when Codex needs to prepare or verify stable or beta release steps, align version naming, assemble release notes, check release auth requirements, or validate publish-time commands and artifacts.
---
...
- Core `alisio` publish is manual `workflow_dispatch`; creating or pushing a tag does not publish by itself.
...
- The production Sparkle feed lives at `https://raw.githubusercontent.com/alisio/alisio/main/appcast.xml`, and the canonical published file is `appcast.xml` on `main` in the `alisio` repo.
...
- When cutting a mac release with a beta GitHub prerelease:
  ...
  - create a prerelease titled `alisio YYYY.M.D-beta.N`
...
```bash
node --import tsx scripts/alisio-npm-postpublish-verify.ts <published-version>
```
...
- Always validate the Alisio npm release path before creating the tag.
...
- Include mac release readiness in preflight by running the public validation
  workflow in `alisio/alisio` and the real mac preflight in
  `alisio/releases-private` for every release.
...
- Alisio publish uses GitHub trusted publishing.
...
- `.github/workflows/macos-release.yml` in `alisio/alisio` is now a
...
- Real mac preflight and real mac publish both use
  `alisio/releases-private/.github/workflows/alisio-macos-publish.yml`.
...
- The `npm-release` environment must be approved by `@alisio/alisio-release-managers` before publish continues.
- Mac publish uses
  `alisio/releases-private/.github/workflows/alisio-macos-publish.yml` for
...
- Real private mac publish uploads the packaged `.zip`, `.dmg`, and
  `.dSYM.zip` assets to the existing GitHub release in `alisio/alisio`
  automatically when `ALISIO_PUBLIC_REPO_RELEASE_TOKEN` is present in the
...
- Do not use `NPM_TOKEN` or the plugin OTP flow for Alisio releases.
- `@alisio/*` plugin publishes use a separate maintainer-only flow.
...
9. Start `.github/workflows/alisio-npm-release.yml` with `preflight_only=true`
   and wait for it to pass.
10. Start `.github/workflows/macos-release.yml` in `alisio/alisio` and wait
...
13. Start `.github/workflows/alisio-npm-release.yml` with the same tag for
    the real publish.
14. Wait for `npm-release` approval from `@alisio/alisio-release-managers`.
15. Start
    `alisio/releases-private/.github/workflows/alisio-macos-publish.yml`
...
- Use `alisio-ghsa-maintainer` for GHSA advisory inspection, patch/publish flow, private-fork validation, and GHSA API-specific publish checks.
