...
```bash
export ALISIO_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.alisio/alisio.json' | tr -d '\n'
)"

pnpm test:parallels:macos \
  --discord-token-env ALISIO_PARALLELS_DISCORD_TOKEN \
...
- Use the `alisio` wrapper for guest `message send/read`; `node alisio.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/alisio-parallels-smoke.*`
...
- host inbound nonce appears in `alisio message read` output
