        ...
        <div class="exec-approval-header">
          <div>
            <div class="exec-approval-title">
              ${t("alisio.settings.workspaceUrlConfirmation.title")}
            </div>
            <div class="exec-approval-sub">
              ${t("alisio.settings.workspaceUrlConfirmation.subtitle")}
            </div>
          </div>
        </div>
        <div class="exec-approval-command mono">${pendingGatewayUrl}</div>
        <div class="callout danger" style="margin-top: 12px;">
          ${t("alisio.settings.workspaceUrlConfirmation.warning")}
        </div>
        <div class="exec-approval-actions">
          <button class="btn primary" @click=${() => state.handleGatewayUrlConfirm()}>
            ${t("alisio.settings.workspaceUrlConfirmation.confirm")}
          </button>
