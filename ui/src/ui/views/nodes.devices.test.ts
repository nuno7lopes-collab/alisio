...
describe("nodes devices pending rendering", () => {
  ...
  it("does not repeat covered nodes as a separate runtime inventory section", () => {
    ...
    const runtimeTitles = Array.from(
      container.querySelectorAll(
        ".alisio-connections-runtime-stack .alisio-connections-subpanel__title",
      ),
  ...
  it("keeps requestable same-account runtime access on the paired computer row", () => {
    const onSharingRequest = vi.fn();
    const container = document.createElement("div");
    ...
    const text = container.textContent ?? "";
    expect(text).toContain("Windows Box");
    expect(text).toContain("Models: shared");
    expect(text).toContain("Execution: requestable");
    expect(text).toContain("Trusted");
    expect(text).toContain("Request control");
    ...
    expect(onSharingRequest).toHaveBeenCalledWith("windows-node", ["exec"]);
  });
