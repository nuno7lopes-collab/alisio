...
- `gateway`/`node` approvals are controlled by `~/.alisio/exec-approvals.json`.
- `node` requires a paired node (macOS app or headless node host).
...
- Alisio sets `ALISIO_SHELL=exec` in the spawned command environment (including PTY and sandbox execution) so shell/profile rules can detect exec-tool context.
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  Alisio prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
...
## Exec approvals (macOS app / node host)
...
`alisio security audit` warns when interpreter/runtime `safeBins` entries are missing explicit profiles, and `alisio doctor --fix` can scaffold missing custom `safeBinProfiles` entries.
`alisio security audit` and `alisio doctor` also warn when you explicitly add broad-behavior bins such as `jq` back into `safeBins`.
