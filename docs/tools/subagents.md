...
- For manual spawns, delivery is resilient:
  - Alisio tries direct `agent` delivery first with a stable idempotency key.
...
- Otherwise delivery depends on requester depth:
  ...
  - if a nested requester subagent session is gone, Alisio falls back to that session's requester when available
...
- Sub-agent context uses the minimal bootstrap allowlist: `AGENTS.md`, `TOOLS.md`, `SOUL.md`, `IDENTITY.md`, and `USER.md`. It still excludes `MEMORY.md`, `HEARTBEAT.md`, and `BOOTSTRAP.md`.
