...
Alisio supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, Alisio also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `alisio configure --section web`. It stores the key in
`~/.alisio/alisio.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.alisio/.env` (or your service environment). See [Env vars](/help/faq#env-vars-and-env-loading).
