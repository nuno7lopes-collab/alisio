...
- **Debugging blocked tools**: see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `alisio sandbox explain`.
...
Legacy `agent.*` configs are migrated by `alisio doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=alisio-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${ALISIO_STATE_DIR:-$HOME/.alisio}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
