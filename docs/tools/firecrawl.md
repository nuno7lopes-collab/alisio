...
Alisio can use **Firecrawl** in three ways:
...
- Choosing Firecrawl in onboarding or `alisio configure --section web` enables the bundled Firecrawl plugin automatically.
...
Alisio always uses `proxy: "auto"` plus `storeInCache: true` for Firecrawl requests.
