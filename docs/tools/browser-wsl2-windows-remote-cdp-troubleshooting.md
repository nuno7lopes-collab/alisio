...
read_when:
  - Running Alisio Gateway in WSL2 while Chrome lives on Windows
...
- Alisio Gateway runs inside WSL2
- Chrome runs on Windows
- browser control must cross the WSL2/Windows boundary

It also covers the layered failure pattern from [issue #39369](https://github.com/alisio/alisio/issues/39369): several independent problems can show up at once, which makes the wrong layer look broken first.
...
- Alisio and Chrome are on the same machine
...
If this fails on Windows, Alisio is not the problem yet.
...
Fix that before touching Alisio config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point Alisio at the address that is reachable from WSL2:
...
- test the same URL with `curl` before expecting Alisio to succeed
...
```bash
alisio browser open https://example.com --browser-profile remote
alisio browser tabs --browser-profile remote
```

Good result:

- the tab opens in Windows Chrome
- `alisio browser tabs` returns the target
...
- only then debug Alisio config or Control UI auth
