...
Exec approvals are the **macOS app / node host guardrail** for letting a sandboxed agent run
commands on a real host (`gateway` or `node`). Think of it like a safety interlock:
commands are allowed only when policy + allowlist + (optional) user approval all agree.
Exec approvals are **in addition** to tool policy and elevated gating (unless elevated is set to `full`, which skips approvals).
Effective policy is the **stricter** of `tools.exec.*` and approvals defaults; if an approvals field is omitted, the `tools.exec` value is used.

If the app UI is **not available**, any request that requires a prompt is
...
- **gateway host** → `alisio` process on the gateway machine
- **node host** → node runner (macOS app or headless node host)
...
- For shell scripts and direct interpreter/runtime file invocations, Alisio also tries to bind
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.alisio/exec-approvals.sock",
...
When `tools.exec.strictInlineEval=true`, Alisio treats inline code-eval forms as approval-only even if the interpreter binary itself is allowlisted.
...
On macOS app approvals, raw shell text containing shell control or expansion syntax
...
- allowlist entries live in host-local `~/.alisio/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `alisio approvals allowlist ...`).
- `alisio security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `alisio doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
If you explicitly opt `jq` into `safeBins`, Alisio still rejects the `env` builtin in safe-bin
...
- If Alisio cannot identify exactly one concrete local file for an interpreter/runtime command
...
After an approved async exec finishes, Alisio sends a followup `agent` turn to the same session.
...
topics, Alisio preserves the topic for the approval prompt and the post-approval follow-up.
...
When an async exec approval is denied, Alisio prevents the agent from reusing
