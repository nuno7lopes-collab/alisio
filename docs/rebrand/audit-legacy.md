...
- `docs/zh-CN/**` and `docs/.i18n/zh-CN.tm.jsonl`
  - **Why it stays:** generated by `docs-i18n`, which needs a provider API key (OpenAI or Anthropic) to run.
  - **Where referenced:** all Chinese output pages and their translation-memory cache.
  - **Removal plan:** set `OPENAI_API_KEY` (or `ANTHROPIC_API_KEY`) and rerun:
