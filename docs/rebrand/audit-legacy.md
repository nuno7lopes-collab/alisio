...
- Generated Chinese docs are updated via `docs-i18n` only.
- `docs-i18n` can use `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, or `~/.pi/agent/auth.json` created via `pi` and `/login` (for example `openai-codex` OAuth).
...
- zh-CN output + translation memory: pending `docs-i18n` regeneration once any supported translation credentials are available.

## Temporary Legacy Holdouts

- `docs/zh-CN/**` and `docs/.i18n/zh-CN.tm.jsonl`
  - **Why it stays:** generated by `docs-i18n`, which needs supported translation credentials to run.
  - **Where referenced:** all Chinese output pages and their translation-memory cache.
  - **Removal plan:** configure supported translation credentials (`OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, or `~/.pi/agent/auth.json` via `pi` + `/login`) and rerun:
    1. `pnpm docs:check-i18n-glossary`
    2. `scripts/docs-i18n` regeneration (doc mode)
    3. commit regenerated `docs/zh-CN/**` and `docs/.i18n/zh-CN.tm.jsonl`
