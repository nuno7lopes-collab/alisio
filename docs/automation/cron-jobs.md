...
- `sessionTarget: "isolated"` or `sessionTarget: "session:..."` creates a visible task that shows up in `alisio tasks` with delivery notifications.
...
- Jobs persist under `~/.alisio/cron/` so restarts don’t lose schedules.
...
- For upgrades, `alisio doctor --fix` can normalize legacy cron store fields before the scheduler touches them.
...
alisio cron list
alisio cron run <job-id>
alisio cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.alisio/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `alisio cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, Alisio applies a
...
Main-session cron jobs create [background task](/automation/tasks) records with `silent` notify policy (no notifications by default). They appear in `alisio tasks list` but do not generate delivery messages.
...
These detached runs create [background task](/automation/tasks) records visible in `alisio tasks` and subject to task audit and maintenance.
...
If `delivery` is omitted for isolated jobs, Alisio defaults to `announce`.
...
- CLI equivalents: `alisio cron add --light-context ...` and `alisio cron edit --light-context`.
...
- `schedule.at` accepts ISO 8601. Tool/API values without a timezone are treated as UTC; the CLI also accepts `alisio cron add|edit --at "<offset-less-iso>" --tz <iana>` for local wall-clock one-shots.
...
- Job store: `~/.alisio/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.alisio/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned by size and line count).
- Isolated cron run sessions in `sessions.json` are pruned by `cron.sessionRetention` (default `24h`; set `false` to disable).
- Override store path: `cron.store` in config.

## Retry policy

When a job fails, Alisio classifies errors as **transient** (retryable) or **permanent** (disable immediately).
...
- For removed run sessions no longer referenced by the session store, Alisio archives transcript files and purges old deleted archives on the same retention window.
...
- review growth periodically with `alisio cron runs` and adjust retention before logs become large
...
```bash
alisio system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`alisio system event`](/cli/system).

## Troubleshooting

### "Nothing runs"

- Check cron is enabled: `cron.enabled` and `ALISIO_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- Alisio applies exponential retry backoff for recurring jobs after consecutive errors:
