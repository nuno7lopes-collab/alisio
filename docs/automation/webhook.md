...
read_when:
  ...
  - Wiring external systems into Alisio
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your Alisio config directory (typically `~/.alisio/hooks/transforms`).
...
- `alisio webhooks gmail setup` writes `hooks.gmail` config for `alisio webhooks gmail run`.
...
```bash
curl -X POST http://127.0.0.1:40705/hooks/wake \
...
```bash
curl -X POST http://127.0.0.1:40705/hooks/agent \
  -H 'x-alisio-token: SECRET' \
  -H 'Content-Type: application/json' \
...
```bash
curl -X POST http://127.0.0.1:40705/hooks/gmail \
  -H 'Authorization: Bearer SECRET' \
  -H 'Content-Type: application/json' \
