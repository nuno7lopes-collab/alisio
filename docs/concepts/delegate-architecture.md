---
summary: "Delegate architecture: running Alisio as a named agent on behalf of an organization"
...
Goal: run Alisio as a **named delegate** — an agent with its own identity that acts "on behalf of" people in an organization. The agent never impersonates a human. It sends, reads, and schedules under its own account with explicit delegation permissions.
...
Alisio's default mode is a **personal assistant** — one human, one agent. Delegates extend this to organizations:
...
2. **Scope control**: the identity provider enforces what the delegate can access, independent of Alisio's own tool policy.
...
{
  id: "delegate",
  workspace: "~/.alisio/workspace-delegate",
  tools: {
...
- Cron run history: `~/.alisio/cron/runs/<jobId>.jsonl`
- Session transcripts: `~/.alisio/agents/delegate/sessions`
- Identity provider audit logs (Exchange, Google Workspace)

All delegate actions flow through Alisio's session store. For compliance, ensure these logs are retained and reviewed.
...
```bash
alisio agents add delegate
```

This creates:

- Workspace: `~/.alisio/workspace-delegate`
- State: `~/.alisio/agents/delegate/agent`
- Sessions: `~/.alisio/agents/delegate/sessions`
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.alisio/workspace" },
      {
        id: "delegate",
        workspace: "~/.alisio/workspace-delegate",
        tools: {
...
~/.alisio/agents/delegate/agent/auth-profiles.json
```
...
```json5
{
  agents: {
    list: [
      { id: "main", default: true, workspace: "~/.alisio/workspace" },
      {
        id: "org-assistant",
        name: "[Organization] Assistant",
        workspace: "~/.alisio/workspace-org",
        agentDir: "~/.alisio/agents/org-assistant/agent",
