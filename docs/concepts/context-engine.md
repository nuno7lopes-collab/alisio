...
read_when:
  - You want to understand how Alisio assembles model context
...
A **context engine** controls how Alisio builds model context for each run.
It decides which messages to include, how to summarize older history, and how
to manage context across subagent boundaries.

Alisio ships with a built-in `legacy` engine. Plugins can register
...
cat ~/.alisio/alisio.json | jq '.plugins.slots.contextEngine'
```

### Installing a context engine plugin

Context engine plugins are installed like any other Alisio plugin. Install
...
alisio plugins install -l ./my-context-engine
```
...
Every time Alisio runs a model prompt, the context engine participates at
...
Alisio currently calls one subagent lifecycle hook:
...
The `assemble` method can return a `systemPromptAddition` string. Alisio
...
The built-in `legacy` engine preserves Alisio's original behavior:
...
- `estimatedTokens` (required, `number`) — the engine's estimate of total
  tokens in the assembled context. Alisio uses this for compaction threshold
...
- `true` — the engine owns compaction behavior. Alisio disables Pi's built-in
...
`ownsCompaction: false` does **not** mean Alisio automatically falls back to
...
- **Delegating mode** — set `ownsCompaction: false` and have `compact()` call
  `delegateCompactionToRuntime(...)` from `alisio/plugin-sdk/core` to use
  Alisio's built-in compaction behavior.
...
Alisio resolves when it needs a context engine.

## Relationship to compaction and memory

- **Compaction** is one responsibility of the context engine. The legacy engine
  delegates to Alisio's built-in summarization. Plugin engines can implement
...
- Use `alisio doctor` to verify your engine is loading correctly.
...
- Engine errors are logged and surfaced in diagnostics. If a plugin engine
  fails to register or the selected engine id cannot be resolved, Alisio
  does not fall back automatically; runs fail until you fix the plugin or
  switch `plugins.slots.contextEngine` back to `"legacy"`.
- For development, use `alisio plugins install -l ./my-engine` to link a
