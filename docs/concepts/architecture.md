...
### Computers (macOS / headless)

In product-facing docs and UI we say **Computers**. The protocol and CLI still use
...
- Device nodes must include `role: "node"` plus caps/commands/permissions in `connect`.

## Pairing + local trust

- All WS clients (operators + device nodes) include a **device identity** on `connect`.
...
- Start: `alisio gateway` (foreground, logs to stdout).
