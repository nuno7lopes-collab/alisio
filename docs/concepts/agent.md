...
Recommended: use `alisio setup` to create `~/.alisio/alisio.json` if missing and initialize the workspace files.
...
Inside `agents.defaults.workspace`, Alisio expects these user-editable files:
...
Alisio loads skills from three locations (workspace wins on name conflict):

- Bundled (shipped with the install)
- Managed/local: `~/.alisio/skills`
...
delivery are Alisio-owned layers on top of that core.

## Sessions

Session transcripts are stored as JSONL at:

- `~/.alisio/agents/<agentId>/sessions/<SessionId>.jsonl`
