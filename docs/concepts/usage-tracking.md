...
- `/usage cost` in chats: local cost summary aggregated from Alisio session logs.
- CLI: `alisio status --usage` prints a full per-provider breakdown.
- CLI: `alisio channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
