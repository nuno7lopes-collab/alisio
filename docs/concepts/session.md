---
summary: "How Alisio manages conversation sessions"
read_when:
...
Alisio organizes conversations into **sessions**. Each message is routed to a
...
Verify your setup with `alisio security audit`.
...
Alisio automatically bounds session storage over time. By default, it runs
...
Preview with `alisio sessions cleanup --dry-run`.

## Inspecting sessions

- `alisio status` -- session store path and recent activity.
- `alisio sessions --json` -- all sessions (filter with `--active <minutes>`).
