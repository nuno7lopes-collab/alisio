...
read_when:
  - You see the ALISIO_PLUGIN_SDK_COMPAT_DEPRECATED warning
  - You see the ALISIO_EXTENSION_API_DEPRECATED warning
  - You are updating a plugin to the modern plugin architecture
  - You maintain an external Alisio plugin
---

# Plugin SDK Migration

Alisio has moved from a broad backwards-compatibility layer to a modern plugin
...
- **`alisio/plugin-sdk/compat`** — a single import that re-exported dozens of
  helpers. It was introduced to keep older hook-based plugins working while the
  new plugin architecture was being built.
- **`alisio/extension-api`** — a bridge that gave plugins direct access to
...
The modern plugin SDK fixes this: each import path (`alisio/plugin-sdk/\<subpath\>`)
...
<Steps>
  <Step title="Find deprecated imports">
    ...
    grep -r "alisio/extension-api" my-plugin/
    ```
  ...
  <Step title="Replace with focused imports">
    ...
    import { runEmbeddedPiAgent } from "alisio/extension-api";
...
```bash
ALISIO_SUPPRESS_PLUGIN_SDK_COMPAT_WARNING=1 alisio gateway run
ALISIO_SUPPRESS_EXTENSION_API_WARNING=1 alisio gateway run
```
