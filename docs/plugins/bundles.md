---
summary: "Install and use Codex, Claude, and Cursor bundles as Alisio plugins"
read_when:
  - You want to install a Codex, Claude, or Cursor-compatible bundle
  - You need to understand how Alisio maps bundle content into native features
...
Alisio can install plugins from three external ecosystems: **Codex**, **Claude**,
and **Cursor**. These are called **bundles** — content and metadata packs that
Alisio maps into native features like skills, hooks, and MCP tools.

<Info>
  Bundles are **not** the same as native Alisio plugins. Native plugins run
...
of requiring authors to rewrite them as native Alisio plugins, Alisio
...
## What Alisio maps from bundles

Not every bundle feature runs in Alisio today. Here is what works and what
...
| ------------- | ------------------------------------------------------------------------------------------- | -------------- |
| Skill content | Bundle skill roots load as normal Alisio skills                                             | All formats    |
| Commands      | `commands/` and `.cursor/commands/` treated as skill roots                                  | Claude, Cursor |
| Hook packs    | Alisio-style `HOOK.md` + `handler.ts` layouts                                               | Codex          |
...
- bundle skill roots load as normal Alisio skill roots
- Claude `commands` roots are treated as additional skill roots
- Cursor `.cursor/commands` roots are treated as additional skill roots

This means Claude markdown command files work through the normal Alisio skill
loader. Cursor command markdown works through the same path.

#### Hook packs

- bundle hook roots work **only** when they use the normal Alisio hook-pack
...
- Alisio merges bundle MCP config into the effective embedded Pi settings as
  `mcpServers`
- Alisio exposes supported bundle MCP tools during embedded Pi agent turns by
...
- `transport` may be set to `"streamable-http"` or `"sse"`; when omitted, Alisio uses `sse`
...
Alisio registers bundle MCP tools with provider-safe names in the form
...
- Claude `settings.json` is imported as default embedded Pi settings when the
  bundle is enabled
- Alisio sanitizes shell override keys before applying them
...
These are recognized and shown in diagnostics, but Alisio does not run them:
...
<AccordionGroup>
  <Accordion title="Codex bundles">
    ...
    Codex bundles fit Alisio best when they use skill roots and Alisio-style
...
Alisio checks for native plugin format first:

1. `alisio.plugin.json` or valid `package.json` with `alisio.extensions` — treated as **native plugin**
2. Bundle markers (`.codex-plugin/`, `.claude-plugin/`, or default Claude/Cursor layout) — treated as **bundle**

If a directory contains both, Alisio uses the native path. This prevents
...
- Alisio does **not** load arbitrary bundle runtime modules in-process
...
<AccordionGroup>
  <Accordion title="Bundle is detected but capabilities do not run">
    Run `alisio plugins inspect <id>`. If a capability is listed but marked as
  ...
  <Accordion title="Claude settings do not apply">
    Only embedded Pi settings from `settings.json` are supported. Alisio does
  ...
  <Accordion title="Claude hooks do not execute">
    ...
    Alisio hook-pack layout or ship a native plugin.
  </Accordion>
</AccordionGroup>
