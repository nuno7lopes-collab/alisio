...
read_when:
  - You want to place an outbound voice call from Alisio
...
Voice calls for Alisio via a plugin. Supports outbound notifications and
...
- Use `alisio voicecall ...` or the `voice_call` tool
...
alisio voicecall start --to "+15555550123"   # alias for call
...
alisio voicecall latency                     # summarize turn latency from logs
