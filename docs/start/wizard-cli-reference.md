...
- Model and auth setup (OpenAI Code subscription OAuth, Anthropic API key or setup token, plus MiniMax, GLM, Moonshot, AI Gateway, and custom server options)
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.alisio/alisio.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - CLI `--reset` defaults to `config+creds+sessions`; use `--reset-scope full` to also remove workspace.
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `alisio doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - Multi-account channels can also prompt for `accountId` and optional display names during onboarding.
    - DM security: default is pairing. First DM sends a code; approve via
      `alisio pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `alisio health`.
    - `alisio status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
<AccordionGroup>
  ...
  <Accordion title="Custom provider">
    Works with local or remote OpenAI-compatible and Anthropic-compatible endpoints.
...
- Command reference: [`alisio onboard`](/cli/onboard)
