---
summary: "Host Alisio on Oracle Cloud's Always Free ARM tier"
read_when:
  - Setting up Alisio on Oracle Cloud
  - Looking for free VPS hosting for Alisio
...
Run a persistent Alisio Gateway on Oracle Cloud's **Always Free** ARM tier (up to 4 OCPU, 24 GB RAM, 200 GB storage) at no cost.
...
<Steps>
  ...
  <Step title="Install Tailscale">
    ...
    From now on, connect via Tailscale: `ssh ubuntu@alisio`.
...
**Tailscale will not connect** -- Run `sudo tailscale up --ssh --hostname=alisio --reset` to re-authenticate.

**Gateway will not start** -- Run `alisio doctor --non-interactive` and check logs with `journalctl --user -u alisio-gateway -n 50`.
...
- [Updating](/install/updating) -- keep Alisio up to date
