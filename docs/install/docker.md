---
summary: "Optional Docker-based setup and onboarding for Alisio"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run Alisio on a host without local installs.
...
<Steps>
  <Step title="Build the image">
    ...
    ```bash
    export ALISIO_IMAGE="ghcr.io/alisio/alisio:latest"
    ./scripts/docker/setup.sh
    ```

    Pre-built images are published at the
    [GitHub Container Registry](https://github.com/alisio/alisio/pkgs/container/alisio).
  ...
  <Step title="Complete onboarding">
    ...
    `alisio-gateway` directly. `alisio-cli` is for commands you run after
...
<Note>
Run `docker compose` from the repo root. If you enabled `ALISIO_EXTRA_MOUNTS`
or `ALISIO_HOME_VOLUME`, the setup script writes `docker-compose.extra.yml`;
include it with `-f docker-compose.yml -f docker-compose.extra.yml`.
</Note>

<Note>
Because `alisio-cli` shares `alisio-gateway`'s network namespace, it is a
post-start tool. Before `docker compose up -d alisio-gateway`, run onboarding
and setup-time config writes through `alisio-gateway` with
...
| Variable                     | Purpose                                                          |
| ---------------------------- | ---------------------------------------------------------------- |
| `ALISIO_IMAGE`               | Use a remote image instead of building locally                   |
| `ALISIO_DOCKER_APT_PACKAGES` | Install extra apt packages during build (space-separated)        |
| `ALISIO_EXTENSIONS`          | Pre-install extension deps at build time (space-separated names) |
| `ALISIO_EXTRA_MOUNTS`        | Extra host bind mounts (comma-separated `source:target[:opts]`)  |
| `ALISIO_HOME_VOLUME`         | Persist `/home/node` in a named Docker volume                    |
| `ALISIO_SANDBOX`             | Opt in to sandbox bootstrap (`1`, `true`, `yes`, `on`)           |
| `ALISIO_DOCKER_SOCKET`       | Override Docker socket path                                      |
...
Docker Compose bind-mounts `ALISIO_CONFIG_DIR` to `/home/node/.alisio` and
`ALISIO_WORKSPACE_DIR` to `/home/node/.alisio/workspace`, so those paths
...
and rolling file logs under `/tmp/alisio/`.
...
<AccordionGroup>
  <Accordion title="Enable agent sandbox for Docker gateway">
    ...
    ```bash
    export ALISIO_SANDBOX=1
    export ALISIO_DOCKER_SOCKET=/run/user/1000/docker.sock
    ./scripts/docker/setup.sh
    ```
  ...
  <Accordion title="Automation / CI (non-interactive)">
    ...
    ```bash
    docker compose run -T --rm alisio-cli gateway probe
    docker compose run -T --rm alisio-cli devices list --json
    ```

  </Accordion>

  <Accordion title="Shared-network security note">
    `alisio-cli` uses `network_mode: "service:alisio-gateway"` so CLI
    ...
    `no-new-privileges` on `alisio-cli`.
  </Accordion>

  <Accordion title="Permissions and EACCES">
    The image runs as `node` (uid 1000). If you see permission errors on
    `/home/node/.alisio`, make sure your host bind mounts are owned by uid 1000:
  ...
  <Accordion title="Power-user container options">
    ...
    1. **Persist `/home/node`**: `export ALISIO_HOME_VOLUME="alisio_home"`
    2. **Bake system deps**: `export ALISIO_DOCKER_APT_PACKAGES="git curl jq"`
    ...
    4. **Persist browser downloads**: set
       ...
       `ALISIO_HOME_VOLUME` or `ALISIO_EXTRA_MOUNTS`.
...
<AccordionGroup>
  <Accordion title="Image missing or sandbox container not starting">
    ...
    [`scripts/sandbox-setup.sh`](https://github.com/alisio/alisio/blob/main/scripts/sandbox-setup.sh)
  ...
  <Accordion title="Custom tools not found in sandbox">
    Alisio runs commands with `sh -lc` (login shell), which sources
