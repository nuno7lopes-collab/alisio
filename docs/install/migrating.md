---
summary: "Move (migrate) an Alisio install from one machine to another"
read_when:
  - You are moving Alisio to a new laptop/server
  - You want to preserve sessions, auth, and channel logins (WhatsApp, etc.)
title: "Migration Guide"
---

# Migrating Alisio to a New Machine

This guide moves an Alisio gateway to a new machine without redoing onboarding.

## What Gets Migrated

When you copy the **state directory** (`~/.alisio/` by default) and your **workspace**, you preserve:

- **Config** -- `alisio.json` and all gateway settings
...
<Tip>
Run `alisio status` on the old machine to confirm your state directory path.
Custom profiles use `~/.alisio-<profile>/` or a path set via `ALISIO_STATE_DIR`.
</Tip>
...
<Steps>
  <Step title="Stop the gateway and back up">
    ...
    alisio gateway stop
    cd ~
    tar -czf alisio-state.tgz .alisio
    ```

    If you use multiple profiles (e.g. `~/.alisio-work`), archive each separately.

  </Step>

  <Step title="Install Alisio on the new machine">
    [Install](/install) the CLI (and Node if needed) on the new machine.
    It is fine if onboarding creates a fresh `~/.alisio/` -- you will overwrite it next.
  </Step>
...
<AccordionGroup>
  <Accordion title="Profile or state-dir mismatch">
    If the old gateway used `--profile` or `ALISIO_STATE_DIR` and the new one does not,
    channels will appear logged out and sessions will be empty.
    Launch the gateway with the **same** profile or state-dir you migrated, then rerun `alisio doctor`.
  </Accordion>

  <Accordion title="Copying only alisio.json">
...
- [ ] `alisio status` shows the gateway running
