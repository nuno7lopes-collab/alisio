---
summary: "Deploy Alisio Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run Alisio on a Kubernetes cluster
  - You want to test Alisio in a Kubernetes environment
title: "Kubernetes"
---

# Alisio on Kubernetes

A minimal starting point for running Alisio on Kubernetes — not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.

## Why not Helm?

Alisio is a single container with some config files. The interesting customization is in agent content (markdown files, skills, config overrides), not infrastructure templating. Kustomize handles overlays without the overhead of a Helm chart. If your deployment grows more complex, a Helm chart can be layered on top of these manifests.
...
```bash
kubectl get secret alisio-secrets -n alisio -o jsonpath='{.data.ALISIO_GATEWAY_TOKEN}' | base64 -d
```
...
Edit `alisio.json` in `scripts/k8s/manifests/configmap.yaml`. See [Gateway configuration](/gateway/configuration) for the full reference.
...
```bash
kubectl patch secret alisio-secrets -n alisio \
  -p '{"stringData":{"<PROVIDER>_API_KEY":"..."}}'
kubectl rollout restart deployment/alisio -n alisio
```

### Custom namespace

```bash
ALISIO_NAMESPACE=my-namespace ./scripts/k8s/deploy.sh
```
...
```yaml
image: ghcr.io/alisio/alisio:latest # or pin to a specific version from https://github.com/alisio/alisio/releases
```
