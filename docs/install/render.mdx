---
summary: "Deploy Alisio on Render with Infrastructure-as-Code"
read_when:
  - Deploying Alisio to Render
  - You want a declarative cloud deploy with Render Blueprints
title: "Render"
---

Deploy Alisio on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/alisio/alisio)
...
repository configures everything needed to run Alisio:
...
services:
  - type: web
    ...
    envVars:
      - key: ALISIO_GATEWAY_PORT
        value: "8080"
      - key: ALISIO_STATE_DIR
        value: /data/.alisio
      - key: ALISIO_WORKSPACE_DIR
        value: /data/workspace
      - key: ALISIO_GATEWAY_TOKEN
...
Connect using the `ALISIO_GATEWAY_TOKEN` value that was auto-generated during deploy (find it in **Dashboard → your service → Environment**).
...
If you use the original Alisio repository, Render will not auto-deploy your Alisio. To update it, run a manual Blueprint sync from the dashboard.
...
For Alisio, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
- Missing `ALISIO_GATEWAY_TOKEN` — verify it is set in **Dashboard → Environment**
- Port mismatch — ensure `ALISIO_GATEWAY_PORT=8080` is set so the gateway binds to the port Render expects
...
regularly export your config via `alisio backup create` in the Render shell.
