---
summary: "Run Alisio Gateway 24/7 on a GCP Compute Engine VM (Docker) with durable state"
read_when:
  - You want Alisio running 24/7 on GCP
  - You want a production-grade, always-on Gateway on your own VM
  - You want full control over persistence, binaries, and restart behavior
title: "GCP"
---

# Alisio on GCP Compute Engine (Docker, Production VPS Guide)

## Goal

Run a persistent Alisio Gateway on a GCP Compute Engine VM using Docker, with durable state, baked-in binaries, and safe restart behavior.

If you want "Alisio 24/7 for ~$5-12/mo", this is a reliable setup on Google Cloud.
...
- Start the Alisio Gateway in Docker
- Persist `~/.alisio` + `~/.alisio/workspace` on the host (survives restarts/rebuilds)
...
<Steps>
  ...
  <Step title="Clone the Alisio repository">
    ```bash
