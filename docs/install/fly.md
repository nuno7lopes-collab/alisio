---
title: Fly.io
summary: "Step-by-step Fly.io deployment for Alisio with persistent storage and HTTPS"
read_when:
  - Deploying Alisio on Fly.io
  - Setting up Fly volumes, secrets, and first-run config
---

# Fly.io Deployment

**Goal:** Alisio Gateway running on a [Fly.io](https://fly.io) machine with persistent storage, automatic HTTPS, and Discord/channel access.
...
<Steps>
  <Step title="Create the Fly app">
    ...
    fly apps create my-alisio

    # Create a persistent volume (1GB is usually enough)
    fly volumes create alisio_data --size 1 --region iad
    ```
  ...
  <Step title="Configure fly.toml">
    ...
    **Security note:** The default config exposes a public URL. For a hardened deployment with no public IP, see [Private Deployment](#private-deployment-hardened) or use `fly.private.toml`.
    ...
    [env]
      NODE_ENV = "production"
      ALISIO_PREFER_PNPM = "1"
      ALISIO_STATE_DIR = "/data"
    ...
    **Key settings:**
    ...
    | `internal_port = 3000`         | Must match `--port 3000` (or `ALISIO_GATEWAY_PORT`) for Fly health checks |
    | `memory = "2048mb"`            | 512MB is too small; 2GB recommended                                         |
    | `ALISIO_STATE_DIR = "/data"` | Persists state on the volume                                                |
  ...
  <Step title="Set secrets">
    ...
    fly secrets set ALISIO_GATEWAY_TOKEN=$(openssl rand -hex 32)
    ...
    **Notes:**
    ...
    - Non-loopback binds (`--bind lan`) require `ALISIO_GATEWAY_TOKEN` for security.
    - Treat these tokens like passwords.
    - **Prefer env vars over config file** for all API keys and tokens. This keeps secrets out of `alisio.json` where they could be accidentally exposed or logged.
  ...
  <Step title="Create config file">
    ...
    **Note:** With `ALISIO_STATE_DIR=/data`, the config path is `/data/alisio.json`.
  ...
  <Step title="Access the Gateway">
    ...
    Or visit `https://my-alisio.fly.dev/`

    Paste your gateway token (the one from `ALISIO_GATEWAY_TOKEN`) to authenticate.
...
**Fix:** Ensure `internal_port` matches the gateway port (set `--port 3000` or `ALISIO_GATEWAY_PORT=3000`).
...
If using `--allow-unconfigured`, the gateway creates a minimal config. Your custom config at `/data/alisio.json` should be read on restart.

Verify the config exists:

```bash
fly ssh console --command "cat /data/alisio.json"
```
...
echo '{"your":"config"}' | fly ssh console -C "tee /data/alisio.json"

# Or use sftp
fly sftp shell
> put /local/path/config.json /data/alisio.json
```

**Note:** `fly sftp` may fail if the file already exists. Delete first:

```bash
fly ssh console --command "rm /data/alisio.json"
```
...
**Fix:** Ensure `ALISIO_STATE_DIR=/data` is set in `fly.toml` and redeploy.
...
```bash
# List current IPs
fly ips list -a my-alisio

# Release public IPs
fly ips release <public-ipv4> -a my-alisio
fly ips release <public-ipv6> -a my-alisio
...
fly ips allocate-v6 --private -a my-alisio
```
...
fly proxy 3000:3000 -a my-alisio
