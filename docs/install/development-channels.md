...
Alisio ships three update channels:
...
- **`dev`**: ensures a git checkout (default `~/alisio`, override with
  `ALISIO_GIT_DIR`), switches to `main`, rebases on upstream, builds, and
...
alisio update --tag 2026.3.30-beta.1
...
alisio update --tag alisio@2026.3.30-beta.1
```

Notes:

- `--tag` applies to **package (npm) installs only**. Git installs ignore it.
- The tag is not persisted. Your next `alisio update` uses your configured
  channel as usual.
- Downgrade protection: if the target version is older than your current version,
  Alisio prompts for confirmation (skip with `--yes`).

## Dry run

Preview what `alisio update` would do without making changes:

```bash
alisio update --dry-run
alisio update --channel beta --dry-run
alisio update --tag 2026.3.30-beta.1 --dry-run
alisio update --dry-run --json
```
...
When you switch channels with `alisio update`, Alisio also syncs plugin
sources:
