---
summary: "CLI reference for `alisio config` (get/set/unset/file/schema/validate)"
read_when:
...
Config helpers for non-interactive edits in `alisio.json`: get/set/unset/file/schema/validate
values by path and print the active config file. Run without a subcommand to
open the configure wizard (same as `alisio configure`).

## Examples

```bash
alisio config file
alisio config schema
alisio config get browser.executablePath
...
alisio config set secrets.providers.vaultfile --provider-source file --provider-path /etc/alisio/secrets.json --provider-mode json
...
alisio config validate
alisio config validate --json
```

### `config schema`

Print the generated JSON schema for `alisio.json` to stdout as plain text.
...
```bash
alisio config schema > alisio.schema.json
```
...
alisio config get agents.list[0].id
```
...
alisio config set agents.list[1].tools.exec.node "node-id-or-name"
```
...
alisio config set channels.whatsapp.groups '["*"]' --strict-json
```

## `config set` modes

`alisio config set` supports four assignment styles:

1. Value mode: `alisio config set <path> <value>`
2. SecretRef builder mode:

```bash
alisio config set channels.discord.token \
...
```bash
alisio config set secrets.providers.vault \
  --provider-source exec \
  --provider-command /usr/local/bin/alisio-vault \
  --provider-arg read \
...
alisio config set secrets.providers.vaultfile \
  '{"source":"file","path":"/etc/alisio/secrets.json","mode":"json"}' \
  --strict-json
```
...
Use `--dry-run` to validate changes without writing `alisio.json`.
...
- `config file`: Print the active config file path (resolved from `ALISIO_CONFIG_PATH` or default location).
