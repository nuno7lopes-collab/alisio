---
summary: "CLI reference for `alisio message` (send + channel actions)"
read_when:
...
```
alisio message <subcommand> [flags]
```
...
- On cache miss, Alisio will attempt a live directory lookup when the provider supports it.
...
- `alisio message` resolves supported channel SecretRefs before running the selected action.
- Resolution is scoped to the active action target when possible:
  ...
  - when `--account` is omitted, Alisio does not force a `default` account SecretRef scope
...
```
alisio message send --channel discord \
...
```
alisio message poll --channel discord \
  --target channel:123 \
  --poll-question "Snack?" \
...
```
alisio message poll --channel telegram \
  --target @mychat \
  --poll-question "Lunch?" \
  --poll-option Pizza --poll-option Sushi \
...
```
alisio message poll --channel msteams \
...
```
alisio message react --channel slack \
...
```
alisio message react --channel signal \
...
```
alisio message send --channel telegram --target @mychat --message "Choose:" \
...
```bash
alisio message send --channel telegram --target @mychat \
