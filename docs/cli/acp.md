...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an Alisio Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`alisio acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
runtime. It focuses on session routing, prompt delivery, and basic streaming
updates.

If you want an external MCP client to talk directly to Alisio channel
conversations instead of hosting an ACP harness session, use
[`alisio mcp serve`](/cli/mcp) instead.
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the Alisio gateway or agent instead.                                                                                                                                       |
...
alisio acp --session-label "support inbox"

# Reset the session key before the first prompt
alisio acp --session agent:main:main --reset-session
```
...
it to drive an Alisio Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `alisio acp` over stdio.
...
```bash
alisio acp --session agent:main:main
alisio acp --session agent:design:main
alisio acp --session agent:qa:bug-123
```
...
If you want ACPX-backed sessions to see Alisio plugin tools, enable the
...
For a repo-local Alisio checkout, use the direct CLI entrypoint instead of the
...
pull contextual information from an Alisio agent without scraping a terminal.
...
In Zed, open the Agent panel and select “Alisio ACP” to start a thread.
