...
- Use the macOS app’s **Remote over SSH** mode (Settings → General → “Alisio runs”).
...
One gateway service owns state + channels. Paired computers (nodes) are peripherals.

Flow example (Telegram → paired computer):

- Telegram message arrives at the **Gateway**.
- Gateway runs the **agent** and decides whether to call a computer (`node`) tool.
- Gateway calls the paired computer over the Gateway WebSocket (`node.*` RPC).
- The paired computer returns the result; Gateway replies back out to Telegram.

Notes:

- **Paired computers (nodes) do not run the gateway service.** Only one gateway should run per host unless you intentionally run isolated profiles (see [Multiple gateways](/gateway/multiple-gateways)).
- macOS app “node mode” is just a paired-computer client over the Gateway WebSocket.
...
- URL override safety:
  ...
  - Env URL overrides (`ALISIO_GATEWAY_URL`) may use env credentials only (`ALISIO_GATEWAY_TOKEN` / `ALISIO_GATEWAY_PASSWORD`).
- Local mode defaults:
  - token: `ALISIO_GATEWAY_TOKEN` -> `gateway.auth.token` -> `gateway.remote.token` (remote fallback applies only when local auth token input is unset)
  - password: `ALISIO_GATEWAY_PASSWORD` -> `gateway.auth.password` -> `gateway.remote.password` (remote fallback applies only when local auth password input is unset)
- Remote mode defaults:
  - token: `gateway.remote.token` -> `ALISIO_GATEWAY_TOKEN` -> `gateway.auth.token`
  - password: `ALISIO_GATEWAY_PASSWORD` -> `gateway.remote.password` -> `gateway.auth.password`
- Node-host local-mode exception: `gateway.remote.token` / `gateway.remote.password` are ignored.
- Remote probe/status token checks are strict by default: they use `gateway.remote.token` only (no local token fallback) when targeting remote mode.
- Gateway env overrides use `ALISIO_GATEWAY_*` only.
...
- Plaintext `ws://` is loopback-only by default. For trusted private networks,
  set `ALISIO_ALLOW_INSECURE_PRIVATE_WS=1` on the client process as break-glass.
