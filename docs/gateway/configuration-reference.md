...
summary: "Complete reference for every Alisio config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.alisio/alisio.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — Alisio uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `alisio doctor` into `whatsapp/default`.
...
- In multi-account setups (2+ account ids), set an explicit default (`channels.telegram.defaultAccount` or `channels.telegram.accounts.default`) to avoid fallback routing; `alisio doctor` warns when this is missing or invalid.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["alisio-dm"] },
      guilds: {
        "123456789012345678": {
          slug: "friends-of-alisio",
          requireMention: false,
...
- `channels.discord.threadBindings` controls Discord thread-bound routing:
  ...
  - `spawnSubagentSessions`: opt-in switch for `sessions_spawn({ thread: true, runtime: "subagent" })` temporary task-thread creation/binding. The key name is legacy and is scheduled for replacement after 2026-06-30.
...
- Alisio additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
- `channels.discord.streaming` is the stream mode key.
...
- `channels.slack.streaming` is the stream mode key.
...
Mattermost ships as a plugin: `alisio plugins install @alisio/mattermost`.
...
- `commands.callbackUrl` must resolve to the Alisio gateway endpoint and be reachable from the Mattermost server.
...
Alisio spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
- If you add a non-default account via `alisio channels add` (or channel onboarding) while still on a single-account top-level channel config, Alisio moves account-scoped top-level single-account values into `channels.<channel>.accounts.default` first so the original account keeps working.
- Existing channel-only bindings (no `accountId`) keep matching the default account; account-scoped bindings remain optional.
- `alisio doctor --fix` also repairs mixed shapes by moving account-scoped top-level single-account values into `accounts.default` when named accounts exist but `default` is missing.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@alisio", "alisio"] } }],
  },
}
```
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@alisio"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `alisio.json`). For gateway `chat.send` clients, persistent `/config set|unset` writes also require `operator.admin`; read-only `/config show` stays available to normal write-scoped operator clients.
...
Optional repository root shown in the system prompt's Runtime line. If unset, Alisio auto-detects by walking upward from the workspace.

```json5
{
  agents: { defaults: { repoRoot: "~/Projects/alisio" } },
}
```
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, Alisio assumes `anthropic` (deprecated).
    ...
    defaults: {
      compaction: {
        ...
        memoryFlush: {
          ...
          prompt: "Write any lasting notes to memory/backlog/YYYY-MM-DD/compaction.md; reply with NO_REPLY if nothing to store.",
        },
      },
    },
  },
...
- `timeoutSeconds`: maximum seconds allowed for a single compaction operation before Alisio aborts it. Default: `900`.
- `identifierPolicy`: `strict` (default), `off`, or `custom`. `strict` prepends built-in opaque identifier retention guidance during compaction summarization.
- `identifierInstructions`: optional custom identifier-preservation text used when `identifierPolicy=custom`.
- `postCompactionSections`: optional AGENTS.md H2/H3 section names to re-inject after compaction. Defaults to `["Session Startup", "Red Lines"]`; set `[]` to disable reinjection.
    ...
    defaults: {
      sandbox: {
        ...
        docker: {
          ...
          apparmorProfile: "alisio-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
...
- `identityData` / `certificateData` / `knownHostsData`: inline contents or SecretRefs that Alisio materializes into temp files at runtime
...
- `none`: per-scope sandbox workspace under `~/.alisio/sandboxes`
...
In `remote` mode, host-local edits made outside Alisio are not synced into the sandbox automatically after the seed step.
...
**Sandboxed browser** (`sandbox.browser.enabled`): Chromium + CDP in a container. Does not require `browser.enabled` in `alisio.json`.

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `alisio-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
...
- Launch defaults are defined in `scripts/sandbox-browser-entrypoint.sh` and tuned for container hosts:
  ...
  - `--remote-debugging-port=<derived from ALISIO_BROWSER_CDP_PORT>`
  ...
  - `--disable-3d-apis`, `--disable-software-rasterizer`, and `--disable-gpu` are
    ...
    `ALISIO_BROWSER_DISABLE_GRAPHICS_FLAGS=0` if WebGL/3D usage requires it.
  - `ALISIO_BROWSER_DISABLE_EXTENSIONS=0` re-enables extensions if your workflow
    depends on them.
  - `--renderer-process-limit=2` can be changed with
    `ALISIO_BROWSER_RENDERER_PROCESS_LIMIT=<N>`; set `0` to use Chromium's
    ...
    list: [
      {
        ...
        agentDir: "~/.alisio/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@alisio"] },
        sandbox: { mode: "off" },
        runtime: {
          type: "acp",
          acp: {
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.alisio/workspace-home" },
      { id: "work", workspace: "~/.alisio/workspace-work" },
    ],
  },
  bindings: [
...
For `type: "acp"` entries, Alisio resolves by exact conversation identity (`match.channel` + account + `match.peer.id`) and does not use the route binding tier order above.
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.alisio/workspace-public",
...
- **`dmScope`**: how DMs are grouped.
  ...
  - `per-channel-peer`: isolate per channel + sender (default; recommended for multi-user inboxes).
...
- **`parentForkMaxTokens`**: max parent-session `totalTokens` allowed when creating a forked thread session (default `100000`).
  - If parent `totalTokens` is above this value, Alisio starts a fresh thread session instead of inheriting parent transcript history.
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.alisio/settings/tts.json",
      elevenlabs: {
...
- When `openai.baseUrl` points to a non-OpenAI endpoint, Alisio treats it as an OpenAI-compatible TTS server and relaxes model/voice validation.

---

## Talk

Defaults for Talk mode (macOS).
...
- `silenceTimeoutMs` controls how long Talk mode waits after user silence before it sends the transcript. Unset keeps the platform default pause window (`700 ms on macOS`).
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:alisio`     | All built-in tools (excludes provider plugins)                                           |
...
- Files are materialized into the child workspace at `.alisio/attachments/<uuid>/` with a `.manifest.json`.
...
Alisio uses the built-in model catalog. Add custom providers via `models.providers` in config or `~/.alisio/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `ALISIO_AGENT_DIR` (or `PI_CODING_AGENT_DIR`, a legacy environment variable alias).
...
- `models.providers.*.models.*.compat.supportsDeveloperRole`: optional compatibility hint. For `api: "openai-completions"` with a non-empty non-native `baseUrl` (host not `api.openai.com`), Alisio forces this to `false` at runtime. Empty/omitted `baseUrl` keeps default OpenAI behavior.
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Use `opencode/...` refs for the Zen catalog or `opencode-go/...` refs for the Go catalog. Shortcut: `alisio onboard --auth-choice opencode-zen` or `alisio onboard --auth-choice opencode-go`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `alisio onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `alisio onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `alisio onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `alisio onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `alisio onboard --auth-choice minimax-api`.
...
- Loaded from `~/.alisio/extensions`, `<workspace>/.alisio/extensions`, plus `plugins.load.paths`.
- Discovery accepts native Alisio plugins plus compatible Codex bundles and Claude bundles, including manifestless Claude default-layout bundles.
...
- `plugins.entries.<id>.config`: plugin-defined config object (validated by native Alisio plugin schema when available).
- Enabled Claude bundle plugins can also contribute embedded Pi defaults from `settings.json`; Alisio applies those as sanitized agent settings, not as raw Alisio config patches.
- `plugins.slots.memory`: pick the active memory plugin id, or `"none"` to disable memory plugins.
- `plugins.slots.contextEngine`: pick the active context engine plugin id; defaults to `"legacy"` unless you install and select another engine.
- `plugins.installs`: CLI-managed install metadata used by `alisio plugins update`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or ALISIO_GATEWAY_PASSWORD
...
- `ALISIO_ALLOW_INSECURE_PRIVATE_WS=1`: client-side break-glass override that allows plaintext `ws://` to trusted private-network IPs; default remains loopback-only for plaintext.
...
```json5
{
  gateway: {
    tls: {
      enabled: false,
      autoGenerate: false,
      certPath: "/etc/alisio/tls/server.crt",
      keyPath: "/etc/alisio/tls/server.key",
      caPath: "/etc/alisio/tls/ca-bundle.crt",
    },
  },
}
```
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.alisio/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-alisio-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `ALISIO_SKIP_GMAIL_WATCHER=1` to disable.
...
{
  canvasHost: {
    root: "~/.alisio/workspace/canvas",
    liveReload: true,
    // enabled: false, // or ALISIO_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__alisio__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__alisio__/a2ui/`
...
- Also serves A2UI at `/__alisio__/a2ui/`.
...
- Hostname defaults to `alisio`. Override with `ALISIO_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.alisio/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `alisio dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.alisio/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${ALISIO_GATEWAY_TOKEN}" },
  },
}
```
...
- `secrets apply` targets supported `alisio.json` credential paths.
...
```json5
{
  secrets: {
    providers: {
      ...
      filemain: {
        source: "file",
        path: "~/.alisio/secrets.json",
        mode: "json",
        timeoutMs: 5000,
      },
      vault: {
        source: "exec",
        command: "/usr/local/bin/alisio-vault-resolver",
...
- Static runtime credentials come from in-memory resolved snapshots built from auth profiles and env-backed provider auth.
...
- Default log file: `/tmp/alisio/alisio-YYYY-MM-DD.log`.
...
- `cli.banner.taglineMode` controls banner tagline style:
  ...
  - `"default"`: fixed neutral tagline (`All your chats, one Alisio.`).
  - `"off"`: no tagline text (banner title/version still shown).
- To hide the entire banner (not just taglines), set env `ALISIO_HIDE_BANNER=1`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `alisio doctor --fix` can strip unknown keys).
...
- Paths: resolved relative to the including file, but must stay inside the top-level config directory (`dirname` of `alisio.json`). Absolute/`../` forms are allowed only when they still resolve inside that boundary.
