...
Alisio can auto-configure Tailscale **Serve** (tailnet) or **Funnel** (public) for the
...
- `funnel`: Public HTTPS via `tailscale funnel`. Alisio requires a shared password.
...
- `token` (default when `ALISIO_GATEWAY_TOKEN` is set)
- `password` (shared secret via `ALISIO_GATEWAY_PASSWORD` or config)

When `tailscale.mode = "serve"` and `gateway.auth.allowTailscale` is `true`,
Control UI/WebSocket auth can use Tailscale identity headers
(`tailscale-user-login`) without supplying a token/password. Alisio verifies
the identity by resolving the `x-forwarded-for` address via the local Tailscale
daemon (`tailscale whois`) and matching it to the header before accepting it.
Alisio only treats a request as Serve when it arrives from loopback with
...
- Control UI: `http://<tailscale-ip>:40705/`
- WebSocket: `ws://<tailscale-ip>:40705`

Note: loopback (`http://127.0.0.1:40705`) will **not** work in this mode.
...
Prefer `ALISIO_GATEWAY_PASSWORD` over committing a password to disk.
...
- Set `gateway.tailscale.resetOnExit` if you want Alisio to undo `tailscale serve`
...
Avoid Funnel for browser control; treat device pairing like operator access.
