...
`alisio doctor` is the repair + migration tool for Alisio. It fixes stale
...
- Extra workspace dir detection (`~/alisio`).
...
you to run `alisio doctor`.

Doctor will:

- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.alisio/alisio.json` with the updated schema.
...
- Sessions store + transcripts:
  - from `~/.alisio/sessions/` to `~/.alisio/agents/<agentId>/sessions/`
- Agent dir:
...
migrated via `alisio doctor`.

### 3b) Legacy cron store migrations

Doctor also checks the cron job store (`~/.alisio/cron/jobs.json` by default,
...
- **Multiple state dirs**: warns when multiple `~/.alisio` folders exist across
  home directories or when `ALISIO_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.alisio/alisio.json` is
...
offers to remove them and install the Alisio service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named Alisio gateway services are considered first-class and are not
flagged as "extra."
...
- `alisio doctor --generate-gateway-token` forces generation only when no token SecretRef is configured.
...
- `alisio doctor --fix` now uses the same read-only SecretRef summary model as status-family commands for targeted config repairs.
...
- `alisio doctor` prompts before rewriting supervisor config.
- `alisio doctor --yes` accepts the default repair prompts.
- `alisio doctor --repair` applies recommended fixes without prompts.
- `alisio doctor --repair --force` overwrites custom supervisor configs.
...
- You can always force a full rewrite via `alisio gateway install --force`.
