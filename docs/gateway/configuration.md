...
read_when:
  - Setting up Alisio for the first time
...
Alisio reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.alisio/alisio.json`.

If the file is missing, Alisio uses safe defaults. Common reasons to add a config:
...
<Tip>
**New to configuration?** Start with `alisio onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    alisio onboard       # full onboarding flow
    alisio configure     # config wizard
    ```
  </Tab>
  <Tab title="CLI (one-liners)">
    ```bash
    alisio config get agents.defaults.workspace
    alisio config set agents.defaults.heartbeat.every "2h"
    alisio config unset plugins.entries.brave.config.webSearch.apiKey
    ```
  </Tab>
  ...
  <Tab title="Direct edit">
    Edit `~/.alisio/alisio.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<Warning>
Alisio only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

When validation fails:

- The Gateway does not boot
- Only diagnostic commands work (`alisio doctor`, `alisio logs`, `alisio health`, `alisio status`)
- Run `alisio doctor` to see exact issues
- Run `alisio doctor --fix` (or `--yes`) to apply repairs
...
<AccordionGroup>
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    Relay-backed push is configured in `alisio.json`.
    ...
    ```bash
    alisio config set gateway.push.apns.relay.baseUrl https://relay.example.com
    ```
    ...
    - `ALISIO_APNS_RELAY_BASE_URL` and `ALISIO_APNS_RELAY_TIMEOUT_MS` still work as temporary env overrides.
    - `ALISIO_APNS_RELAY_ALLOW_HTTP=true` remains a loopback-only development escape hatch; do not persist HTTP relay URLs in config.
...
The Gateway watches `~/.alisio/alisio.json` and applies changes automatically — no manual restart needed for most settings.
...
<AccordionGroup>
  <Accordion title="config.apply (full replace)">
    ...
    <Warning>
    `config.apply` replaces the **entire config**. Use `config.patch` for partial updates, or `alisio config set` for single keys.
    </Warning>
    ...
    ```bash
    alisio gateway call config.get --params '{}'  # capture payload.hash
    alisio gateway call config.apply --params '{
      "raw": "{ agents: { defaults: { workspace: \"~/.alisio/workspace\" } } }",
      "baseHash": "<hash>",
  ...
  <Accordion title="config.patch (partial update)">
    ...
    ```bash
    alisio gateway call config.patch --params '{
...
Alisio reads env vars from the parent process plus:

- `.env` from the current working directory (if present)
- `~/.alisio/.env` (global fallback)
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, Alisio runs your login shell and imports only the missing keys:
...
Env var equivalent: `ALISIO_LOAD_SHELL_ENV=1`
</Accordion>
...
```json5
{
  gateway: { auth: { token: "${ALISIO_GATEWAY_TOKEN}" } },
