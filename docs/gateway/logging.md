...
- Default rolling log file is under `/tmp/alisio/` (one file per day): `alisio-YYYY-MM-DD.log`
  - Date uses the gateway host's local timezone.
- The log file path and level can be configured via `~/.alisio/alisio.json`:
  - `logging.file`
  - `logging.level`
...
`alisio gateway` supports a per-gateway style switch:
