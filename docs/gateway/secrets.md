...
Alisio supports additive SecretRefs so supported credentials do not need to be stored as plaintext in configuration.
...
- `gateway.auth.token` SecretRef is inactive for startup auth resolution when `ALISIO_GATEWAY_TOKEN` is set, because env token input wins for that runtime.
...
When onboarding runs in interactive mode and you choose SecretRef storage, Alisio runs preflight validation before saving:
...
- Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). Alisio validates the resolved target path.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/Alisio QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
  ...
  secrets: {
    providers: {
      vault_openai: {
        ...
        allowSymlinkCommand: true, // required for Homebrew symlinked binaries
        trustedDirs: ["/opt/homebrew"],
        args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/alisio"],
...
- Alisio resolves these refs during sandbox activation, not lazily during each SSH call.
...
- `REF_SHADOWED` (audit finding when `auth-profiles.json` credentials take precedence over `alisio.json` refs)
...
When reload-time activation fails after a healthy state, Alisio enters degraded secrets state.
...
- Strict command paths (for example `alisio memory` remote-memory paths and `alisio qr --remote`) read from the active snapshot and fail fast when a required SecretRef is unavailable.
- Read-only command paths (for example `alisio status`, `alisio status --all`, `alisio channels status`, `alisio channels resolve`, `alisio security audit`, and read-only doctor/config repair flows) also prefer the active snapshot, but degrade instead of aborting when a targeted SecretRef is unavailable in that command path.
...
- Snapshot refresh after backend secret rotation is handled by `alisio secrets reload`.
...
- plaintext values at rest (`alisio.json`, `auth-profiles.json`, `.env`, and generated `agents/*/agent/models.json`)
...
- Use `alisio secrets audit --allow-exec` to execute exec providers during audit.
...
- lets you select supported secret-bearing fields in `alisio.json` plus `auth-profiles.json` for one agent scope
...
- `alisio secrets configure --agent <id>`
...
```bash
alisio secrets apply --from /tmp/alisio-secrets-plan.json
alisio secrets apply --from /tmp/alisio-secrets-plan.json --allow-exec
alisio secrets apply --from /tmp/alisio-secrets-plan.json --dry-run
alisio secrets apply --from /tmp/alisio-secrets-plan.json --dry-run --allow-exec
```
...
Alisio intentionally does not write rollback backups containing historical plaintext secret values.
