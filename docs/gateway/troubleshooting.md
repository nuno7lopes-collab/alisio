...
- `alisio gateway status` shows `Runtime: running` and `RPC probe: ok`.
- `alisio doctor` reports no blocking config/service issues.
- `alisio channels status --probe` shows connected/ready channels.
...
alisio config get agents.defaults.models
```
...
- `Gateway start blocked: set gateway.mode=local` → local gateway mode is not enabled. Fix: set `gateway.mode="local"` in your config (or run `alisio configure`). If you are running Alisio via Podman, the default config path is `~/.alisio/alisio.json`.
...
alisio browser start --browser-profile alisio
