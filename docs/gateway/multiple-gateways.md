---
summary: "Run multiple Alisio Gateways on one host (isolation, ports, and profiles)"
read_when:
...
- `ALISIO_CONFIG_PATH` — per-instance config file
- `ALISIO_STATE_DIR` — per-instance sessions, creds, caches
...
Profiles auto-scope `ALISIO_STATE_DIR` + `ALISIO_CONFIG_PATH` and suffix service names.
...
Base port = `gateway.port` (or `ALISIO_GATEWAY_PORT` / `--port`).
...
```bash
alisio --profile main status
alisio --profile rescue status
alisio --profile rescue browser status
```
