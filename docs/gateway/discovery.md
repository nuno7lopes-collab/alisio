...
- **Gateway WS (control plane)**: the WebSocket endpoint on `127.0.0.1:40705` by default; can be bound to LAN/tailnet via `gateway.bind`.
- **Direct WS transport**: a LAN/tailnet-facing Gateway WS endpoint (no SSH).
- **SSH transport (fallback)**: remote control by forwarding `127.0.0.1:40705` over SSH.
- **Legacy TCP bridge (removed)**: older node transport that is no longer shipped or advertised for discovery.
...
- Service types:
  - `_alisio-gw._tcp` (gateway transport beacon)
- TXT keys (non-secret):
  - `role=gateway`
  - `lanHost=<hostname>.local`
  - `sshPort=22` (or whatever is advertised)
  - `gatewayPort=40705` (Gateway WS + HTTP)
  - `gatewayTls=1` (only when TLS is enabled)
  - `gatewayTlsSha256=<sha256>` (only when TLS is enabled and fingerprint is available)
  - `canvasPort=<port>` (canvas host port; currently the same as `gatewayPort` when the canvas host is enabled)
  - `cliPath=<path>` (optional; absolute path to a runnable `alisio` entrypoint or binary)
...
- `ALISIO_DISABLE_BONJOUR=1` disables advertising.
- `gateway.bind` in `~/.alisio/alisio.json` controls the Gateway bind mode.
- `ALISIO_SSH_PORT` overrides the SSH port advertised in TXT (defaults to 22).
- `ALISIO_TAILNET_DNS` publishes a `tailnetDns` hint (MagicDNS).
- `ALISIO_CLI_PATH` overrides the advertised CLI path.
