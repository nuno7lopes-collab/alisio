...
- During heartbeat runs, Alisio treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `alisio sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Alisio skips the heartbeat run entirely (no model call).
...
- Set a cheaper `model` (e.g. `local/my-small-model`).
