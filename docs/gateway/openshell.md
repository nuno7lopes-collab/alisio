---
title: OpenShell
summary: "Use OpenShell as a managed sandbox backend for Alisio agents"
read_when:
...
OpenShell is a managed sandbox backend for Alisio. Instead of running Docker
containers locally, Alisio delegates sandbox lifecycle to the `openshell` CLI,
...
- Alisio Gateway running on the host
...
2. Restart the Gateway. On the next agent turn, Alisio creates an OpenShell
...
- You edit files locally outside Alisio and want those changes visible in the
...
- When the sandbox is first created, Alisio seeds the remote workspace from
...
- Alisio does **not** sync remote changes back into the local workspace.
...
Important: if you edit files on the host outside Alisio after the initial seed,
the remote sandbox does **not** see those changes. Use
`alisio sandbox recreate` to re-seed.
...
| `from`                    | `string`                 | `"alisio"`    | Sandbox source for first-time create                  |
...
1. Alisio calls `openshell sandbox create` (with `--from`, `--gateway`,
   `--policy`, `--providers`, `--gpu` flags as configured).
2. Alisio calls `openshell sandbox ssh-config <name>` to get SSH connection
...
- [Sandbox CLI](/cli/sandbox) -- `alisio sandbox` commands
