...
Alisio features that can generate provider usage or paid API calls.
...
- `alisio status --usage` and `alisio channels list` show provider **usage windows**
...
Alisio can pick up credentials from:
...
`alisio models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
