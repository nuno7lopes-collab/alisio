...
When `provider` is not set, Alisio selects the first available:
...
When sqlite-vec is unavailable, Alisio falls back to in-process cosine
...
| Key                   | Type     | Default                             | Description                                 |
| --------------------- | -------- | ----------------------------------- | ------------------------------------------- |
| `store.path`          | `string` | `~/.alisio/memory/{agentId}.sqlite` | Index location (supports `{agentId}` token) |
