...
- With Anthropic API-key auth profiles, Alisio seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, Alisio injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
Alisio exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.alisio/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$ALISIO_STATE_DIR/logs/cache-trace.jsonl`
...
- `ALISIO_CACHE_TRACE=1` enables cache tracing.
- `ALISIO_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `ALISIO_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `ALISIO_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `ALISIO_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
