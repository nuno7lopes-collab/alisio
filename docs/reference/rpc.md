...
Alisio integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Alisio owns lifecycle when `channels.signal.autoStart=true`.
...
- Alisio spawns `imsg rpc` as a child process (legacy iMessage integration).
