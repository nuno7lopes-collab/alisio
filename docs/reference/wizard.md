...
<Steps>
  <Step title="Existing config detection">
    - If `~/.alisio/alisio.json` exists, choose **Keep / Modify / Reset**.
    ...
    - If the config is invalid or contains legacy keys, the wizard stops and asks
      you to run `alisio doctor` before continuing.
  ...
  <Step title="Model/Auth">
    ...
    - **Anthropic Claude CLI**: during macOS CLI onboarding it checks Keychain item "Claude Code-credentials" (choose "Always Allow" so launchd starts don't block); on Linux/Windows it reuses `~/.claude/.credentials.json` if present and switches model selection to `claude-cli/...`.
  ...
  <Step title="Workspace">
    - Default `~/.alisio/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `alisio pairing approve <channel> <code>` or use allowlists.
  </Step>
  <Step title="Web search">
    ...
    - Configure later: `alisio configure --section web`.
  </Step>
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `alisio health`.
    - Tip: `alisio status --deep` adds gateway health probes to status output (requires a reachable gateway).
  </Step>
  ...
  <Step title="Finish">
    - Summary + next steps, including the macOS app and remote computer options for extra features.
  </Step>
</Steps>
...
- Stores it under `~/.alisio/tools/signal-cli/<version>/`.
...
`alisio agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.alisio/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.alisio/agents/<agentId>/sessions/`.
