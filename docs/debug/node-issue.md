...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Alisio
...
Running Alisio via Node with `tsx` fails at startup with:

```
[alisio] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `alisio status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch alisio.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node alisio.mjs status` works on Node 25.
