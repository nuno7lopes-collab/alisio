...
read_when:
  - Setting up Zalo Personal for Alisio
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside Alisio.
...
- Install via CLI: `alisio plugins install @alisio/zalouser`
- Or from a source checkout: `alisio plugins install ./path/to/local/zalouser-plugin`
...
2. Login (QR, on the Gateway machine):
   - `alisio channels login --channel zalouser`
...
```bash
alisio directory self --channel zalouser
alisio directory peers list --channel zalouser --query "name"
alisio directory groups list --channel zalouser --query "work"
```
...
- `alisio pairing list zalouser`
- `alisio pairing approve zalouser <code>`
...
- On startup, Alisio resolves group/user names in allowlists to IDs and logs the mapping.
...
- When a group message is skipped because mention is required, Alisio stores it as pending group history and includes it on the next processed group message.
...
- Alisio sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, Alisio sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `alisio channels logout --channel zalouser && alisio channels login --channel zalouser`
...
- The channel now runs fully in Alisio without external CLI binaries.
