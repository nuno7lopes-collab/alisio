...
Tlon is a decentralized messenger built on Urbit. Alisio connects to your Urbit ship and can
...
```bash
alisio plugins install @alisio/tlon
```

Local checkout (when running from a git repo):

```bash
alisio plugins install ./path/to/local/tlon-plugin
```
...
By default, Alisio blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `alisio message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Alisio replies in-thread.
