...
```bash
alisio plugins install @alisio/msteams
```

Local checkout (when running from a git repo):

```bash
alisio plugins install ./path/to/local/msteams-plugin
```
...
3. Configure Alisio with those credentials.
...
- Talk to Alisio via Teams DMs, group chats, or channels.
...
- On startup, Alisio resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.alisio/alisio.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring Alisio, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                  |
   | ------------------ | ------------------------------------------------------ |
   | **Bot handle**     | Your bot name, e.g., `alisio-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `alisio plugins install @alisio/msteams`
   - From a local checkout: `alisio plugins install ./path/to/local/msteams-plugin`
...
```json5
{
  ...
  description: { short: "Alisio in Teams", full: "Alisio in Teams" },
...
Alisio handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, Alisio only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/AlisioShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

Alisio sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `alisio message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.alisio/msteams-polls.json`.
...
```bash
alisio message send --channel msteams \
...
alisio message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
alisio message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
alisio message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
alisio message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
