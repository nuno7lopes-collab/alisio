...
read_when:
  - You want Alisio to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Alisio to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- Onboarding (`alisio onboard`) and `alisio channels add` list optional channel plugins.
...
```bash
alisio plugins install @alisio/nostr
```

Use a local checkout (dev workflows):

```bash
alisio plugins install --link <path-to-local-nostr-plugin>
```
...
alisio channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY" --relay-urls "wss://relay.damus.io,wss://relay.primal.net"
```
  ...
  channels: {
    nostr: {
      ...
      profile: {
        ...
        nip05: "alisio@example.com",
        lud16: "alisio@example.com",
      },
    },
  },
}
