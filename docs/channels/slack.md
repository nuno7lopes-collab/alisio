...
```
  ...
  <Tab title="HTTP Events API mode">
    <Steps>
      ...
      <Step title="Configure Alisio HTTP mode">
...
<Tabs>
  <Tab title="DM policy">
    ...
    Pairing in DMs uses `alisio pairing approve slack <code>`.
...
- The interactive callback values are Alisio-generated opaque tokens, not raw agent-authored values.
- If generated interactive blocks would exceed Slack Block Kit limits, Alisio falls back to the original text reply instead of sending an invalid blocks payload.
...
- With default `session.dmScope=per-channel-peer`, Slack DMs keep isolated direct session keys.
...
`typingReaction` adds a temporary reaction to the inbound Slack message while Alisio is processing a reply, then removes it when the run finishes. This is a useful fallback when Slack native assistant typing is unavailable, especially in DMs.
...
```json
{
  "display_information": {
    "name": "Alisio",
    "description": "Slack connector for Alisio"
  },
  "features": {
    "bot_user": {
      "display_name": "Alisio",
    ...
    "slash_commands": [
      {
        "command": "/alisio",
        "description": "Send a message to Alisio",
        "should_escape": false
      }
    ]
  },
...
Alisio supports Slack native text streaming via the Agents and AI Apps API.
...
- If streaming fails mid-reply, Alisio falls back to normal delivery for remaining payloads.
