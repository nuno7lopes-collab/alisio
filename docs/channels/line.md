...
read_when:
  - You want to connect Alisio to LINE
...
LINE connects to Alisio via the LINE Messaging API. The plugin runs as a webhook
...
```bash
alisio plugins install @alisio/line
```

Local checkout (when running from a git repo):

```bash
alisio plugins install ./path/to/local/line-plugin
```
...
- LINE signature verification is body-dependent (HMAC over the raw body), so Alisio applies strict pre-auth body limits and timeout before verification.
- Alisio processes webhook events from the verified raw request bytes. Upstream middleware-transformed `req.body` values are ignored for signature-integrity safety.
...
```bash
alisio pairing list line
alisio pairing approve line <CODE>
```
