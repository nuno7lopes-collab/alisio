...
Alisio routes replies **back to the channel where a message came from**. The
...
Alisio infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, Alisio still records inbound session metadata, but it
...
Broadcast groups let you run **multiple agents** for the same peer **when Alisio would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
...
{
  agents: {
    list: [{ id: "support", name: "Support", workspace: "~/.alisio/workspace-support" }],
  },
  bindings: [
...
Session stores live under the state directory (default `~/.alisio`):

- `~/.alisio/agents/<agentId>/sessions/sessions.json`
