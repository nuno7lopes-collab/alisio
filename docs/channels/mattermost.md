---
summary: "Mattermost bot setup and Alisio config"
read_when:
...
```bash
alisio plugins install @alisio/mattermost
```

Local checkout (when running from a git repo):

```bash
alisio plugins install ./path/to/local/mattermost-plugin
```
...
4. Configure Alisio and start the gateway.
...
Native slash commands are opt-in. When enabled, Alisio registers `oc_*` slash commands via
...
- If `callbackUrl` is omitted, Alisio derives one from gateway host/port + `callbackPath`.
...
- Reachability requirement: the callback endpoint must be reachable from the Mattermost server.
  - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as Alisio.
  - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to Alisio.
  - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from Alisio, not `404`.
...
- Approve via:
  - `alisio pairing list mattermost`
  - `alisio pairing approve mattermost <CODE>`
...
Use these target formats with `alisio message send` or cron/webhooks:
...
Alisio resolves them **user-first**:

- If the ID exists as a user (`GET /api/v4/users/<id>` succeeds), Alisio sends a **DM** by resolving the direct channel via `/api/v4/channels/direct`.
...
When Alisio sends to a Mattermost DM target and needs to resolve the direct channel first, it
...
- If `interactions.callbackBaseUrl` is omitted, Alisio derives the callback URL from
...
- Reachability rule: the button callback URL must be reachable from the Mattermost server.
  `localhost` only works when Mattermost and Alisio run on the same host/network namespace.
...
1. Derive the secret from the bot token:
   `HMAC-SHA256(key="alisio-mattermost-interactions", data=botToken)`
...
secret = hmac.new(
    b"alisio-mattermost-interactions",
...
`alisio message send` and cron/webhook deliveries.
