...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Alisio to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.

---

## Bundled plugin

Feishu ships bundled with current Alisio releases, so no separate plugin install
is required.
...
```bash
alisio plugins install @alisio/feishu
```
...
If you just installed Alisio, run onboarding:
...
2. Configuring app credentials in Alisio
...
1. You already ran `alisio channels add` for Feishu
2. The gateway is running (`alisio gateway status`)
...
```bash
alisio pairing approve feishu <CODE>
```
...
2. Run `alisio logs --follow` and look for `chat_id`
...
2. Run `alisio logs --follow` and look for `open_id`
...
| Command                  | Description                   |
| ------------------------ | ----------------------------- |
| `alisio gateway status`  | Show gateway status           |
| `alisio gateway install` | Install/start gateway service |
| `alisio gateway stop`    | Stop gateway service          |
| `alisio gateway restart` | Restart gateway service       |
| `alisio logs --follow`   | Tail gateway logs             |
...
4. Check logs: `alisio logs --follow`
...
5. Ensure the gateway is running: `alisio gateway status`
6. Check logs: `alisio logs --follow`
