...
- Onboarding (`alisio onboard`) and `alisio channels add --channel whatsapp`
  prompt to install the WhatsApp plugin the first time you select it.
- `alisio channels login --channel whatsapp` also offers the install flow when
  the plugin is not present yet.
- Dev channel + git checkout: defaults to the local plugin path.
- Stable/Beta: defaults to the npm package `@alisio/whatsapp`.

Manual install stays available:

```bash
alisio plugins install @alisio/whatsapp
```
...
<AccordionGroup>
  <Accordion title="Dedicated number (recommended)">
    ...
    - separate WhatsApp identity for Alisio
  ...
  <Accordion title="WhatsApp Web-only channel scope">
    The messaging platform channel is WhatsApp Web-based (`Baileys`) in current Alisio channel architecture.
...
- if `messages.responsePrefix` is unset, self-chat replies default to `[{identity.name}]` or `[alisio]`
...
<AccordionGroup>
  ...
  <Accordion title="Credential paths and legacy compatibility">
    - current auth path: `~/.alisio/credentials/whatsapp/<accountId>/creds.json`
    - backup file: `creds.json.bak`
    - legacy default auth in `~/.alisio/credentials/` is still recognized/migrated for default-account flows
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="Group messages unexpectedly ignored">
    ...
    - duplicate keys in `alisio.json` (JSON5): later entries override earlier ones, so keep a single `groupPolicy` per scope
