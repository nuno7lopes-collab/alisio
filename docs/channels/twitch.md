...
read_when:
  - Setting up Twitch chat integration for Alisio
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Alisio connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
alisio plugins install @alisio/twitch
```

Local checkout (when running from a git repo):

```bash
alisio plugins install ./path/to/local/twitch-plugin
```
...
4. Configure the token:
   - Env: `ALISIO_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "alisio", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use ALISIO_TWITCH_ACCESS_TOKEN env var)
...
```bash
ALISIO_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
