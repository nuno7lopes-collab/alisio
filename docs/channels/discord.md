...
You will need to create a new application with a bot, add the bot to your server, and pair it to Alisio. We recommend adding your bot to your own private server. If you don't have one yet, [create one first](https://support.discord.com/hc/en-us/articles/204849977-How-do-I-create-a-server) (choose **Create My Own > For me and my friends**).

<Steps>
  <Step title="Create a Discord application and bot">
    Go to the [Discord Developer Portal](https://discord.com/developers/applications) and click **New Application**. Name it something like "Alisio".

    Click **Bot** on the sidebar. Set the **Username** to whatever you call your Alisio agent.
  ...
  <Step title="Enable Developer Mode and collect your IDs">
    ...
    Save your **Server ID** and **User ID** alongside your Bot Token — you'll send all three to Alisio in the next step.
  ...
  <Step title="Allow DMs from server members">
    ...
    This lets server members (including bots) send you DMs. Keep this enabled if you want to use Discord DMs with Alisio. If you only plan to use guild channels, you can disable DMs after pairing.

  </Step>

  <Step title="Set your bot token securely (do not send it in chat)">
    Your Discord bot token is a secret (like a password). Set it on the machine running Alisio before messaging your agent.

```bash
export DISCORD_BOT_TOKEN="YOUR_BOT_TOKEN"
alisio config set channels.discord.token --ref-provider default --ref-source env --ref-id DISCORD_BOT_TOKEN --dry-run
alisio config set channels.discord.token --ref-provider default --ref-source env --ref-id DISCORD_BOT_TOKEN
alisio config set channels.discord.enabled true --strict-json
alisio gateway
```

    If Alisio is already running as a background service, restart it via the Alisio Mac app or by stopping and restarting the `alisio gateway run` process.

  </Step>

  <Step title="Configure Alisio and pair">

    <Tabs>
      <Tab title="Ask your agent">
        Chat with your Alisio agent on any existing channel (e.g. Telegram) and tell it. If Discord is your first channel, use the CLI / config tab instead.
...
```bash
alisio pairing list discord
alisio pairing approve discord <CODE>
```
...
Discord forum and media channels only accept thread posts. Alisio supports two ways to create them:
...
- Use `alisio message thread create` to create a thread directly. Do not pass `--message-id` for forum channels.

Example: send to forum parent to create a thread

```bash
alisio message send --channel discord --target channel:<forumId> \
  --message "Topic title\nBody of the post"
```
...
```bash
alisio message thread create --channel discord --target channel:<forumId> \
...
Alisio supports Discord components v2 containers for agent messages. Use the message tool with a `components` payload. Interaction results are routed back to the agent as normal inbound messages and follow the existing Discord `replyToMode` settings.
...
- Alisio adds a trigger button automatically
...
<Tabs>
  ...
  <Tab title="Guild policy">
    ...
    - names/tags are supported for `users`, but IDs are safer; `alisio security audit` warns when name/tag entries are used
...
<AccordionGroup>
  ...
  <Accordion title="Copy IDs">
    ...
    Prefer numeric IDs in Alisio config for reliable audits and probes.
...
- Commands may still be visible in Discord UI for users who are not authorized; execution still enforces Alisio auth and returns "not authorized".
...
<AccordionGroup>
  ...
  <Accordion title="Live stream preview">
    Alisio can stream draft replies by sending a temporary message and editing it as text arrives.
...
```
    ...
    enabled for Discord, Alisio skips the preview stream to avoid double streaming.
...
```
    ...
    - `spawnAcpSessions` is only required when Alisio needs to create/bind a child thread via `--thread auto|here`. It is not required for `/acp spawn ... --bind here` in the current channel.
  ...
  <Accordion title="Ack reactions">
    `ackReaction` sends an acknowledgement emoji while Alisio is processing an inbound message.
...
```json5
{
  channels: {
    discord: {
      ...
      activityType: 1,
      activityUrl: "https://twitch.tv/alisio",
    },
  },
}
```
...
```
  ...
  <Accordion title="Exec approvals in Discord">
    ...
    - env-first local auth (`ALISIO_GATEWAY_TOKEN` / `ALISIO_GATEWAY_PASSWORD` then `gateway.auth.*`)
...
Alisio uses Discord components v2 for exec approvals and cross-context markers. Discord message actions can also accept `components` for custom UI (advanced; requires constructing a component payload via the discord tool), while legacy `embeds` remain available but are not recommended.
...
Alisio can join Discord voice channels for realtime, continuous conversations. This is separate from voice message attachments.
...
- Alisio also watches receive decrypt failures and auto-recovers by leaving/rejoining the voice channel after repeated failures in a short window.
...
Discord voice messages show a waveform preview and require OGG/Opus audio plus metadata. Alisio generates the waveform automatically, but it needs `ffmpeg` and `ffprobe` available on the gateway host to inspect and convert audio files.

Requirements and constraints:

- Provide a **local file path** (URLs are rejected).
- Omit text content (Discord does not allow text + voice message in the same payload).
- Any audio format is accepted; Alisio converts to OGG/Opus when needed.
...
```
  ...
  <Accordion title="Voice STT drops with DecryptionFailed(...)">
    ...
    - keep Alisio current (`alisio update`) so the Discord voice receive recovery logic is present
...
- streaming: `streaming`, `draftChunk`, `blockStreaming`, `blockStreamingCoalesce`
...
- If command deploy/state is stale, restart gateway and re-check with `alisio channels status --probe`.
