...
This guide summarizes a sane workflow for working on the pi integration in Alisio.
...
```bash
ALISIO_LIVE_TEST=1 pnpm test -- src/agents/pi-embedded-runner-extraparams.live.test.ts
```
...
- Trigger the agent directly:
  - `pnpm alisio agent --message "Hello" --thinking low`
...
State lives under the Alisio state directory. Default is `~/.alisio`. If `ALISIO_STATE_DIR` is set, use that directory instead.

To reset everything:

- `alisio.json` for config
