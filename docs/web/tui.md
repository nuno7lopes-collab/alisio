...
```bash
alisio tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `alisio tui --deliver`
...
- Local shell commands receive `ALISIO_SHELL=tui-local` in their environment.
...
- If your terminal uses a light background and auto-detection is wrong, set `ALISIO_THEME=light` before launching `alisio tui`.
- To force the original dark palette instead, set `ALISIO_THEME=dark`.
...
- Check the Gateway logs: `alisio logs --follow`.
- Confirm the agent can run: `alisio status` and `alisio models status`.
...
- No agents in picker: check `alisio agents list` and your routing config.
