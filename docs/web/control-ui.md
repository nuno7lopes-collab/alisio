...
If the page fails to load, start the Gateway first: `alisio gateway`.
...
created. Re-run `alisio devices list` before approval.

Once approved, the device is remembered and won't require re-approval unless
you revoke it with `alisio devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.alisio/alisio.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop alisio`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Alisio
...
Alisio **blocks** Control UI connections without device identity.

**Recommended fix:** use HTTPS (Tailscale Serve) or open the UI locally:

- `https://<magicdns>/` (Serve)
- `http://127.0.0.1:40705/` (on the gateway host)
...
Then point the UI at your Gateway WS URL (e.g. `ws://127.0.0.1:40705`).
