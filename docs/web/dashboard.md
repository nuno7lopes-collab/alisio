...
- If the UI prompts for auth, paste the token from `gateway.auth.token` (or `ALISIO_GATEWAY_TOKEN`) into Control UI settings.
...
- Retrieve or supply the token from the gateway host:
  - Plaintext config: `alisio config get gateway.auth.token`
  - SecretRef-managed config: resolve the external secret provider or export `ALISIO_GATEWAY_TOKEN` in this shell, then rerun `alisio dashboard`
  - No token configured: `alisio doctor --generate-gateway-token`
