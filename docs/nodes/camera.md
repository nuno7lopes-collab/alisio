...
```bash
alisio nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
alisio nodes camera snap --node <id> --facing front
alisio nodes camera clip --node <id> --duration 3000
alisio nodes camera clip --node <id> --no-audio
```
...
Use the main `alisio` CLI to invoke camera commands on the macOS node.

Examples:

```bash
alisio nodes camera list --node <id>            # list camera ids
alisio nodes camera snap --node <id>            # prints MEDIA:<path>
alisio nodes camera snap --node <id> --max-width 1280
alisio nodes camera snap --node <id> --delay-ms 2000
alisio nodes camera snap --node <id> --device-id <id>
alisio nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
alisio nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
alisio nodes camera clip --node <id> --device-id <id>
alisio nodes camera clip --node <id> --no-audio
```

Notes:

- `alisio nodes camera snap` defaults to `maxWidth=1600` unless overridden.
