---
summary: "Alisio on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up Alisio on DigitalOcean
  - Looking for cheap VPS hosting for Alisio
title: "DigitalOcean (Platform)"
---

# Alisio on DigitalOcean

## Goal

Run a persistent Alisio Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
# Check service
systemctl --user status alisio-gateway.service

# View logs
journalctl --user -u alisio-gateway.service -f
```
...
```bash
alisio config set gateway.bind tailnet
alisio gateway restart
```

Open: `http://<tailscale-ip>:40705` (token required).
...
```bash
alisio channels login whatsapp
# Scan QR code
```
...
- `~/.alisio/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl -u alisio --no-pager -n 50
```
