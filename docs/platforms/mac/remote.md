---
summary: "macOS app flow for controlling a remote Alisio gateway over SSH"
read_when:
...
- **Remote over SSH (default)**: Alisio commands are executed on the remote host. The mac app opens an SSH connection with `-o BatchMode` plus your chosen identity/key and a local port-forward.
...
1. Install Node + pnpm and build/install the Alisio CLI (`pnpm install && pnpm build && pnpm link --global`).
2. Ensure `alisio` is on PATH for non-interactive shells (symlink into `/usr/local/bin` or `/opt/homebrew/bin` if needed).
3. Open SSH with key auth. We recommend **Tailscale** IPs for stable reachability off-LAN.

## macOS app setup

1. Open _Settings → General_.
2. Under **Alisio runs**, pick **Remote over SSH** and set:
...
- Run `alisio channels login --verbose` **on the remote host**. Scan the QR with WhatsApp on your phone.
...
Pick sounds per notification from scripts with `alisio` and `node.invoke`, e.g.:
