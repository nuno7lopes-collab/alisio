---
summary: "Menu bar icon states and animations for Alisio on macOS"
read_when:
...
- **Working (agent running):** `AppState.isWorking=true` drives a “tail/leg scurry” micro-motion: faster leg wiggle and slight offset while work is in-flight. It should wrap real workspace runs and other long native macOS tasks.
...
- Agent activity: set `AppStateStore.shared.setWorking(true/false)` around work spans in the native workspace flow. Keep spans short and reset in `defer` blocks to avoid stuck animations.
