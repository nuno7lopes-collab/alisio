...
- Network path:
  ...
  - Tailnet via unicast DNS-SD (example domain: `alisio.internal.`), **or**
...
- `ALISIO_APNS_RELAY_BASE_URL` still works as a temporary env override for the gateway.
...
- Only genuine Alisio iOS builds distributed through Apple can use the hosted relay.
...
- To allow hosted relay usage only for official/TestFlight Alisio builds.
...
```bash
export ALISIO_APNS_TEAM_ID="TEAMID"
export ALISIO_APNS_KEY_ID="KEYID"
export ALISIO_APNS_PRIVATE_KEY_P8="$(cat /path/to/AuthKey_KEYID.p8)"
```
...
The Gateway advertises `_alisio-gw._tcp` on `local.`. The iOS app lists these automatically.

### Tailnet (cross-network)

If mDNS is blocked, use a unicast DNS-SD zone (choose a domain; example: `alisio.internal.`) and Tailscale split DNS.
...
```bash
alisio nodes invoke --node "iOS Node" --command canvas.eval --params '{"javaScript":"(() => { const {ctx} = window.__alisio; ctx.clearRect(0,0,innerWidth,innerHeight); ctx.lineWidth=6; ctx.strokeStyle=\"#ff2d55\"; ctx.beginPath(); ctx.moveTo(40,40); ctx.lineTo(innerWidth-40, innerHeight-40); ctx.stroke(); return \"ok\"; })()"}'
```

```bash
alisio nodes invoke --node "iOS Node" --command canvas.snapshot --params '{"maxWidth":900,"format":"jpeg"}'
```
...
- Pairing prompt never appears: run `alisio devices list` and approve manually.
