...
Nostr 是一种用于社交网络的去中心化协议。此渠道使 Alisio 能够通过 NIP-04 接收并回复加密私信。

## 安装（按需）

### 新手引导（推荐）

- 设置向导（`alisio onboard`）和 `alisio channels add` 会列出可选渠道插件。
...
```bash
alisio plugins install --link <path-to-alisio>/extensions/nostr
```
