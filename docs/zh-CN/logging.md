...
你可以在 `~/.alisio/alisio.json` 中覆盖此设置：
...
所有日志配置都在 `~/.alisio/alisio.json` 的 `logging` 下。
...
- Alisio 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `alisio plugins enable diagnostics-otel` 启用插件。
...
- `alisio.tokens`（计数器，属性：`alisio.token`、`alisio.channel`、`alisio.provider`、`alisio.model`）
- `alisio.cost.usd`（计数器，属性：`alisio.channel`、`alisio.provider`、`alisio.model`）
- `alisio.run.duration_ms`（直方图，属性：`alisio.channel`、`alisio.provider`、`alisio.model`）
- `alisio.context.tokens`（直方图，属性：`alisio.context`、`alisio.channel`、`alisio.provider`、`alisio.model`）

消息流：

- `alisio.webhook.received`（计数器，属性：`alisio.channel`、`alisio.webhook`）
- `alisio.webhook.error`（计数器，属性：`alisio.channel`、`alisio.webhook`）
- `alisio.webhook.duration_ms`（直方图，属性：`alisio.channel`、`alisio.webhook`）
- `alisio.message.queued`（计数器，属性：`alisio.channel`、`alisio.source`）
- `alisio.message.processed`（计数器，属性：`alisio.channel`、`alisio.outcome`）
- `alisio.message.duration_ms`（直方图，属性：`alisio.channel`、`alisio.outcome`）

队列 + 会话：

- `alisio.queue.lane.enqueue`（计数器，属性：`alisio.lane`）
- `alisio.queue.lane.dequeue`（计数器，属性：`alisio.lane`）
- `alisio.queue.depth`（直方图，属性：`alisio.lane` 或 `alisio.channel=heartbeat`）
- `alisio.queue.wait_ms`（直方图，属性：`alisio.lane`）
- `alisio.session.state`（计数器，属性：`alisio.state`、`alisio.reason`）
- `alisio.session.stuck`（计数器，属性：`alisio.state`）
- `alisio.session.stuck_age_ms`（直方图，属性：`alisio.state`）
- `alisio.run.attempt`（计数器，属性：`alisio.attempt`）
...
- `alisio.webhook.error`
  - `alisio.channel`、`alisio.webhook`、`alisio.chatId`、`alisio.error`
- `alisio.message.processed`
  - `alisio.channel`、`alisio.outcome`、`alisio.chatId`、`alisio.messageId`、`alisio.sessionKey`、`alisio.sessionId`、`alisio.reason`
- `alisio.session.stuck`
  - `alisio.state`、`alisio.ageMs`、`alisio.queueDepth`、`alisio.sessionKey`、`alisio.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `alisio doctor`。
