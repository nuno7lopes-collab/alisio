---
read_when:
  - 你想在 Alisio 中使用 MiniMax 模型
  - 你需要 MiniMax 设置指南
summary: 在 Alisio 中使用 MiniMax M2.5
title: MiniMax
x-i18n:
...
```bash
alisio plugins enable minimax  # 如果已加载则跳过。
alisio gateway restart  # 如果 gateway 已在运行，则重启
...
详情请参阅 [MiniMax plugin README](https://github.com/alisio/alisio/tree/main/extensions/minimax)。
...
### 可选：通过兼容 OpenAI 的本地服务器运行（手动）

**最适合：** 通过你自己的兼容 OpenAI 端点进行本地推理。
在强力硬件上，本地运行大型 MiniMax 模型依然是可行方案。
...
- 使用 `alisio models list` 和 `alisio models set minimax/MiniMax-M2.5` 进行切换。
...
- 运行 `alisio configure` 并选择 **MiniMax M2.5**，或者
