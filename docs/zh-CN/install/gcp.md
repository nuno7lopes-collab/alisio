---
read_when:
  - 你想在 GCP 上 24/7 运行 Alisio
  - 你想要在自己的 VM 上运行生产级、常驻的 Gateway 网关
  - 你想完全控制持久化、二进制文件和重启行为
summary: 在 GCP Compute Engine VM（Docker）上 24/7 运行 Alisio Gateway 网关并持久化状态
title: GCP
x-i18n:
...
# 在 GCP Compute Engine 上运行 Alisio（Docker，生产 VPS 指南）

## 目标

使用 Docker 在 GCP Compute Engine VM 上运行持久化的 Alisio Gateway 网关，具有持久状态、内置二进制文件和安全的重启行为。

如果你想要"Alisio 24/7 大约 $5-12/月"，这是在 Google Cloud 上的可靠设置。
...
- 安装 Docker（隔离的应用运行时）
- 在 Docker 中启动 Alisio Gateway 网关
- 在主机上持久化 `~/.alisio` + `~/.alisio/workspace`（重启/重建后仍保留）
...
```bash
gcloud projects create my-alisio-project --name="Alisio Gateway"
gcloud config set project my-alisio-project
```
...
```bash
gcloud compute instances create alisio-gateway \
  --zone=us-central1-a \
...
```bash
gcloud compute ssh alisio-gateway --zone=us-central1-a
```
...
XDG_CONFIG_HOME=/home/node/.alisio
```
...
services:
  alisio-gateway:
    ...
    environment:
      ...
      - NODE_ENV=production
      - TERM=xterm-256color
      - ALISIO_GATEWAY_BIND=${ALISIO_GATEWAY_BIND}
      - ALISIO_GATEWAY_PORT=${ALISIO_GATEWAY_PORT}
      - ALISIO_GATEWAY_TOKEN=${ALISIO_GATEWAY_TOKEN}
...
```bash
docker compose exec alisio-gateway which gog
docker compose exec alisio-gateway which goplaces
docker compose exec alisio-gateway which wacli
```
...
```bash
docker compose logs -f alisio-gateway
```
...
Alisio 在 Docker 中运行，但 Docker 不是真实来源。
所有长期状态必须在重启、重建和重启后仍然存在。

| 组件             | 位置                              | 持久化机制    | 说明                        |
| ---------------- | --------------------------------- | ------------- | --------------------------- |
| Gateway 网关配置 | `/home/node/.alisio/`           | 主机卷挂载    | 包括 `alisio.json`、令牌  |
| 模型认证配置文件 | `/home/node/.alisio/`           | 主机卷挂载    | OAuth 令牌、API 密钥        |
| Skill 配置       | `/home/node/.alisio/skills/`    | 主机卷挂载    | Skill 级别状态              |
| 智能体工作区     | `/home/node/.alisio/workspace/` | 主机卷挂载    | 代码和智能体产物            |
| WhatsApp 会话    | `/home/node/.alisio/`           | 主机卷挂载    | 保留 QR 登录                |
| Gmail 密钥环     | `/home/node/.alisio/`           | 主机卷 + 密码 | 需要 `GOG_KEYRING_PASSWORD` |
...
```bash
# 首先停止 VM
gcloud compute instances stop alisio-gateway --zone=us-central1-a

# 更改机器类型
gcloud compute instances set-machine-type alisio-gateway \
  --zone=us-central1-a \
  --machine-type=e2-small

# 启动 VM
gcloud compute instances start alisio-gateway --zone=us-central1-a
```
...
1. 创建服务账户：
   ...
   ```bash
   gcloud iam service-accounts create alisio-deploy \
     --display-name="Alisio Deployment"
   ```

2. 授予 Compute Instance Admin 角色（或更窄的自定义角色）：
   ```bash
   gcloud projects add-iam-policy-binding my-alisio-project \
     --member="serviceAccount:alisio-deploy@my-alisio-project.iam.gserviceaccount.com" \
