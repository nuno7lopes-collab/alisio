...
summary: 在 exe.dev 上运行 Alisio Gateway 网关（VM + HTTPS 代理）以实现远程访问
title: exe.dev
x-i18n:
...
目标：让 Alisio Gateway 网关运行在 exe.dev VM 上，并且可通过你的笔记本电脑访问：`https://<vm-name>.exe.xyz`
...
提示：请让这个 VM 保持**有状态**。Alisio 会将状态存储在 `~/.alisio/` 和 `~/.alisio/workspace/` 下。
...
## 4）设置 nginx，将 Alisio 代理到端口 8000
...
`gateway.auth.token` 中的令牌（可通过 `alisio config get gateway.auth.token` 获取，或使用
`alisio doctor --generate-gateway-token` 生成）。使用 `alisio devices list` 和
`alisio devices approve <requestId>` 批准设备。如果拿不准，请在浏览器中使用 Shelley！
