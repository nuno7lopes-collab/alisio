...
<Tabs>
  <Tab title="install.sh">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://alisio.ai/install.sh | bash
    ```

    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://alisio.ai/install.sh | bash -s -- --help
    ```

  </Tab>
  <Tab title="install-cli.sh">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://alisio.ai/install-cli.sh | bash
    ```

    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://alisio.ai/install-cli.sh | bash -s -- --help
    ```
  ...
  <Tab title="install.ps1">
    ...
    ```powershell
    & ([scriptblock]::Create((iwr -useb https://alisio.ai/install.ps1))) -Tag beta -NoOnboard -DryRun
    ```
...
<Note>
如果安装成功但在新终端中找不到 `alisio`，请参见 [Node.js 故障排除](/install/node#troubleshooting)。
</Note>
...
<Steps>
  ...
  <Step title="默认确保使用 Node.js 24">
    检查 Node 版本，并在需要时安装 Node 24（macOS 上使用 Homebrew，Linux apt/dnf/yum 上使用 NodeSource 设置脚本）。为了兼容性，Alisio 仍支持 Node 22 LTS，目前为 `22.16+`。
  </Step>
  <Step title="确保安装 Git">
    如果缺少 Git，则安装它。
  </Step>
  <Step title="安装 Alisio">
    - `npm` 方法（默认）：全局 npm 安装
    - `git` 方法：克隆/更新仓库，使用 pnpm 安装依赖，构建，然后将包装器安装到 `~/.local/bin/alisio`
  </Step>
  <Step title="安装后任务">
    - 在升级和 git 安装时运行 `alisio doctor --non-interactive`（尽力而为）
...
如果在 Alisio 检出目录中运行（`package.json` + `pnpm-workspace.yaml`），脚本会提供：
...
<Tabs>
  ...
  <Tab title="跳过新手引导">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://alisio.ai/install.sh | bash -s -- --no-onboard
    ```
  </Tab>
  <Tab title="Git 安装">
    ```bash
  ...
  <Tab title="通过 npm 安装 GitHub main">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://alisio.ai/install.sh | bash -s -- --version main
    ```
  </Tab>
  <Tab title="试运行">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://alisio.ai/install.sh | bash -s -- --dry-run
    ```
  </Tab>
</Tabs>
...
| `ALISIO_GIT_UPDATE=0\|1`                              | 切换 git 更新                        |
| `ALISIO_NO_PROMPT=1`                                  | 禁用提示                             |
...
<Info>
适用于你希望所有内容都放在本地前缀（默认 `~/.alisio`）下，并且不依赖系统 Node 的环境。
</Info>
...
<Steps>
  ...
  <Step title="在前缀下安装 Alisio">
    使用 `--prefix <prefix>` 通过 npm 安装，然后将包装器写入 `<prefix>/bin/alisio`。
  </Step>
</Steps>
...
<Tabs>
  ...
  <Tab title="自定义前缀 + 版本">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://alisio.ai/install-cli.sh | bash -s -- --prefix /opt/alisio --version latest
    ```
  </Tab>
  <Tab title="自动化 JSON 输出">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://alisio.ai/install-cli.sh | bash -s -- --json --prefix /opt/alisio
    ```
  </Tab>
  <Tab title="运行新手引导">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://alisio.ai/install-cli.sh | bash -s -- --onboard
    ```
  </Tab>
</Tabs>
...
<Steps>
  ...
  <Step title="安装 Alisio">
    ...
    - `git` 方法：克隆/更新仓库，使用 pnpm 安装/构建，并将包装器安装到 `%USERPROFILE%\.local\bin\alisio.cmd`
  </Step>
  <Step title="安装后任务">
    在可能情况下将所需 bin 目录添加到用户 PATH，然后在升级和 git 安装时运行 `alisio doctor --non-interactive`（尽力而为）。
  </Step>
</Steps>
...
<Tabs>
  ...
  <Tab title="Git 安装">
    ```powershell
    & ([scriptblock]::Create((iwr -useb https://alisio.ai/install.ps1))) -InstallMethod git
    ```
  </Tab>
  <Tab title="通过 npm 安装 GitHub main">
    ```powershell
    & ([scriptblock]::Create((iwr -useb https://alisio.ai/install.ps1))) -Tag main
    ```
  </Tab>
  <Tab title="自定义 git 目录">
    ```powershell
    & ([scriptblock]::Create((iwr -useb https://alisio.ai/install.ps1))) -InstallMethod git -GitDir "C:\alisio"
    ```
  </Tab>
  <Tab title="试运行">
    ```powershell
    & ([scriptblock]::Create((iwr -useb https://alisio.ai/install.ps1))) -DryRun
    ```
  </Tab>
  <Tab title="调试跟踪">
    ```powershell
    ...
    Set-PSDebug -Trace 1
    & ([scriptblock]::Create((iwr -useb https://alisio.ai/install.ps1))) -NoOnboard
    Set-PSDebug -Trace 0
    ```
  </Tab>
</Tabs>
...
| `ALISIO_GIT_DIR=<path>`          | 检出目录      |
| `ALISIO_NO_ONBOARD=1`            | 跳过新手引导  |
| `ALISIO_GIT_UPDATE=0`            | 禁用 git pull |
| `ALISIO_DRY_RUN=1`               | 试运行模式    |
...
<Tabs>
  <Tab title="install.sh（非交互式 npm）">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://alisio.ai/install.sh | bash -s -- --no-prompt --no-onboard
    ```
  </Tab>
...
<AccordionGroup>
  ...
  <Accordion title="sharp/libvips 问题">
    ...
    ```bash
    SHARP_IGNORE_GLOBAL_LIBVIPS=0 curl -fsSL --proto '=https' --tlsv1.2 https://alisio.ai/install.sh | bash
    ```
