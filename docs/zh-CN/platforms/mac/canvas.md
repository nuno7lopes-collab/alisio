...
- `~/Library/Application Support/Alisio/canvas/<session>/...`

Canvas 面板通过**自定义 URL 方案**提供这些文件：

- `alisio-canvas://<session>/<path>`

示例：

- `alisio-canvas://main/` → `<canvasRoot>/main/index.html`
- `alisio-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
- `alisio-canvas://main/widgets/todo/` → `<canvasRoot>/main/widgets/todo/index.html`
...
```bash
alisio nodes canvas present --node <id>
alisio nodes canvas navigate --node <id> --url "/"
alisio nodes canvas eval --node <id> --js "document.title"
alisio nodes canvas snapshot --node <id>
```
...
```
http://<gateway-host>:40709/__alisio__/a2ui/
```
...
alisio nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

快速测试：

```bash
alisio nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```
...
```js
window.location.href = "alisio://agent?message=Review%20this%20design";
```
