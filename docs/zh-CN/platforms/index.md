...
- 服务状态：`alisio gateway status`

## Gateway 网关服务安装（CLI）

使用以下任一方式（均受支持）：

- 向导（推荐）：`alisio onboard --install-daemon`
- 直接安装：`alisio gateway install`
- 配置流程：`alisio configure` → 选择 **Gateway 服务**
- 修复/迁移：`alisio doctor`（会提供安装或修复服务的选项）

服务目标取决于操作系统：

- macOS：LaunchAgent（`ai.alisio.gateway` 或 `ai.alisio.<profile>`；旧版为 `com.alisio.*`）
- Linux/WSL2：systemd 用户服务（`alisio-gateway[-<profile>].service`）
