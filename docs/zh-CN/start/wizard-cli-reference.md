...
- 模型和身份验证设置（OpenAI Code 订阅 OAuth、Anthropic API 密钥或 setup token，以及 MiniMax、GLM、Moonshot、AI Gateway 和自定义服务器选项）
- 工作区位置和 bootstrap 文件
...
<Steps>
  ...
  <Step title="渠道">
    ...
    - 私信安全：默认是配对。首次私信会发送一个代码；通过
      `alisio pairing approve <channel> <code>` 批准，或使用 allowlist。
  </Step>
  <Step title="守护进程安装">
    - macOS：LaunchAgent
  ...
  <Step title="健康检查">
    - 启动 Gateway 网关（如有需要），并运行 `alisio health`。
    - `alisio status --deep` 会在状态输出中添加 Gateway 网关健康探测。
  </Step>
  <Step title="Skills">
    - 读取可用的 Skills 并检查要求。
...
<AccordionGroup>
  ...
  <Accordion title="自定义提供商">
    适用于本地或远程的兼容 OpenAI 和兼容 Anthropic 端点。
...
- OAuth 凭证：`~/.alisio/credentials/oauth.json`
- Auth profile（API 密钥 + OAuth）：`~/.alisio/agents/<agentId>/agent/auth-profiles.json`
...
`~/.alisio/credentials/oauth.json`（或 `$ALISIO_STATE_DIR/credentials/oauth.json`）
到 Gateway 网关主机。
</Note>

## 输出和内部机制

`~/.alisio/alisio.json` 中的典型字段：
...
`alisio agents add` 会写入 `agents.list[]` 和可选的 `bindings`。

WhatsApp 凭证位于 `~/.alisio/credentials/whatsapp/<accountId>/`。
会话存储在 `~/.alisio/agents/<agentId>/sessions/` 下。
...
- 下载适当的发布资源
- 将其存储在 `~/.alisio/tools/signal-cli/<version>/`
...
- 命令参考：[`alisio onboard`](/cli/onboard)
