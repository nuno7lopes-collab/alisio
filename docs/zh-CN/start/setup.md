...
summary: 设置指南：在保持最新的同时保持你的 Alisio 设置个性化
title: 设置
x-i18n:
...
- **个性化设置存放在仓库之外：** `~/.alisio/workspace`（工作区）+ `~/.alisio/alisio.json`（配置）。
...
- **配置：** `~/.alisio/alisio.json`（JSON/JSON5 格式）
- **工作区：** `~/.alisio/workspace`（Skills、提示、记忆；将其设为私有 git 仓库）
...
如果你还没有全局安装，通过 `pnpm alisio setup` 运行它。

## 稳定工作流（macOS 应用优先）

1. 安装并启动 **Alisio.app**（菜单栏）。
...
- 运行 `alisio setup`，然后 `alisio channels login`，然后手动启动 Gateway 网关（`alisio gateway`）。
...
- **端口错误：** Gateway 网关 WS 默认为 `ws://127.0.0.1:40705`；保持应用 + CLI 在同一端口上。
- **状态存储位置：**
  - 凭证：`~/.alisio/credentials/`
  - 会话：`~/.alisio/agents/<agentId>/sessions/`
  - 日志：`/tmp/alisio/`
...
- **配对允许列表**：`~/.alisio/credentials/<channel>-allowFrom.json`
- **模型认证配置文件**：`~/.alisio/agents/<agentId>/agent/auth-profiles.json`
- **旧版 OAuth 导入**：`~/.alisio/credentials/oauth.json`
  更多详情：[安全](/gateway/security#credential-storage-map)。

## 更新（不破坏你的设置）

- 将 `~/.alisio/workspace` 和 `~/.alisio/` 保持为"你的东西"；不要将个人提示/配置放入 `alisio` 仓库。
