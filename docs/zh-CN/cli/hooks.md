...
read_when:
  - 你想管理智能体钩子
  - 你想安装或更新钩子
summary: CLI 参考：`alisio hooks`（智能体钩子）
title: hooks
x-i18n:
...
通过将特定钩子添加到配置（`~/.alisio/config.json`）来启用它。

**注意：** 由插件管理的钩子在 `alisio hooks list` 中显示 `plugin:<id>`，
...
```bash
# 本地目录
alisio hooks install ./my-hook-pack

# 本地压缩包
alisio hooks install ./my-hook-pack.zip

# NPM 包
alisio hooks install @alisio/my-hook-pack

# 链接本地目录而不复制
alisio hooks install -l ./my-hook-pack
```

## 更新钩子

```bash
alisio hooks update <id>
alisio hooks update --all
```
...
**输出：** `~/.alisio/workspace/memory/YYYY-MM-DD-slug.md`
