...
Alisio 使用 Bonjour（mDNS / DNS‑SD）作为**仅限局域网的便捷方式**来发现
...
2. 在专用区域下发布 `_alisio-gw._tcp` 的 DNS‑SD 记录
   （示例：`alisio.internal.`）。
...
- 从 `~/.alisio/dns/<domain>.db` 提供你选择的域名服务（示例：`alisio.internal.`）
...
你的发现域名中浏览 `_alisio-gw._tcp`，无需多播。
...
- 在 `~/.alisio/alisio.json` 中设置 `gateway.bind: "tailnet"`。
- 重启 Gateway 网关（或重启 macOS 菜单栏应用）。

## 什么在广播

只有 Gateway 网关广播 `_alisio-gw._tcp`。
...
- `transport=gateway`
- `cliPath=<path>`（可选；可运行的 `alisio` 入口点的绝对路径）
...
iOS 节点使用 `NWBrowser` 来发现 `_alisio-gw._tcp`。
...
- `ALISIO_DISABLE_BONJOUR=1` 禁用广播（旧版：`ALISIO_DISABLE_BONJOUR`）。
- `~/.alisio/alisio.json` 中的 `gateway.bind` 控制 Gateway 网关绑定模式。
- `ALISIO_SSH_PORT` 覆盖 TXT 中广播的 SSH 端口（旧版：`ALISIO_SSH_PORT`）。
- `ALISIO_TAILNET_DNS` 在 TXT 中发布 MagicDNS 提示（旧版：`ALISIO_TAILNET_DNS`）。
- `ALISIO_CLI_PATH` 覆盖广播的 CLI 路径（旧版：`ALISIO_CLI_PATH`）。
