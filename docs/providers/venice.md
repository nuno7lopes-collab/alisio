---
summary: "Use Venice AI privacy-focused models in Alisio"
read_when:
  - You want privacy-focused inference in Alisio
...
```bash
alisio onboard --auth-choice venice-api-key
```
...
```bash
alisio agent --model venice/kimi-k2-5 --message "Hello, are you working?"
```

## Model Selection

After setup, Alisio shows all available Venice models. Pick based on your needs:
...
```bash
alisio models set venice/kimi-k2-5
alisio models set venice/claude-opus-4-6
```

List all available models:

```bash
alisio models list | grep venice
```

## Configure via `alisio configure`

1. Run `alisio configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Alisio automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
alisio agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
alisio agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
alisio agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
alisio agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
alisio agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `alisio models list` to see currently available models. Some models may be temporarily offline.
