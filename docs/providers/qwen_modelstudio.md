...
read_when:
  - You want to use Qwen (Alibaba Cloud Model Studio) with Alisio
...
```bash
# China endpoint
alisio onboard --auth-choice modelstudio-standard-api-key-cn

# Global/Intl endpoint
alisio onboard --auth-choice modelstudio-standard-api-key
```
...
```bash
# China endpoint
alisio onboard --auth-choice modelstudio-api-key-cn

# Global/Intl endpoint
alisio onboard --auth-choice modelstudio-api-key
```
...
`~/.alisio/.env` or via `env.shellEnv`).
