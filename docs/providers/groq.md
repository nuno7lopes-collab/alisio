...
read_when:
  - You want to use Groq with Alisio
...
(Llama, Gemma, Mistral, and more) using custom LPU hardware. Alisio connects
...
media-understanding provider, Alisio uses Groq's `whisper-large-v3-turbo`
...
available to that process (for example, in `~/.alisio/.env` or via
`env.shellEnv`).

## Available models

Groq's model catalog changes frequently. Run `alisio models list | grep groq`
