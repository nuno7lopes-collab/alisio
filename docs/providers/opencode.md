---
summary: "Use OpenCode Zen and Go catalogs with Alisio"
read_when:
...
OpenCode exposes two hosted catalogs in Alisio:

- `opencode/...` for the **Zen** catalog
- `opencode-go/...` for the **Go** catalog

Both catalogs use the same OpenCode API key. Alisio keeps the runtime provider ids
...
alisio onboard --opencode-zen-api-key "$OPENCODE_API_KEY"
```

### Go catalog

```bash
alisio onboard --auth-choice opencode-go
alisio onboard --opencode-go-api-key "$OPENCODE_API_KEY"
```
