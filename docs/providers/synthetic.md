---
summary: "Use Synthetic's Anthropic-compatible API in Alisio"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. Alisio registers it as the
...
```bash
alisio onboard --auth-choice synthetic-api-key
```
...
Note: Alisio's Anthropic client appends `/v1` to the base URL, so use
