---
summary: "Run Alisio with SGLang (OpenAI-compatible self-hosted server)"
read_when:
  - You want to run Alisio against a local SGLang server
...
Alisio can connect to SGLang using the `openai-completions` API.

Alisio can also **auto-discover** available models from SGLang when you opt
...
define `models.providers.sglang`, Alisio will query:
