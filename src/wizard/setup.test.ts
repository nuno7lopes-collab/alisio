...
const resolveGatewayPort = vi.hoisted(() =>
  vi.fn((_cfg?: unknown, env?: NodeJS.ProcessEnv) => {
    const raw = env?.ALISIO_GATEWAY_PORT ?? process.env.ALISIO_GATEWAY_PORT;
...
vi.mock("../commands/onboard-helpers.js", () => ({
  DEFAULT_WORKSPACE: "/tmp/alisio-workspace",
...
describe("runSetupWizard", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "alisio-onboard-suite-"));
  });
  ...
  it("resolves gateway.auth.password SecretRef for local setup probe", async () => {
    ...
    process.env.ALISIO_GATEWAY_PASSWORD = "gateway-ref-password"; // pragma: allowlist secret
    probeGatewayReachable.mockClear();
    readConfigFileSnapshot.mockResolvedValueOnce({
      path: "/tmp/.alisio/alisio.json",
      exists: true,
  ...
  it("shows the resolved gateway port in quickstart for fresh envs", async () => {
    const previousPort = process.env.ALISIO_GATEWAY_PORT;
    process.env.ALISIO_GATEWAY_PORT = "40707";
    const note: WizardPrompter["note"] = vi.fn(async () => {});
    const prompter = buildWizardPrompter({ note });
    const runtime = createRuntime();
    ...
    try {
      ...
      if (previousPort === undefined) {
        ...
        process.env.ALISIO_GATEWAY_PORT = previousPort;
      }
    }
