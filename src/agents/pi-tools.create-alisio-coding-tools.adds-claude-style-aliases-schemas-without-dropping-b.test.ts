...
const defaultTools = createAlisioCodingTools({ senderIsOwner: true });
...
describe("createAlisioCodingTools", () => {
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const openAiTools = createAlisioCodingTools({
      modelProvider: "openai",
      modelId: "gpt-5.2",
    });
    expect(openAiTools.some((tool) => tool.name === "apply_patch")).toBe(true);

    const codexTools = createAlisioCodingTools({
    ...
    const disabledOpenAiTools = createAlisioCodingTools({
    ...
    const anthropicTools = createAlisioCodingTools({
      config: disabledConfig,
    ...
    const allowed = createAlisioCodingTools({
    ...
    const denied = createAlisioCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
    ...
    const oauthTools = createAlisioCodingTools({
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
