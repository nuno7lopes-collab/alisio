...
async function loadFreshAlisioToolsModuleForTest() {
  vi.resetModules();
...
describe("gateway tool", () => {
  beforeEach(async () => {
    await loadFreshAlisioToolsModuleForTest();
  });
  ...
  it("schedules SIGUSR1 restart", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "alisio-test-"));

    try {
      await withEnvAsync({ ALISIO_STATE_DIR: stateDir, ALISIO_PROFILE: "isolated" }, async () => {
        const tool = requireGatewayTool();
        ...
        expect(parsed.payload?.doctorHint).toBe(
          "Run: alisio --profile isolated doctor --non-interactive",
        );
  ...
  it("rejects config.patch when it changes exec security", async () => {
    const { callGatewayTool } = await import("./tools/gateway.js");
  ...
  it("rejects config.apply when protected exec settings are omitted", async () => {
    ...
    await expect(
      tool.execute("call-missing-protected", {
        action: "config.apply",
        raw: '{ agents: { defaults: { workspace: "~/alisio" } } }',
      }),
    ).rejects.toThrow(
