...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    alisio: process.env.ALISIO_GATEWAY_TOKEN,
  };
  ...
  afterAll(() => {
    if (envSnapshot.alisio === undefined) {
      delete process.env.ALISIO_GATEWAY_TOKEN;
    } else {
      process.env.ALISIO_GATEWAY_TOKEN = envSnapshot.alisio;
    }
  });
  ...
  it("uses ALISIO_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.ALISIO_GATEWAY_TOKEN = "local-env-token";
    configState.value = {
      gateway: {
