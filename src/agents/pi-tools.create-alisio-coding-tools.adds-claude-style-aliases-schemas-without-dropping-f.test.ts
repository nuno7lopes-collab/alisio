...
describe("createAlisioCodingTools", () => {
  it("accepts Claude Code parameter aliases for read/write/edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "alisio-alias-"));
    try {
      const tools = createAlisioCodingTools({ workspaceDir: tmpDir });
  ...
  it("accepts broader file/edit alias variants", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "alisio-alias-broad-"));
    try {
      const tools = createAlisioCodingTools({ workspaceDir: tmpDir });
      const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
  ...
  it("coerces structured content blocks for write", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "alisio-structured-write-"));
    try {
      const tools = createAlisioCodingTools({ workspaceDir: tmpDir });
      ...
      await writeTool?.execute("tool-structured-write", {
        ...
        content: [
          ...
          {
            type: "input_text",
            text: "const root = path.join(process.env.HOME, 'alisio-workspace');\n",
          },
        ],
      });
      ...
      expect(written).toBe(
        "const path = require('path');\nconst root = path.join(process.env.HOME, 'alisio-workspace');\n",
      );
    } finally {
      await fs.rm(tmpDir, { recursive: true, force: true });
    }
  ...
  it("coerces structured old/new text blocks for edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "alisio-structured-edit-"));
    try {
