...
const GOOGLE_LIVE_SINGLE_KEY = "ALISIO_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "ALISIO_LIVE_ANTHROPIC_KEY",
    listVar: "ALISIO_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "ALISIO_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `ALISIO_LIVE_${base}_KEY`;
