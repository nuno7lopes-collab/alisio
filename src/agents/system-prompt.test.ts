...
describe("buildAgentSystemPrompt", () => {
  ...
  it("includes a CLI quick reference section", () => {
    ...
    expect(prompt).toContain("## Alisio CLI Quick Reference");
    expect(prompt).toContain("alisio gateway restart");
  ...
  it("lists available tools when provided", () => {
    ...
    expect(prompt).toContain(
      "If a requested capability is not listed above, say it is unavailable in this session and do not claim you are using it now.",
    );
  });

  it("does not advertise fallback tools when none are exposed", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/alisio",
      toolNames: [],
    });

    expect(prompt).toContain("No tools are currently exposed to you in this session.");
    expect(prompt).not.toContain("Pi lists the standard tools above. This runtime enables:");
    expect(prompt).not.toContain("- browser: control Alisio's dedicated browser");
  });
  ...
  it("preserves tool casing in the prompt", () => {
    ...
    expect(prompt).toContain("Alisio docs: /tmp/alisio/docs");
    expect(prompt).toContain(
  ...
  it("includes docs guidance when docsPath is provided", () => {
    ...
    expect(prompt).toContain("Legacy docs mirror (temporary): https://docs.alisio.ai");
    expect(prompt).toContain(
      "For Alisio behavior, commands, config, or architecture: consult local docs first.",
    );
  ...
  it("adds ClaudeBot self-update guidance when gateway tool is available", () => {
    ...
    expect(prompt).toContain("## Alisio Self-Update");
  ...
  it("tells the model not to reread injected bootstrap files or guess daily note paths", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/alisio",
    });
  ...
  it("describes sandboxed runtime and elevated when allowed", () => {
    ...
    expect(prompt).toContain(
      "For read/write/edit/apply_patch, file paths resolve against host workspace: /tmp/alisio. For bash/exec commands, use sandbox container paths under /workspace (or relative paths from that workdir), not host paths.",
    );
...
describe("buildSubagentSystemPrompt", () => {
  it("renders depth-1 orchestrator guidance, labels, and recovery notes", () => {
    ...
    expect(prompt).toContain("Do not use `exec` (`alisio ...`, `acpx ...`)");
    expect(prompt).toContain("Use `subagents` only for Alisio subagents");
