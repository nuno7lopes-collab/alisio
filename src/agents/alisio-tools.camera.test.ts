...
function getNodesTool(options?: { modelHasVision?: boolean; allowMediaInvokeCommands?: boolean }) {
  ...
  const tool = createAlisioTools(toolOptions).find((candidate) => candidate.name === "nodes");
  if (!tool) {
...
async function loadAlisioToolsForTest(): Promise<void> {
  vi.resetModules();
