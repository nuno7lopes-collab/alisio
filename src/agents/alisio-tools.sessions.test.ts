...
import { __testing as alisioToolsTesting, createAlisioTools } from "./alisio-tools.js";
...
describe("sessions tools", () => {
  ...
  beforeEach(() => {
    ...
    alisioToolsTesting.setDepsForTest({
      callGateway: (opts: unknown) => callGatewayMock(opts),
      config: TEST_CONFIG,
    });
  ...
  it("sessions_list filters kinds and includes messages", async () => {
    ...
    const tool = createAlisioTools().find((candidate) => candidate.name === "sessions_list");
    expect(tool).toBeDefined();
    if (!tool) {
  ...
  it("sessions_history filters tool messages by default", async () => {
    ...
    const tool = createAlisioTools().find((candidate) => candidate.name === "sessions_history");
    expect(tool).toBeDefined();
    if (!tool) {
