...
let createAlisioCodingTools: typeof import("./pi-tools.js").createAlisioCodingTools;

describe("createAlisioTools plugin context", () => {
  beforeEach(async () => {
    resolvePluginToolsMock.mockClear();
    vi.resetModules();
    ({ createAlisioTools } = await import("./alisio-tools.js"));
    ({ createAlisioCodingTools } = await import("./pi-tools.js"));
  });
  ...
  it("does not inject messageThreadId defaults for missing params objects", async () => {
    ...
    const wrapped = createAlisioTools({
      config: {} as never,
      agentThreadId: "77",
