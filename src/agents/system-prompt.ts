...
function buildMessagingSection(params: {
  ...
  return [
    ...
    "- Never use exec/curl for provider messaging; Alisio handles all routing internally.",
...
function buildDocsSection(params: { docsPath?: string; isMinimal: boolean; readToolName: string }) {
  ...
  return [
    ...
    "Legacy docs mirror (temporary): https://docs.alisio.ai",
...
export function buildAgentSystemPrompt(params: {
  ...
  if (promptMode === "none") {
    return "You are a personal assistant running inside Alisio.";
  }

  const lines = [
    "You are a personal assistant running inside Alisio.",
    "",
    "## Tooling",
    ...
    toolLines.length > 0
      ? toolLines.join("\n")
      : "No tools are currently exposed to you in this session.",
    ...
    "",
    ...safetySection,
    "## Alisio CLI Quick Reference",
    "Alisio is controlled via subcommands. Do not invent commands.",
    "To manage the Gateway daemon service (start/stop/restart):",
    "- alisio gateway status",
    "- alisio gateway start",
    "- alisio gateway stop",
    "- alisio gateway restart",
    "If unsure, ask the user to run `alisio help` (or `alisio gateway --help`) and paste the output.",
    "",
    ...skillsSection,
    ...memorySection,
    // Skip self-update for subagent/none modes
    hasGateway && !isMinimal ? "## Alisio Self-Update" : "",
    hasGateway && !isMinimal
      ? [
          ...
          "After restart, Alisio pings the last active session automatically.",
        ].join("\n")
      : "",
    hasGateway && !isMinimal ? "" : "",
    "",
  ...
  if (!isMinimal && heartbeatPrompt) {
    lines.push(
      ...
      "HEARTBEAT_OK",
      'Alisio treats a leading/trailing "HEARTBEAT_OK" as a heartbeat ack (and may discard it).',
