...
const buildStatusMessageMock = vi.hoisted(() => vi.fn(() => "Alisio\n🧠 Model: GPT-5.4"));
...
async function loadFreshAlisioToolsForSessionStatusTest() {
  vi.resetModules();
...
describe("session_status tool", () => {
  beforeEach(async () => {
    buildStatusMessageMock.mockClear();
    await loadFreshAlisioToolsForSessionStatusTest();
  });
  ...
  it("returns a status card for the current session", async () => {
    ...
    expect(details.statusText).toContain("Alisio");
