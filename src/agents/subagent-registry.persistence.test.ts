...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["ALISIO_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "alisio-subagent-"));
  ...
  it("uses isolated temp state when ALISIO_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "alisio-test-state"));
  });
});
