...
type ModelsConfig = NonNullable<AlisioConfig["models"]>;
...
function resolveConfigBackedProviderAuth(params: { provider: string; config?: AlisioConfig }):
  | {
      apiKey: string;
      discoveryApiKey?: string;
      mode: "api_key";
...
function resolveXaiConfigFallbackAuth(params: { provider: string; config?: AlisioConfig }):
  | {
      apiKey: string;
      source: string;
      mode: "api-key";
