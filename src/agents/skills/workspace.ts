...
function resolveSkillsLimits(config?: AlisioConfig): ResolvedSkillsLimits {
...
function loadSkillEntries(
  ...
  const skillEntries: SkillEntry[] = Array.from(merged.values()).map((skill) => {
    ...
    const contract = resolveSkillManifestContract({ frontmatter, skill });
    return {
      skill,
      frontmatter,
      metadata: contract.metadata,
      manifest: contract.manifest,
      manifestValidation: contract.validation,
...
function applySkillsPromptLimits(params: { skills: Skill[]; config?: AlisioConfig }): {
...
function resolveWorkspaceSkillPromptState(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}${compact ? " (compact format, descriptions omitted)" : ""}. Run \`alisio skills check\` to audit.`
    : compact
      ? `⚠️ Skills catalog using compact format (descriptions omitted). Run \`alisio skills check\` to audit.`
      : "";
  const prompt = [
    remoteNote,
    truncationNote,
