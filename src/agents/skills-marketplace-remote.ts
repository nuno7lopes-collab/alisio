...
import {
  downloadMarketplaceSkillArchive,
  fetchMarketplaceSkillDetail,
  resolveSkillsMarketplaceBaseUrl,
  searchMarketplaceSkills,
  type SkillsMarketplaceSkillDetail,
  type SkillsMarketplaceSkillSearchResult,
} from "../infra/skills-marketplace-registry.js";

const MARKETPLACE_DOT_DIR = ".alisio-marketplace";
const LEGACY_MARKETPLACE_DOT_DIRS = [".clawhub", ".clawdhub"];
const SKILL_ORIGIN_RELATIVE_PATH = path.join(MARKETPLACE_DOT_DIR, "origin.json");

export type MarketplaceSkillOrigin = {
  version: 1;
  registry: string;
  slug: string;
...
export type MarketplaceSkillsLockfile = {
  version: 1;
  skills: Record<
    string,
    {
...
export type InstallMarketplaceSkillResult =
  | {
      ok: true;
      slug: string;
      version: string;
      targetDir: string;
      detail: SkillsMarketplaceSkillDetail;
    }
  | { ok: false; error: string };

export type UpdateMarketplaceSkillResult =
  | {
      ok: true;
      slug: string;
...
function resolveMarketplaceMetadataPaths(rootDir: string, fileName: string): string[] {
  return [
    path.join(rootDir, MARKETPLACE_DOT_DIR, fileName),
    ...LEGACY_MARKETPLACE_DOT_DIRS.map((dir) => path.join(rootDir, dir, fileName)),
  ];
}
...
async function resolveRequestedUpdateSlug(params: {
  ...
  lock: MarketplaceSkillsLockfile;
}): Promise<string> {
  ...
  const trackedOrigin = await readMarketplaceSkillOrigin(trackedTargetDir);
...
export async function readMarketplaceSkillsLockfile(
  workspaceDir: string,
): Promise<MarketplaceSkillsLockfile> {
  for (const candidate of resolveMarketplaceMetadataPaths(workspaceDir, "lock.json")) {
    try {
      const raw = JSON.parse(
        await fs.readFile(candidate, "utf8"),
      ) as Partial<MarketplaceSkillsLockfile>;
...
export async function writeMarketplaceSkillsLockfile(
  workspaceDir: string,
  lockfile: MarketplaceSkillsLockfile,
): Promise<void> {
  const targetPath = path.join(workspaceDir, MARKETPLACE_DOT_DIR, "lock.json");
  await fs.mkdir(path.dirname(targetPath), { recursive: true });
  await fs.writeFile(targetPath, `${JSON.stringify(lockfile, null, 2)}\n`, "utf8");
}

export async function readMarketplaceSkillOrigin(
  skillDir: string,
): Promise<MarketplaceSkillOrigin | null> {
  for (const candidate of resolveMarketplaceMetadataPaths(skillDir, "origin.json")) {
    try {
      const raw = JSON.parse(
        await fs.readFile(candidate, "utf8"),
      ) as Partial<MarketplaceSkillOrigin>;
      if (
        raw.version === 1 &&
        ...
        return raw as MarketplaceSkillOrigin;
      }
    } catch {
      // try the next path
    }
...
export async function writeMarketplaceSkillOrigin(
  skillDir: string,
...
export async function searchSkillsFromMarketplace(params: {
  query?: string;
  limit?: number;
  baseUrl?: string;
}): Promise<SkillsMarketplaceSkillSearchResult[]> {
  return await searchMarketplaceSkills({
...
async function resolveInstallVersion(params: {
  ...
  version?: string;
  baseUrl?: string;
}): Promise<{ detail: SkillsMarketplaceSkillDetail; version: string }> {
  const detail = await fetchMarketplaceSkillDetail({
    slug: params.slug,
    baseUrl: params.baseUrl,
  });
  if (!detail.skill) {
    throw new Error(`Skill "${params.slug}" not found in the marketplace registry.`);
  }
...
async function performMarketplaceSkillInstall(
  ...
  try {
    ...
    params.logger?.info?.(`Downloading ${params.slug}@${version} from the marketplace registry…`);
    const archive = await downloadMarketplaceSkillArchive({
      slug: params.slug,
      version,
      baseUrl: params.baseUrl,
    });
    try {
      const install = await withExtractedArchiveRoot({
        ...
        tempDirPrefix: "alisio-skill-marketplace-",
        timeoutMs: 120_000,
      ...
      await writeMarketplaceSkillOrigin(install.targetDir, {
        version: 1,
        registry: resolveSkillsMarketplaceBaseUrl(params.baseUrl),
        slug: params.slug,
        installedVersion: version,
        installedAt,
      });
      const lock = await readMarketplaceSkillsLockfile(params.workspaceDir);
      lock.skills[params.slug] = {
        version,
        installedAt,
      };
      await writeMarketplaceSkillsLockfile(params.workspaceDir, lock);
...
async function installRequestedSkillFromMarketplace(
  params: MarketplaceInstallParams,
): Promise<InstallMarketplaceSkillResult> {
  try {
    return await performMarketplaceSkillInstall({
      ...params,
...
async function installTrackedSkillFromMarketplace(
  params: MarketplaceInstallParams,
): Promise<InstallMarketplaceSkillResult> {
  try {
    return await performMarketplaceSkillInstall({
...
async function resolveTrackedUpdateTarget(params: {
  ...
  const targetDir = resolveSkillInstallDir(params.workspaceDir, params.slug);
  const origin = (await readMarketplaceSkillOrigin(targetDir)) ?? null;
  if (!origin && !params.lock.skills[params.slug]) {
    return {
      ok: false,
      slug: params.slug,
      error: `Skill "${params.slug}" is not tracked as a marketplace install.`,
    };
  }
  return {
    ok: true,
...
export async function installMarketplaceRegistrySkill(params: {
  workspaceDir: string;
  slug: string;
  version?: string;
  baseUrl?: string;
  ...
  return await installRequestedSkillFromMarketplace(params);
}

export async function updateMarketplaceSkills(params: {
  workspaceDir: string;
  slug?: string;
  baseUrl?: string;
  logger?: Logger;
}): Promise<UpdateMarketplaceSkillResult[]> {
  const lock = await readMarketplaceSkillsLockfile(params.workspaceDir);
  ...
  const results: UpdateMarketplaceSkillResult[] = [];
  for (const slug of slugs) {
    ...
    const install = await installTrackedSkillFromMarketplace({
      workspaceDir: params.workspaceDir,
      slug: tracked.slug,
...
export async function readTrackedMarketplaceSkillSlugs(workspaceDir: string): Promise<string[]> {
  const lock = await readMarketplaceSkillsLockfile(workspaceDir);
