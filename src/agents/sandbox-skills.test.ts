...
vi.mock("./sandbox/docker.js", () => ({
  ensureSandboxContainer: vi.fn(async () => "alisio-sbx-test"),
}));
...
describe("sandbox skill mirroring", () => {
  ...
  const runContext = async (workspaceAccess: "none" | "ro") => {
    const bundledDir = await fs.mkdtemp(path.join(os.tmpdir(), "alisio-bundled-skills-"));
    await fs.mkdir(bundledDir, { recursive: true });

    process.env.ALISIO_BUNDLED_SKILLS_DIR = bundledDir;
