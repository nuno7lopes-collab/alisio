...
function mockContextDeps(params: {
  ...
  const ensureAlisioModelsJson = vi.fn(async () => {});
  ...
  vi.doMock("./agent-paths.js", () => ({
    resolveAlisioAgentDir: () => "/tmp/alisio-agent",
  }));
...
describe("lookupContextTokens", () => {
  ...
  it("only warms eagerly for real alisio startup commands that need model metadata", async () => {
    const argvSnapshot = process.argv;
    try {
      for (const scenario of [
        {
          argv: ["node", "alisio", "chat"],
          expectedCalls: 1,
        },
        {
          argv: ["node", "alisio", "--profile", "--", "config", "validate"],
          expectedCalls: 0,
        },
        {
          argv: ["node", "alisio", "logs", "--limit", "5"],
          expectedCalls: 0,
        },
        {
        ...
        const { ensureAlisioModelsJson } = mockContextModuleDeps(loadConfigMock);
        process.argv = scenario.argv;
        await import("./context.js");
        await flushAsyncWarmup();
        expect(loadConfigMock).toHaveBeenCalledTimes(scenario.expectedCalls);
        expect(ensureAlisioModelsJson).toHaveBeenCalledTimes(scenario.expectedCalls);
      }
    } finally {
