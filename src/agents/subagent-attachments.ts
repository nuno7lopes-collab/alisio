...
function resolveAttachmentLimits(config: AlisioConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".alisio", "attachments");
  const relDir = path.posix.join(".alisio", "attachments", attachmentId);
