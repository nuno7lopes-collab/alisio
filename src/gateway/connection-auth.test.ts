...
async function expectFailClosedOnUnresolvedLocalAuth(config: AlisioConfig, path: string) {
  await expect(
    resolveGatewayConnectionAuth({
      config,
...
describe("resolveGatewayConnectionAuth", () => {
  ...
  it("resolves local SecretRef token when Alisio env is absent", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first token SecretRef even when Alisio env token exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first password SecretRef even when Alisio env password exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
