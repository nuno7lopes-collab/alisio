 ...
 *
 * Implements the OpenResponses `/v1/responses` endpoint for Alisio Gateway.
 *
...
import { getAgentCommandFromIngress } from "./agent-command-loader.js";
...
function createResponseSessionScope(params: {
  ...
  return normalizeResponseSessionScope({
    ...
    requestedSessionKey: getHeader(params.req, "x-alisio-session-key"),
  });
}
...
async function runResponsesAgentCommand(params: {
  ...
  const agentCommandFromIngress = await getAgentCommandFromIngress();
  ...
  unsubscribe = onAgentEvent((evt) => {
    ...
    if (evt.stream === "lifecycle") {
      ...
      if (phase === "end" || phase === "error") {
        const finalText = accumulatedText || "No response from Alisio.";
