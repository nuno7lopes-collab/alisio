...
export async function withTempConfig(params: {
  ...
  const prevConfigPath = process.env.ALISIO_CONFIG_PATH;

  const dir = await mkdtemp(path.join(os.tmpdir(), params.prefix ?? "alisio-test-config-"));
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.ALISIO_CONFIG_PATH = prevConfigPath;
    }
