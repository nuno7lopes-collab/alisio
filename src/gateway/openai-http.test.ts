    ...
    try {
...

      {
        await expectAgentSessionKeyMatch({
          body: { model: "alisio", messages: [{ role: "user", content: "hi" }] },
          headers: { "x-alisio-agent-id": "beta" },
          matcher: /^agent:beta:/,
        });
      }
...

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "alisio", messages: [{ role: "user", content: "hi" }] },
          {
            "x-alisio-agent-id": "beta",
            "x-alisio-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
...

      {
        ...
        expect(json.error?.message).toBe("Invalid `model`. Use `alisio` or `alisio/<agentId>`.");
        expect(agentCommand).toHaveBeenCalledTimes(0);
      }

      {
        ...
        const json = (await res.json()) as { error?: { type?: string; message?: string } };
        expect(json.error?.type).toBe("invalid_request_error");
        expect(json.error?.message).toBe("Invalid `x-alisio-model`.");
        expect(agentCommand).toHaveBeenCalledTimes(0);
      }
...

      {
        ...
        const choice0 = (json.choices as Array<Record<string, unknown>>)[0] ?? {};
        const msg = (choice0.message as Record<string, unknown> | undefined) ?? {};
        expect(msg.content).toBe("No response from Alisio.");
      }
