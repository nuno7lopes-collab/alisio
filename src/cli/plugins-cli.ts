...
function formatPluginLine(plugin: PluginRecord, verbose = false): string {
  ...
  const format = plugin.format ?? "alisio";
...
export function registerPluginsCli(program: Command) {
  const plugins = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/plugins", "docs.alisio.ai/cli/plugins")}\n`,
    );
  ...
  plugins
    ...
    .action((id: string | undefined, opts: PluginInspectOptions) => {
      ...
      lines.push(`${theme.muted("Format:")} ${inspect.plugin.format ?? "alisio"}`);
  ...
  plugins
    ...
    .action(async (id: string) => {
      ...
      let next: AlisioConfig = enableResult.config;
  ...
  plugins
    .command("install")
    .description(
      "Install a plugin or hook pack (path, archive, npm spec, marketplace:package, or marketplace entry)",
    )
    .argument(
  ...
  plugins
    ...
    .action(() => {
      ...
      const docs = formatDocsLink("/plugin", "docs.alisio.ai/plugin");
      lines.push("");
