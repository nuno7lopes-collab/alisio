...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "alisio-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "alisio-bundled-skills-test-"));
    process.env.ALISIO_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
