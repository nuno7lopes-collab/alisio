    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["alisio channels list", "List configured channels and auth profiles."],
          ["alisio channels status --probe", "Run channel status checks and probes."],
          [
            "alisio channels add --channel telegram --token <token>",
            "Add or update a channel account non-interactively.",
          ],
          ["alisio channels login --channel whatsapp", "Link a WhatsApp Web account."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink(
          "/cli/channels",
          "docs.alisio.ai/cli/channels",
        )}\n`,
    );
