...
${formatHelpExamples([
  ['alisio agent --to +15555550123 --message "status update"', "Start a new session."],
  ['alisio agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'alisio agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'alisio agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['alisio agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'alisio agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.alisio.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.alisio.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['alisio agents set-identity --agent main --name "Alisio" --emoji "🦞"', "Set name + emoji."],
  ["alisio agents set-identity --agent main --avatar avatars/alisio.png", "Set avatar path."],
  [
    "alisio agents set-identity --workspace ~/.alisio/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "alisio agents set-identity --identity-file ~/.alisio/workspace/IDENTITY.md --agent main",
