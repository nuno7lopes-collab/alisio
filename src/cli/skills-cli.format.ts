...
function appendMarketplaceHint(output: string, json?: boolean): string {
  if (json) {
    return output;
  }
  return `${output}\n\nTip: use \`alisio skills search\`, \`alisio skills install\`, and \`alisio skills update\` for Local Marketplace skills.`;
}
...
export function formatSkillsList(report: SkillStatusReport, opts: SkillsListOptions): string {
  ...
  if (skills.length === 0) {
    const message = opts.eligible
      ? `No eligible skills found. Run \`${formatCliCommand("alisio skills list")}\` to see all skills.`
      : "No skills found.";
    return appendMarketplaceHint(message, opts.json);
  }
  ...
  return appendMarketplaceHint(lines.join("\n"), opts.json);
}
...
export function formatSkillInfo(
  ...
  if (!skill) {
    ...
    return appendMarketplaceHint(
      `Skill "${skillName}" not found. Run \`${formatCliCommand("alisio skills list")}\` to see available skills.`,
      opts.json,
    );
  }
  ...
  if (skill.primaryEnv && skill.missing.env.includes(skill.primaryEnv)) {
    ...
    lines.push(
      `  Save via CLI: ${formatCliCommand(`alisio config set skills.entries.${safeSkillKey}.apiKey YOUR_KEY`)}`,
    );
    lines.push(
      `  Stored in: ${theme.muted("~/.alisio/alisio.json")} ${theme.muted(`(skills.entries.${safeSkillKey}.apiKey)`)}`,
    );
  }
