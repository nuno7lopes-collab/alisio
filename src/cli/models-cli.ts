...
export function registerModelsCli(program: Command) {
  const models = program
    ...
    .option("--agent <id>", "Agent id to inspect (overrides ALISIO_AGENT_DIR/PI_CODING_AGENT_DIR)")
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/models", "docs.alisio.ai/cli/models")}\n`,
    );
  ...
  auth
    ...
    .option(
      "--model <id>",
      "Set a specific default model for this provider (requires --set-default)",
    )
    .action(async (opts) => {
      await runModelsCommand(async () => {
