...
function collectDryRunSchemaErrors(config: AlisioConfig): ConfigSetDryRunError[] {
...
export async function runConfigSet(opts: {
  ...
  try {
    ...
    const nextConfig = next as AlisioConfig;
...
export async function runConfigValidate(opts: { json?: boolean; runtime?: RuntimeEnv } = {}) {
  const runtime = opts.runtime ?? defaultRuntime;
  let outputPath = CONFIG_PATH ?? "alisio.json";
...
export function registerConfigCli(program: Command) {
  ...
  cmd
    .command("schema")
    .description("Print the JSON schema for alisio.json")
    .action(async () => {
