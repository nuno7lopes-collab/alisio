...
const SANDBOX_EXAMPLES = {
  main: [
    ["alisio sandbox list", "List all sandbox containers."],
    ["alisio sandbox list --browser", "List only browser containers."],
    ["alisio sandbox recreate --all", "Recreate all containers."],
    ["alisio sandbox recreate --session main", "Recreate a specific session."],
    ["alisio sandbox recreate --agent mybot", "Recreate agent containers."],
    ["alisio sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["alisio sandbox list", "List all sandbox containers."],
    ["alisio sandbox list --browser", "List only browser containers."],
  ...
  recreate: [
    ["alisio sandbox recreate --all", "Recreate all containers."],
    ["alisio sandbox recreate --session main", "Recreate a specific session."],
    ["alisio sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["alisio sandbox recreate --browser --all", "Recreate only browser containers."],
    ["alisio sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["alisio sandbox explain", "Show effective sandbox config."],
    ["alisio sandbox explain --session agent:main:main", "Explain a specific session."],
    ["alisio sandbox explain --agent work", "Explain an agent sandbox."],
    ["alisio sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.alisio.ai/cli/sandbox")}\n`,
    )
    .action(() => {
