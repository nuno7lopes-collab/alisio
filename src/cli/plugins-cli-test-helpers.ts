...
export const loadConfig = vi.fn<() => AlisioConfig>(() => ({}) as AlisioConfig);
export const readConfigFileSnapshot = vi.fn();
export const writeConfigFile = vi.fn<(config: AlisioConfig) => Promise<void>>(
  async () => undefined,
);
export const replaceConfigFile = vi.fn(
  async (params: { nextConfig: AlisioConfig }) => await writeConfigFile(params.nextConfig),
);
export const resolveStateDir = vi.fn(() => "/tmp/alisio-state");
...
vi.mock("../config/config.js", () => ({
  ...
  writeConfigFile: (config: AlisioConfig) => writeConfigFile(config),
  replaceConfigFile: (params: { nextConfig: AlisioConfig }) => replaceConfigFile(params),
}));
...
export function resetPluginsCliTestState() {
  ...
  loadConfig.mockReturnValue({} as AlisioConfig);
  ...
  resolveStateDir.mockReturnValue("/tmp/alisio-state");
  resolveMarketplaceInstallShortcut.mockResolvedValue(null);
  installPluginFromMarketplace.mockResolvedValue({
    ok: false,
    error: "marketplace install failed",
  });
  enablePluginInConfig.mockImplementation((cfg: AlisioConfig) => ({ config: cfg }));
  recordPluginInstall.mockImplementation((cfg: AlisioConfig) => cfg);
  buildPluginStatusReport.mockReturnValue({
    plugins: [],
    diagnostics: [],
  });
  applyExclusiveSlotSelection.mockImplementation(({ config }: { config: AlisioConfig }) => ({
    config,
    warnings: [],
  }));
  ...
  recordHookInstall.mockImplementation((cfg: AlisioConfig) => cfg);
}
