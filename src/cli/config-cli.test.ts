...
import type { ConfigFileSnapshot, AlisioConfig } from "../config/types.js";
...
const mockWriteConfigFile = vi.fn<
  (cfg: AlisioConfig, options?: { unsetPaths?: string[][] }) => Promise<void>
>(async () => {});
...
vi.mock("../config/config.js", () => ({
  ...
  writeConfigFile: (cfg: AlisioConfig, options?: { unsetPaths?: string[][] }) =>
...
function setSnapshot(resolved: AlisioConfig, config: AlisioConfig) {
...
function withRuntimeDefaults(resolved: AlisioConfig): AlisioConfig {
  return {
    ...resolved,
    agents: {
      ...resolved.agents,
...
describe("config cli", () => {
  ...
  describe("config set - issue #6070", () => {
    it("preserves existing config keys when setting a new value", async () => {
      ...
      const runtimeMerged: AlisioConfig = {
  ...
  describe("config get", () => {
    it("redacts sensitive values", async () => {
      ...
      expect(mockLog).toHaveBeenCalledWith("__ALISIO_REDACTED__");
    });
  });
