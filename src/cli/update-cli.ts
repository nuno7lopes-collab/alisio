...
export function registerUpdateCli(program: Command) {
  ...
  const update = program
    .command("update")
    .description("Update Alisio and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["alisio update", "Update a source checkout (git)"],
        ["alisio update --channel beta", "Switch to beta channel (git + npm)"],
        ["alisio update --channel dev", "Switch to dev channel (git + npm)"],
        ["alisio update --tag beta", "One-off update to a dist-tag or version"],
        ["alisio update --tag main", "One-off package install from GitHub main"],
        ["alisio update --dry-run", "Preview actions without changing anything"],
        ["alisio update --no-restart", "Update without restarting the service"],
        ["alisio update --json", "Output result as JSON"],
        ["alisio update --yes", "Non-interactive (accept downgrade prompts)"],
        ["alisio update wizard", "Interactive update wizard"],
        ["alisio --update", "Shorthand for alisio update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run alisio update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.alisio.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["alisio update status", "Show channel + version status."],
          ["alisio update status --json", "JSON output."],
          ["alisio update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.alisio.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
