...
export type PromptDefaultModelResult = { model?: string; config?: AlisioConfig };
...
function resolveConfiguredModelRaw(cfg: AlisioConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: AlisioConfig): string[] {
...
export function applyModelAllowlist(cfg: AlisioConfig, models: string[]): AlisioConfig {
