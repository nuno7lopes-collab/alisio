...
import { noteAuthProfileHealth } from "../commands/doctor-auth.js";
...
import { noteOpenAIOAuthTlsPrerequisites } from "../plugins/provider-openai-codex-oauth-tls.js";
...
export type DoctorHealthFlowContext = {
  ...
  cfg: AlisioConfig;
  cfgForPersistence: AlisioConfig;
  sourceConfigValid: boolean;
  configPath: string;
...
export function resolveDoctorMode(cfg: AlisioConfig): DoctorFlowMode {
...
async function runGatewayConfigHealth(ctx: DoctorHealthFlowContext): Promise<void> {
  if (!ctx.cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("alisio configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("alisio config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(ctx.configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("alisio setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  if (resolveDoctorMode(ctx.cfg) === "local" && hasAmbiguousGatewayAuthModeConfig(ctx.cfg)) {
    note(
      [
        ...
        `Set token mode: ${formatCliCommand("alisio config set gateway.auth.mode token")}`,
        `Set password mode: ${formatCliCommand("alisio config set gateway.auth.mode password")}`,
      ].join("\n"),
      "Gateway auth",
    );
  }
...
async function runWriteConfigHealth(ctx: DoctorHealthFlowContext): Promise<void> {
  ...
  if (!ctx.prompter.shouldRepair) {
    ctx.runtime.log(`Run "${formatCliCommand("alisio doctor --fix")}" to apply changes.`);
  }
}
...
async function runWorkspaceSuggestionsHealth(ctx: DoctorHealthFlowContext): Promise<void> {
  ...
  if (await shouldSuggestMemorySystem({ workspaceDir, cfg: ctx.cfg })) {
