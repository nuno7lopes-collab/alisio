...
function cfg(accounts?: Record<string, unknown> | null, defaultAccount?: string): AlisioConfig {
  if (accounts === null) {
    return {
      channels: {
  ...
  if (accounts === undefined && !defaultAccount) {
    return {} as unknown as AlisioConfig;
  }
  return {
    channels: {
      testchannel: {
...
function expectResolvedAccountIdsCase(params: {
  resolve: (cfg: AlisioConfig) => string[];
  input: AlisioConfig;
  expected: string[];
}) {
...
function expectResolvedDefaultAccountCase(input: AlisioConfig, expected: string) {
