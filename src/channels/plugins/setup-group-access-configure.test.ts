...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: AlisioConfig, policy: ChannelAccessPolicy) => AlisioConfig;
  resolveAllowlist: (params: { cfg: AlisioConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: AlisioConfig; resolved: TResolved }) => AlisioConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: AlisioConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: AlisioConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: AlisioConfig, policy: ChannelAccessPolicy): AlisioConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: AlisioConfig, policy: ChannelAccessPolicy): AlisioConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: AlisioConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: AlisioConfig; resolved: string[] }) => {
      calls.push("apply");
