...
export async function promptGatewayConfig(
    ...
    if (tokenInputMode === "ref") {
      const envVar = guardCancel(
        await text({
          ...
          initialValue: "ALISIO_GATEWAY_TOKEN",
          placeholder: "ALISIO_GATEWAY_TOKEN",
          validate: (value) => {
            const candidate = String(value ?? "").trim();
            if (!isValidEnvSecretRefId(candidate)) {
              return "Use an env var name like ALISIO_GATEWAY_TOKEN.";
            }
      ...
      note(`Validated ${envVarName}. Alisio will store a token SecretRef.`, "Gateway token");
    } else {
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        "Trusted proxy mode: Alisio trusts user identity from a reverse proxy.",
        ...
        "Docs: https://docs.alisio.pt/gateway/trusted-proxy-auth",
      ].join("\n"),
      "Trusted Proxy Auth",
    );
