...
export async function loadValidConfigOrThrow(): Promise<AlisioConfig> {
...
export async function updateConfig(
  mutator: (cfg: AlisioConfig) => AlisioConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: AlisioConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: AlisioConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("alisio agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
