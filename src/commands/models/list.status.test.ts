...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveAlisioAgentDir: vi.fn().mockReturnValue("/tmp/alisio-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/alisio-agent"),
    ...
    createConfigIO: vi.fn().mockReturnValue({
      configPath: "/tmp/alisio-dev/alisio.json",
    }),
...
async function loadFreshModelsStatusCommandModuleForTest() {
  ...
  vi.doMock("../../agents/agent-paths.js", () => ({
    resolveAlisioAgentDir: mocks.resolveAlisioAgentDir,
  }));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/alisio-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  ...
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveAlisioAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.configPath).toBe("/tmp/alisio-dev/alisio.json");
    expect(payload.auth.storePath).toBe("/tmp/alisio-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/alisio-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/alisio-agent-custom");
