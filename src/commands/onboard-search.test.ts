...
function createPerplexityConfig(apiKey: string, enabled?: boolean): AlisioConfig {
  return {
    tools: {
      web: {
        search: {
...
function pluginWebSearchApiKey(config: AlisioConfig, pluginId: string): unknown {
  const entry = (
...
function createDisabledFirecrawlConfig(apiKey?: string): AlisioConfig {
  return {
    tools: {
      web: {
        search: {
...
function readFirecrawlPluginApiKey(config: AlisioConfig): string | undefined {
...
async function runQuickstartPerplexitySetup(
  apiKey: string,
  enabled?: boolean,
): Promise<{ result: AlisioConfig; prompter: WizardPrompter }> {
  const cfg = createPerplexityConfig(apiKey, enabled);
