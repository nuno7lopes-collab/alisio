...
describe("readServiceStatusSummary", () => {
  it("marks Alisio-managed services as installed", async () => {
    const summary = await readServiceStatusSummary(
      createService({
        isLoaded: vi.fn(async () => true),
        readCommand: vi.fn(async () => ({ programArguments: ["alisio", "gateway", "run"] })),
    ...
    expect(summary.managedByAlisio).toBe(true);
  ...
  it("marks running unmanaged services as externally managed", async () => {
    ...
    expect(summary.installed).toBe(true);
    expect(summary.managedByAlisio).toBe(false);
  ...
  it("passes command environment to runtime and loaded checks", async () => {
    const isLoaded = vi.fn(async ({ env }: GatewayServiceEnvArgs) => {
      return env?.ALISIO_GATEWAY_PORT === "40705";
    });
    const readRuntime = vi.fn(async (env?: NodeJS.ProcessEnv) => ({
      status: env?.ALISIO_GATEWAY_PORT === "40705" ? ("running" as const) : ("unknown" as const),
    }));
    ...
    const summary = await readServiceStatusSummary(
      createService({
        ...
        readCommand: vi.fn(async () => ({
          ...
          environment: { ALISIO_GATEWAY_PORT: "40705" },
        })),
        readRuntime,
      }),
      "Daemon",
