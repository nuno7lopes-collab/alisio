...
const SEARCH_TOOL = "https://docs.alisio.pt/mcp.SearchAlisio";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.alisio.pt");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('alisio docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.alisio.pt/");
      runtime.log(`Search: ${formatCliCommand('alisio docs "your query"')}`);
    }
    return;
  }
