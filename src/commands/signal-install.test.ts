...
import type { ReleaseAsset } from "../plugins/signal-cli-install.js";
import {
  extractSignalCliArchive,
  looksLikeArchive,
  pickAsset,
} from "../plugins/signal-cli-install.js";
...
describe("extractSignalCliArchive", () => {
  async function withArchiveWorkspace(run: (workDir: string) => Promise<void>) {
    const workDir = await fs.mkdtemp(path.join(os.tmpdir(), "alisio-signal-install-"));
    try {
      await run(workDir);
    } finally {
