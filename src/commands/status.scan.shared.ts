...
export function hasExplicitMemorySearchConfig(cfg: AlisioConfig, agentId: string): boolean {
  if (
    cfg.agents?.defaults &&
...
export function resolveMemoryPluginStatus(cfg: AlisioConfig): MemoryPluginStatus {
...
export async function resolveSharedMemoryStatusSnapshot(params: {
  ...
  resolveMemoryConfig: (cfg: AlisioConfig, agentId: string) => { store: { path: string } } | null;
