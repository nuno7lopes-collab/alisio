    ...
    createTestRegistry([
      {
        ...
        plugin: {
          ...
          setup: {
            applyAccountConfig: ({ cfg }: { cfg: AlisioConfig }) => cfg,
          },
          setupWizard: {
            channel: "telegram",
            status: {
...
describe("setupChannels post-write hooks", () => {
  ...
  it("collects onboarding post-write hooks and runs them against the final config", async () => {
    ...
    const restore = patchChannelOnboardingAdapterForTest({
      configureInteractive,
      afterConfigWritten,
      getStatus: vi.fn(async ({ cfg }: { cfg: AlisioConfig }) => ({
        channel: "telegram",
    ...
    try {
      const cfg = await setupChannels({} as AlisioConfig, runtime, prompter, {
