...
import { readConfigFileSnapshot, replaceConfigFile, type AlisioConfig } from "../config/config.js";
...
function formatAmbiguousGatewayAuthModeReason(): string {
  return [
    ...
    `Set ${formatCliCommand("alisio config set gateway.auth.mode token")} or ${formatCliCommand("alisio config set gateway.auth.mode password")}.`,
  ].join(" ");
}
