...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: AlisioConfig): string[] {
...
export async function noteSecurityWarnings(cfg: AlisioConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("alisio security audit --deep")}`;
  ...
  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      ...
      `  Check local policy with: ${formatCliCommand("alisio approvals get --gateway")}`,
    );
  }
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("alisio configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("alisio config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("alisio doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("alisio config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('alisio config set session.dmScope "per-channel-peer"') +
