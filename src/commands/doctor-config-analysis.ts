...
import { AlisioSchema } from "../config/zod-schema.js";
...
export function stripUnknownConfigKeys(config: AlisioConfig): {
  config: AlisioConfig;
  removed: string[];
} {
  const parsed = AlisioSchema.safeParse(config);
  if (parsed.success) {
...
export function noteOpencodeProviderOverrides(cfg: AlisioConfig): void {
