...
export async function statusCommand(
  ...
  const daemonValue = (() => {
    ...
    const installedPrefix = daemon.managedByAlisio ? "installed · " : "";
  ...
  const nodeDaemonValue = (() => {
    ...
    const installedPrefix = nodeDaemon.managedByAlisio ? "installed · " : "";
  ...
  runtime.log(theme.heading("Alisio status"));
  runtime.log("");
  ...
  if (summary.taskAudit.errors > 0) {
    runtime.log("");
    runtime.log(
      theme.muted(`Task maintenance: ${formatCliCommand("alisio tasks maintenance --apply")}`),
    );
  }
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`alisio devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("alisio devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("alisio devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("alisio security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("alisio security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.alisio.pt/faq");
  runtime.log("Troubleshooting: https://docs.alisio.pt/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("alisio status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("alisio logs --follow")}`);
  if (nodeOnlyGateway) {
    runtime.log(`  Need node service?  ${formatCliCommand("alisio node status")}`);
  } else if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("alisio status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("alisio gateway probe")}`);
  }
}
