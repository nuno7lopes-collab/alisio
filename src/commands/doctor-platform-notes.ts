...
function hasConfigGatewayCreds(cfg: AlisioConfig): boolean {
...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  const lines = [
    ...
    envToken && envTokenKey
      ...
      : undefined,
    envPassword
      ? `- \`${envPasswordKey ?? "ALISIO_GATEWAY_PASSWORD"}\` is set; it overrides config passwords.`
      : undefined,
...
export function noteStartupOptimizationHints(
  ...
  if (noRespawn !== "1") {
    lines.push(
      "- ALISIO_NO_RESPAWN is not set to 1; set it to avoid extra startup overhead from self-respawn.",
    );
  }
