...
const TEST_ARCHIVE_ROOT = "2026-03-09T00-00-00.000Z-alisio-backup";
...
describe("backupVerifyCommand", () => {
  ...
  async function resetTempHome() {
    ...
    await fs.mkdir(path.join(tempHome.home, ".alisio"), { recursive: true });
    delete process.env.ALISIO_CONFIG_PATH;
  }

  beforeAll(async () => {
    tempHome = await createTempHomeEnv("alisio-backup-verify-test-");
  });
  ...
  it("verifies an archive created by backup create", async () => {
    const stateDir = path.join(tempHome.home, ".alisio");
    const archiveDir = await fs.mkdtemp(path.join(os.tmpdir(), "alisio-backup-verify-out-"));
    try {
      await fs.writeFile(path.join(stateDir, "alisio.json"), JSON.stringify({}), "utf8");
  ...
  it("fails when the archive does not contain a manifest", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "alisio-backup-no-manifest-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
    try {
  ...
  it("fails when the manifest references a missing asset payload", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "alisio-backup-missing-asset-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
    try {
      const rootName = "2026-03-09T00-00-00.000Z-alisio-backup";
        ...
        assets: [
          {
            kind: "state",
            sourcePath: "/tmp/.alisio",
            archivePath: `${rootName}/payload/posix/tmp/.alisio`,
          },
        ],
      };
      await fs.writeFile(
  ...
  it("fails when archive paths contain traversal segments", async () => {
    ...
    await withBrokenArchiveFixture(
      {
        tempPrefix: "alisio-backup-traversal-",
  ...
  it("fails when archive paths contain backslashes", async () => {
    ...
    await withBrokenArchiveFixture(
      {
        tempPrefix: "alisio-backup-backslash-",
  ...
  it("ignores payload manifest.json files when locating the backup manifest", async () => {
    const stateDir = path.join(tempHome.home, ".alisio");
    const externalWorkspace = await fs.mkdtemp(path.join(os.tmpdir(), "alisio-workspace-"));
  ...
  it("fails when the archive contains duplicate root manifest entries", async () => {
    const payloadArchivePath = `${TEST_ARCHIVE_ROOT}/payload/posix/tmp/.alisio/payload.txt`;
    await withBrokenArchiveFixture(
      {
        tempPrefix: "alisio-backup-duplicate-manifest-",
  ...
  it("fails when the archive contains duplicate payload entries", async () => {
    ...
    await withBrokenArchiveFixture(
      {
        tempPrefix: "alisio-backup-duplicate-payload-",
        manifestAssetArchivePath: payloadArchivePath,
        payloads: [
