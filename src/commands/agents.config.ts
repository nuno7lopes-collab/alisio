...
function resolveAgentName(cfg: AlisioConfig, agentId: string) {
...
function resolveAgentModel(cfg: AlisioConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: AlisioConfig): AgentSummary[] {
