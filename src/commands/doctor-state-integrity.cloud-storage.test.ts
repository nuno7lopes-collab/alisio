...
describe("detectMacCloudSyncedStateDir", () => {
  ...
  it("detects state dir under Library/CloudStorage", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "Dropbox", "Alisio", ".alisio");
  ...
  it("detects cloud-synced target when state dir resolves via symlink", () => {
    const symlinkPath = "/tmp/alisio-state";
  ...
  it("ignores cloud-synced symlink prefix when resolved target is local", () => {
    ...
    const resolvedLocalPath = path.join(home, ".alisio");
  ...
  it("anchors cloud detection to OS homedir when ALISIO_HOME is overridden", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "iCloud Drive", ".alisio");
    const originalAlisioHome = process.env.ALISIO_HOME;
    process.env.ALISIO_HOME = "/tmp/alisio-home-override";
    ...
    try {
      ...
      if (originalAlisioHome === undefined) {
