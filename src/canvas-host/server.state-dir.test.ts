...
describe("canvas host state dir defaults", () => {
  ...
  it("uses ALISIO_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("alisio-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("Alisio Canvas");
      } finally {
        await handler.close();
      }
    });
