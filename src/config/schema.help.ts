...
export const FIELD_HELP: Record<string, string> = {
  meta: "Metadata fields automatically maintained by Alisio to record write/version history for this config file. Keep these values system-managed and avoid manual edits unless debugging migration history.",
  "meta.lastTouchedVersion": "Auto-set when Alisio writes the config.",
  ...
  "env.vars":
    "Explicit key/value environment variable overrides merged into runtime process environment for Alisio. Use this for deterministic env configuration instead of relying only on shell profile side effects.",
  wizard:
  ...
  "wizard.lastRunVersion":
    "Alisio version recorded at the time of the most recent wizard run on this config. Use this when diagnosing behavior differences across version-to-version setup changes.",
  ...
  update:
    "Update-channel and startup-check behavior for keeping Alisio runtime versions current. Use conservative channels in production and more experimental channels only in controlled environments.",
  ...
  "talk.voiceAliases":
    'Use this legacy ElevenLabs voice alias map (for example {"Alisio":"EXAVITQu4vr4xnSDxMaL"}) only during migration. Prefer talk.providers.elevenlabs.voiceAliases.',
  "talk.modelId":
  ...
  "agents.list[].runtime":
    "Optional runtime descriptor for this agent. Use embedded for default Alisio execution or acp for external ACP harness defaults.",
  "agents.list[].runtime.type":
    'Runtime type for this agent: "embedded" (default Alisio runtime) or "acp" (ACP harness defaults).',
  ...
  "agents.list[].runtime.acp.agent":
    "Optional ACP harness agent id to use for this Alisio agent (for example codex, claude, cursor, gemini, alisio).",
  ...
  "browser.profiles.*.driver":
    'Per-profile browser driver mode. Use "alisio" for CDP-based profiles, or use "existing-session" for host-local Chrome DevTools MCP attachment.',
  ...
  "discovery.wideArea.domain":
    "Optional unicast DNS-SD domain for wide-area discovery, such as alisio.internal. Use this when you intentionally publish gateway discovery beyond local mDNS scopes.",
  "discovery.mdns":
  ...
  "agents.defaults.sandbox.browser.network":
    "Docker network for sandbox browser containers (default: alisio-sandbox-browser). Avoid bridge if you need stricter isolation.",
  ...
  "gateway.controlUi.basePath":
    "Optional URL prefix where the Control UI is served (e.g. /alisio).",
  ...
  "gateway.nodes.allowCommands":
    "Extra node.invoke commands to allow beyond the gateway defaults (array of command strings). Enabling dangerous commands here is a security-sensitive override and is flagged by `alisio security audit`.",
  ...
  "nodeHost.browserProxy.allowProfiles":
    "Optional allowlist of browser profile names exposed through node proxy routing. Leave empty to preserve the default full profile surface, including profile create/delete routes. When set, Alisio enforces least-privilege profile access and blocks persistent profile create/delete through the proxy.",
  media:
  ...
  "diagnostics.cacheTrace.filePath":
    "JSONL output path for cache trace logs (default: $ALISIO_STATE_DIR/logs/cache-trace.jsonl).",
  ...
  "agents.defaults.memorySearch.sources":
    'Use this to choose which sources are indexed: "memory" reads MEMORY.md + memory files, and "sessions" includes transcript history. Keep ["memory"] unless you need recall from prior chat transcripts; transcript recall remains query-driven instead of being injected into every new turn.',
  ...
  "agents.defaults.memorySearch.remote.apiKey":
    "Supplies a dedicated API key for remote embedding calls used by memory indexing and query-time embeddings. Use this when memory embeddings should use different credentials than global defaults or environment variables. Codex/OpenAI OAuth does not provide embeddings credentials.",
  ...
  "memory.sync.mode": 'Selects the sync transport mode: "off" (default), "cloud", or "direct".',
  ...
  "memory.sync.ui.enabled":
    "Enable the memory sync setup UI entrypoint (default: true). Set false when you want to hide sync setup surfaces while keeping backend support available.",
  ...
  "memory.crdt.pages.enabled":
    "Stores canonical page bodies as CRDT/Yjs state in the derived store and emits CRDT events for edits. Disable only for incident mitigation.",
  "memory.jobs":
  ...
  "agents.defaults.compaction.postCompactionSections":
    'AGENTS.md H2/H3 section names re-injected after compaction so the agent reruns critical startup guidance. Leave unset to use "Session Startup"/"Red Lines"; set to [] to disable reinjection entirely.',
  ...
  "agents.defaults.embeddedPi":
    "Embedded Pi runner hardening controls for how workspace-local Pi settings are trusted and applied in Alisio sessions.",
  ...
  "commands.mcp":
    "Allow /mcp chat command to manage Alisio MCP server config under mcp.servers (default: false).",
  "commands.plugins":
  ...
  mcp: "Global MCP server definitions managed by Alisio. Embedded Pi and other runtime adapters can consume these servers without storing them inside Pi-owned project settings.",
  "mcp.servers":
    "Named MCP server definitions. Alisio stores them in its own config and runtime adapters decide which transports are supported at execution time.",
  session:
  ...
  "session.dmScope":
    'DM session scoping defaults to "per-channel-peer" so each channel+sender pair keeps separate short-term context. Use "main" only when you intentionally want one shared DM session, or choose the other isolated modes for cross-channel or multi-account deployments.',
  ...
  "session.parentForkMaxTokens":
    "Maximum parent-session token count allowed for thread/session inheritance forking. If the parent exceeds this, Alisio starts a fresh thread session instead of forking; set 0 to disable this protection.",
  "session.mainKey":
  ...
  hooks:
    "Inbound webhook automation surface for mapping external events into wake or agent actions in Alisio. Keep this locked down with explicit token/session/agent controls before exposing it beyond trusted networks.",
  "hooks.enabled":
