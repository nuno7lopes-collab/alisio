...
import { buildWebSearchProviderConfig, withTempHome, writeAlisioConfig } from "./test-helpers.js";
import { AlisioSchema } from "./zod-schema.js";

describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = AlisioSchema.safeParse({
      $schema: "https://alisio.ai/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://alisio.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = AlisioSchema.safeParse({ $schema: 123 });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = AlisioSchema.safeParse({
      cron: {
        enabled: true,
...
describe("config strict validation", () => {
  ...
  it("does not mark resolved-only gateway.bind aliases as auto-migratable legacy", async () => {
    await withTempHome(async (home) => {
      await writeAlisioConfig(home, {
        gateway: { bind: "${ALISIO_BIND}" },
      });

      const prev = process.env.ALISIO_BIND;
      process.env.ALISIO_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        ...
        if (prev === undefined) {
          delete process.env.ALISIO_BIND;
        } else {
          process.env.ALISIO_BIND = prev;
        }
      }
    });
  });
