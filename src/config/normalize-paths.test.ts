  ...
  it("expands tilde for path-ish keys only", async () => {
    await withTempHome(async (home) => {
      const cfg = normalizeConfigPaths({
        ...
        logging: { file: "~/.alisio/logs/alisio.log" },
        hooks: {
          path: "~/.alisio/hooks.json5",
      ...
      expect(cfg.logging?.file).toBe(path.join(home, ".alisio", "logs", "alisio.log"));
      expect(cfg.hooks?.path).toBe(path.join(home, ".alisio", "hooks.json5"));
      ...
      expect(cfg.channels?.telegram?.accounts?.personal?.tokenFile).toBe(
        path.join(home, ".alisio", "telegram.token"),
      );
