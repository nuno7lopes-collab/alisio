...
import type { ConfigFileSnapshot } from "./types.alisio.js";
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/alisio/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/alisio/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/alisio/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/alisio/nickserv-password.txt");
