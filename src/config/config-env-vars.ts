...
function collectConfigEnvVarsByTarget(cfg?: AlisioConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: AlisioConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: AlisioConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}
...
export function applyConfigEnvVars(cfg: AlisioConfig, env: NodeJS.ProcessEnv = process.env): void {
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.ALISIO_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
