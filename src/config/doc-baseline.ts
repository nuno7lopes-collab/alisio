...
function resolveRepoRoot(): string {
  const fromPackage = resolveAlisioPackageRootSync({
...
async function loadBundledConfigSchemaResponse(): Promise<ConfigSchemaResponse> {
  ...
  const env = {
    ...process.env,
    HOME: os.tmpdir(),
    ALISIO_STATE_DIR: path.join(os.tmpdir(), "alisio-config-doc-baseline-state"),
    ALISIO_BUNDLED_PLUGINS_DIR: path.join(repoRoot, "extensions"),
  };
