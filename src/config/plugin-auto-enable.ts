...
function collectModelRefs(cfg: AlisioConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: AlisioConfig, providerId: string): boolean {
...
function hasPluginOwnedWebSearchConfig(cfg: AlisioConfig, pluginId: string): boolean {
...
function hasPluginOwnedToolConfig(cfg: AlisioConfig, pluginId: string): boolean {
  if (pluginId === "xai") {
...
function parseExternalCatalogChannelEntries(raw: unknown): ExternalCatalogChannelEntry[] {
  ...
  for (const entry of list) {
    ...
    const channel = manifestBlock.channel;
...
function collectCandidateChannelIds(cfg: AlisioConfig, env: NodeJS.ProcessEnv): string[] {
  return listPotentialConfiguredChannelIds(cfg, env).map(
    (channelId) => normalizeChatChannelId(channelId) ?? channelId,
  );
}

function hasConfiguredWebSearchPluginEntry(cfg: AlisioConfig): boolean {
...
function configMayNeedPluginManifestRegistry(cfg: AlisioConfig): boolean {
...
function configMayNeedPluginAutoEnable(cfg: AlisioConfig, env: NodeJS.ProcessEnv): boolean {
...
function isPluginExplicitlyDisabled(cfg: AlisioConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: AlisioConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: AlisioConfig, pluginId: string): AlisioConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
