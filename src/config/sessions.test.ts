...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "alisio-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ ALISIO_STATE_DIR: stateDir }, fn);
  ...
  it("derives session transcripts dir from ALISIO_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { ALISIO_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when ALISIO_STATE_DIR differs from stored paths", () => {
