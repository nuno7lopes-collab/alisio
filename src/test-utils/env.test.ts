...
describe("env test utils", () => {
  it("captureEnv restores mutated keys", () => {
    const keyA = "ALISIO_ENV_TEST_A";
    const keyB = "ALISIO_ENV_TEST_B";
  ...
  it("captureFullEnv restores added keys and baseline values", () => {
    const key = "ALISIO_ENV_TEST_ADDED";
  ...
  it("withEnv applies values only inside callback", () => {
    const key = "ALISIO_ENV_TEST_SYNC";
  ...
  it("withEnv restores values when callback throws", () => {
    const key = "ALISIO_ENV_TEST_SYNC_THROW";
    const prev = process.env[key];
  ...
  it("withEnv can delete a key only inside callback", () => {
    const key = "ALISIO_ENV_TEST_SYNC_DELETE";
    const prev = process.env[key];
  ...
  it("withEnvAsync restores values when callback throws", async () => {
    const key = "ALISIO_ENV_TEST_ASYNC";
    const prev = process.env[key];
  ...
  it("withEnvAsync applies values only inside async callback", async () => {
    const key = "ALISIO_ENV_TEST_ASYNC_OK";
    const prev = process.env[key];
  ...
  it("withEnvAsync can delete a key only inside callback", async () => {
    const key = "ALISIO_ENV_TEST_ASYNC_DELETE";
    const prev = process.env[key];
    process.env[key] = "outer";
  ...
  it("createPathResolutionEnv clears leaked path overrides before applying explicit ones", () => {
    ...
    const previousBundledDir = process.env.ALISIO_BUNDLED_PLUGINS_DIR;
    process.env.ALISIO_HOME = "/srv/alisio-home";
    process.env.ALISIO_STATE_DIR = "/srv/alisio-state";
    process.env.ALISIO_BUNDLED_PLUGINS_DIR = "/srv/alisio-bundled";
    ...
    try {
      ...
      expect(env.ALISIO_BUNDLED_PLUGINS_DIR).toBeUndefined();
    } finally {
      restoreEnvKey("ALISIO_HOME", previousAlisioHome);
      restoreEnvKey("ALISIO_STATE_DIR", previousAlisioStateDir);
      restoreEnvKey("ALISIO_BUNDLED_PLUGINS_DIR", previousBundledDir);
    }
  });
  ...
  it("withPathResolutionEnv only applies the explicit path env inside the callback", () => {
    ...
    try {
      const seen = withPathResolutionEnv(
        homeDir,
        { ALISIO_BUNDLED_PLUGINS_DIR: "~/bundled" },
        (env) => ({
          processHome: process.env.HOME,
          processAlisioHome: process.env.ALISIO_HOME,
          processBundledDir: process.env.ALISIO_BUNDLED_PLUGINS_DIR,
          envBundledDir: env.ALISIO_BUNDLED_PLUGINS_DIR,
        }),
      );
