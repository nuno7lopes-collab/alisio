...
const { makeStorePath } = createCronStoreHarness({ prefix: "alisio-cron-store-" });
...
describe("resolveCronStorePath", () => {
  ...
  it("uses ALISIO_HOME for tilde expansion", () => {
    vi.stubEnv("ALISIO_HOME", "/srv/alisio-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/alisio-home", "cron", "jobs.json"));
  });
});
