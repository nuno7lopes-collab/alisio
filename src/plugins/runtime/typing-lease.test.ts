...
function expectTypingPulseCount(pulse: MockedPulse, expected: number) {
  expect(pulse.mock.calls).toHaveLength(expected);
}

function createPulseWithBackgroundFailure<
...
async function expectIndependentTypingLeases<
...
async function expectBackgroundTypingPulseFailuresAreSwallowed<
  TParams extends { intervalMs?: number; pulse: (...args: never[]) => Promise<unknown> },
...
async function expectDefaultTypingLeaseInterval<
  TParams extends { intervalMs?: number; pulse: (...args: never[]) => Promise<unknown> },
  TLease extends { stop: () => void },
>(params: {
  createLease: (params: TParams) => Promise<TLease>;
...
describe("createTypingLease", () => {
  ...
  it("swallows background pulse failures", async () => {
    const pulse = createPulseWithBackgroundFailure<
      (params: { target: string; lane?: string }) => Promise<unknown>
    >();
