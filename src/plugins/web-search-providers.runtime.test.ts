...
let loadAlisioPluginsMock: ReturnType<typeof vi.fn>;
    ...
    plugins: [
      {
        ...
        manifestPath: "/tmp/brave/alisio.plugin.json",
        channels: [],
        providers: [],
        skills: [],
        hooks: [],
      ...
      },
      {
        ...
        manifestPath: "/tmp/noise/alisio.plugin.json",
        channels: [],
        providers: [],
        skills: [],
        hooks: [],
...
function expectLoaderCallCount(count: number) {
  expect(loadAlisioPluginsMock).toHaveBeenCalledTimes(count);
}
...
describe("resolvePluginWebSearchProviders", () => {
  ...
  beforeEach(() => {
    ...
    loadAlisioPluginsMock = vi
      .spyOn(loaderModule, "loadAlisioPlugins")
  ...
  it.each([
    ...
    },
    {
      ...
      mutate: (_config: { plugins?: Record<string, unknown> }, env: NodeJS.ProcessEnv) => {
        env.ALISIO_HOME = "/tmp/alisio-home-b";
      },
    },
  ] as const)("$name", ({ mutate }) => {
    const config = createBraveAllowConfig();
    const env = createWebSearchEnv({ ALISIO_HOME: "/tmp/alisio-home-a" });
  ...
  it.each([
    ...
    },
    {
      ...
      env: {
        ALISIO_PLUGIN_DISCOVERY_CACHE_MS: "0",
      },
    },
  ])("$title", ({ env }) => {
    expectSnapshotMemoization({
  ...
  it("does not leak host Vitest env into an explicit non-Vitest cache key", () => {
    ...
    expect(loadAlisioPluginsMock).toHaveBeenCalledTimes(1);
  });
  ...
  it("expires web-search snapshot memoization after the shortest plugin cache ttl", () => {
    ...
    const config = createBraveAllowConfig();
    const env = createWebSearchEnv({
      ALISIO_PLUGIN_DISCOVERY_CACHE_MS: "5",
      ALISIO_PLUGIN_MANIFEST_CACHE_MS: "20",
    });
    ...
    expect(loadAlisioPluginsMock).toHaveBeenCalledTimes(2);
  });

  it("invalidates web-search snapshots when cache-control env values change in place", () => {
    const config = createBraveAllowConfig();
    const env = createWebSearchEnv({
      ALISIO_PLUGIN_DISCOVERY_CACHE_MS: "1000",
    });
    ...
    expectSnapshotLoaderCalls({
      ...
      env,
      mutate: () => {
        env.ALISIO_PLUGIN_DISCOVERY_CACHE_MS = "5";
      },
      expectedLoaderCalls: 2,
    });
  });
