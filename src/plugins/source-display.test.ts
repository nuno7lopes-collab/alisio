...
const PLUGIN_SOURCE_ROOTS = {
  stock: path.resolve(path.sep, "opt", "homebrew", "lib", "node_modules", "alisio", "extensions"),
  global: path.resolve(path.sep, "Users", "x", ".alisio", "extensions"),
  workspace: path.resolve(path.sep, "Users", "x", "ws", ".alisio", "extensions"),
};
...
describe("formatPluginSourceForTable", () => {
  ...
  it("resolves source roots from an explicit env override", () => {
    const homeDir = path.resolve(path.sep, "tmp", "alisio-home");
    expectResolvedSourceRoots({
      ...
      expected: {
        ...
        workspace: path.join(homeDir, "ws", ".alisio", "extensions"),
      },
    });
  });
});
