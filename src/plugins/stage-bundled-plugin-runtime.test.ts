...
describe("stageBundledPluginRuntime", () => {
  it("stages bundled dist plugins as runtime wrappers and links staged dist node_modules", () => {
    const repoRoot = makeRepoRoot("alisio-stage-bundled-runtime-");
  ...
  it("writes wrappers that forward plugin entry imports into canonical dist files", async () => {
    const repoRoot = makeRepoRoot("alisio-stage-bundled-runtime-chunks-");
  ...
  it("stages root runtime sidecars that bundled plugin boundaries resolve directly", () => {
    const repoRoot = makeRepoRoot("alisio-stage-bundled-runtime-sidecars-");
  ...
  it("keeps plugin command registration on the canonical dist graph when loaded from dist-runtime", async () => {
    const repoRoot = makeRepoRoot("alisio-stage-bundled-runtime-commands-");
    ...
    fs.writeFileSync(
      ...
      [
        "const registry = globalThis.__alisioTestPluginCommands ??= new Map();",
  ...
  it("copies package metadata files but symlinks other non-js plugin artifacts into the runtime overlay", () => {
    const repoRoot = makeRepoRoot("alisio-stage-bundled-runtime-assets-");
    createDistPluginDir(repoRoot, "diffs");
    setupRepoFiles(repoRoot, {
      [bundledDistPluginFile("diffs", "package.json")]: JSON.stringify(
        { name: "@alisio/diffs", alisio: { extensions: ["./index.js"] } },
        null,
        2,
      ),
      [bundledDistPluginFile("diffs", "alisio.plugin.json")]: "{}\n",
    ...
    expectRuntimeArtifactText({
      repoRoot,
      pluginId: "diffs",
      relativePath: "alisio.plugin.json",
      expectedText: "{}\n",
      symbolicLink: false,
    });
    expectRuntimeArtifactText({
  ...
  it("preserves package metadata needed for bundled plugin discovery from dist-runtime", () => {
    const repoRoot = makeRepoRoot("alisio-stage-bundled-runtime-discovery-");
    ...
    setupRepoFiles(repoRoot, {
      ...
      [bundledDistPluginFile("demo", "alisio.plugin.json")]: JSON.stringify(
        {
          id: "demo",
          channels: ["demo"],
    ...
    const env = {
      ...process.env,
      ALISIO_BUNDLED_PLUGINS_DIR: runtimeExtensionsDir,
    };
  ...
  it("removes stale runtime plugin directories that are no longer in dist", () => {
    const repoRoot = makeRepoRoot("alisio-stage-bundled-runtime-stale-");
  ...
  it("removes dist-runtime when the built bundled plugin tree is absent", () => {
    const repoRoot = makeRepoRoot("alisio-stage-bundled-runtime-missing-");
  ...
  it("tolerates EEXIST when an identical runtime symlink is materialized concurrently", () => {
    const repoRoot = makeRepoRoot("alisio-stage-bundled-runtime-eexist-");
