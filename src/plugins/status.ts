...
import { normalizeAlisioVersionBase } from "../config/version.js";
...
function resolveReportedPluginVersion(
  ...
  return (
    normalizeAlisioVersionBase(resolveCompatibilityHostVersion(env)) ??
    normalizeAlisioVersionBase(plugin.version) ??
    plugin.version
  );
}
