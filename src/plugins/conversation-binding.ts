...
const APPROVALS_PATH = "~/.alisio/plugin-binding-approvals.json";
...
const LEGACY_CODEX_PLUGIN_SESSION_PREFIXES = [
  "alisio-app-server:thread:",
  "alisio-codex-app-server:thread:",
] as const;
...
const PLUGIN_BINDING_PENDING_REQUESTS_KEY = Symbol.for("alisio.pluginBindingPendingRequests");
...
const pluginBindingGlobalStateKey = Symbol.for("alisio.plugins.binding.global-state");
...
export function buildPluginBindingUnavailableText(binding: PluginConversationBinding): string {
  return `The bound plugin ${resolvePluginBindingDisplayName(binding)} is not currently loaded. Routing this message to Alisio instead.${buildDetachHintSuffix(binding.detachHint)}`;
}
