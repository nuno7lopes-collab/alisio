...
function summarizeGroupPolicy(cfg: AlisioConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function isGatewayRemotelyExposed(cfg: AlisioConfig): boolean {
...
function collectModels(cfg: AlisioConfig): ModelRef[] {
  const out: ModelRef[] = [];
  addModel(
    out,
...
function listKnownNodeCommands(cfg: AlisioConfig): Set<string> {
...
function hasWebSearchKey(cfg: AlisioConfig, env: NodeJS.ProcessEnv): boolean {
  return hasBundledWebSearchCredential({ config: cfg, env });
}

function isWebSearchEnabled(cfg: AlisioConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: AlisioConfig): boolean {
...
function isBrowserEnabled(cfg: AlisioConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: AlisioConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
function listPotentialMultiUserSignals(cfg: AlisioConfig): string[] {
  const out = new Set<string>();
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return [];
...
function collectRiskyToolExposureContexts(cfg: AlisioConfig): {
...
export function collectAttackSurfaceSummaryFindings(cfg: AlisioConfig): SecurityAuditFinding[] {
...
export function collectSecretsInConfigFindings(cfg: AlisioConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
...
export function collectSandboxDockerNoopFindings(cfg: AlisioConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectSandboxDangerousConfigFindings(cfg: AlisioConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const agents = Array.isArray(cfg.agents?.list) ? cfg.agents.list : [];
...
export function collectNodeDenyCommandPatternFindings(cfg: AlisioConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectMinimalProfileOverrideFindings(cfg: AlisioConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: AlisioConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: AlisioConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectLikelyMultiUserSetupFindings(cfg: AlisioConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  ...
  findings.push({
    ...
    detail:
      ...
      `\n${impactLine}\n${riskyContextsDetail}\n` +
      "Alisio's default security model is personal-assistant (one trusted operator boundary), not hostile multi-tenant isolation on one shared gateway.",
    remediation:
