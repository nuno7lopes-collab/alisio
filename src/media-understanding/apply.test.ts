...
function createGroqAudioConfig(): AlisioConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): AlisioConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): AlisioConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: AlisioConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  beforeAll(async () => {
    ...
    const baseDir = resolvePreferredAlisioTmpDir();
