...
This directory contains hooks that ship with Alisio. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**What it does**: Writes a canonical backlog note under `memory/backlog/`, then lets the memory jobs promote it into daily and topic memory through the canonical pipeline.
...
- **Managed hooks**: `~/.alisio/hooks/` (shared across workspaces)
...
Hooks can be configured in `~/.alisio/alisio.json`:
...
2. Restart gateway: `pkill -9 -f 'alisio.*gateway' && pnpm alisio gateway`
3. Enable the hook: `alisio hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.alisio.ai/automation/hooks
