...
`;
    ...
    expect(parsed.alisio.emoji).toBe("💾");
    expect(parsed.alisio.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.alisio.emoji).toBe("📝");
    expect(parsed.alisio.events).toEqual(["command"]);
    expect(parsed.alisio.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.alisio.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"alisio": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"alisio": {"events": ["test"]}}');
  });
...
describe("resolveAlisioHookMetadata", () => {
  it("extracts alisio metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveAlisioHookMetadata(frontmatter);
  ...
  it("returns undefined when alisio key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    ...
    expect(result?.install?.[1].package).toBe("@alisio/hook");
  });
...
`;
    ...
    const alisio = resolveAlisioHookMetadata(frontmatter);
    expect(alisio).toBeDefined();
    expect(alisio?.emoji).toBe("💾");
    expect(alisio?.events).toEqual(["command:new", "command:reset"]);
    expect(alisio?.requires?.config).toEqual(["workspace.dir"]);
    expect(alisio?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const alisio = resolveAlisioHookMetadata(frontmatter);
    expect(alisio?.emoji).toBe("disk");
    expect(alisio?.events).toEqual(["command:new"]);
  });
});
