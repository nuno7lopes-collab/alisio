...
describe("bundle plugin hooks", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "alisio-plugin-hooks-"));
  });
  ...
  beforeEach(async () => {
    ...
    previousBundledHooksDir = process.env.ALISIO_BUNDLED_HOOKS_DIR;
  ...
  afterEach(() => {
    ...
    if (previousBundledHooksDir === undefined) {
      delete process.env.ALISIO_BUNDLED_HOOKS_DIR;
    } else {
      process.env.ALISIO_BUNDLED_HOOKS_DIR = previousBundledHooksDir;
    }
  });
  ...
  function createConfig(enabled: boolean): AlisioConfig {
    return {
      hooks: {
        internal: {
          enabled: true,
  ...
  it("exposes enabled bundle hook dirs as plugin-managed hook entries", async () => {
    ...
    expect(entries[0]?.hook.source).toBe("alisio-plugin");
  ...
  it("does not treat Claude hooks.json bundles as Alisio hook packs", async () => {
    const bundleRoot = path.join(workspaceDir, ".alisio", "extensions", "claude-bundle");
