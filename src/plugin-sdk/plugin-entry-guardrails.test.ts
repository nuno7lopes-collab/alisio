...
const CORE_PLUGIN_ENTRY_IMPORT_RE =
  /import\s*\{[^}]*\bdefinePluginEntry\b[^}]*\}\s*from\s*"alisio\/plugin-sdk\/core"/;
    ...
    for (const entry of readdirSync(EXTENSIONS_DIR, { withFileTypes: true })) {
      ...
      try {
        const pkg = JSON.parse(readFileSync(packageJsonPath, "utf8")) as {
          alisio?: { extensions?: unknown };
        };
        const extensions = Array.isArray(pkg.alisio?.extensions) ? pkg.alisio.extensions : [];
        if (
          extensions.some(
