...
const RUNTIME_API_EXPORT_GUARDS: Record<string, readonly string[]> = {
  ...
  [bundledPluginFile("imessage", "runtime-api.ts")]: [
    'export { DEFAULT_ACCOUNT_ID, buildChannelConfigSchema, getChatChannelMeta, type ChannelPlugin, type AlisioConfig, type AlisioConfig as OpenClawConfig } from "alisio/plugin-sdk/core";',
    'export { PAIRING_APPROVED_MESSAGE } from "alisio/plugin-sdk/channel-status";',
    'export { buildComputedAccountStatusSnapshot, collectStatusIssuesFromLastError } from "alisio/plugin-sdk/status-helpers";',
    'export { formatTrimmedAllowFromEntries, resolveIMessageConfigAllowFrom, resolveIMessageConfigDefaultTo } from "alisio/plugin-sdk/channel-config-helpers";',
    ...
    'export type IMessageAccountConfig = Omit< NonNullable<NonNullable<RuntimeApiAlisioConfig["channels"]>["imessage"]>, "accounts" | "defaultAccount" >;',
  ...
  [bundledPluginFile("googlechat", "runtime-api.ts")]: [
    'export * from "alisio/plugin-sdk/googlechat";',
  ],
  [bundledPluginFile("matrix", "runtime-api.ts")]: [
    ...
    'export { assertHttpUrlTargetsPrivateNetwork, closeDispatcher, createPinnedDispatcher, resolvePinnedHostnameWithPolicy, ssrfPolicyFromAllowPrivateNetwork, type LookupFn, type SsrFPolicy } from "alisio/plugin-sdk/ssrf-runtime";',
    ...
    'export type { ChannelDirectoryEntry, ChannelMessageActionContext, AlisioConfig, AlisioConfig as OpenClawConfig, PluginRuntime, RuntimeLogger, RuntimeEnv, WizardPrompter } from "alisio/plugin-sdk/matrix-runtime-shared";',
    'export { formatZonedTimestamp } from "alisio/plugin-sdk/matrix-runtime-shared";',
    'export function chunkTextForOutbound(text: string, limit: number): string[] { const chunks: string[] = []; let remaining = text; while (remaining.length > limit) { const window = remaining.slice(0, limit); const splitAt = Math.max(window.lastIndexOf("\\n"), window.lastIndexOf(" ")); const breakAt = splitAt > 0 ? splitAt : limit; chunks.push(remaining.slice(0, breakAt).trimEnd()); remaining = remaining.slice(breakAt).trimStart(); } if (remaining.length > 0 || text.length === 0) { chunks.push(remaining); } return chunks; }',
  ],
  [bundledPluginFile("nextcloud-talk", "runtime-api.ts")]: [
    'export * from "alisio/plugin-sdk/nextcloud-talk";',
  ],
  ...
  [bundledPluginFile("telegram", "runtime-api.ts")]: [
    'export type { ChannelMessageActionAdapter, ChannelPlugin, AlisioConfig, AlisioConfig as OpenClawConfig, OpenClawPluginApi, PluginRuntime, TelegramAccountConfig, TelegramActionConfig, TelegramNetworkConfig } from "alisio/plugin-sdk/telegram-core";',
    'export type { TelegramApiOverride } from "./src/send.js";',
    'export type { OpenClawPluginService, OpenClawPluginServiceContext, PluginLogger } from "alisio/plugin-sdk/core";',
    'export type { AcpRuntime, AcpRuntimeCapabilities, AcpRuntimeDoctorReport, AcpRuntimeEnsureInput, AcpRuntimeEvent, AcpRuntimeHandle, AcpRuntimeStatus, AcpRuntimeTurnInput, AcpRuntimeErrorCode, AcpSessionUpdateTag } from "alisio/plugin-sdk/acp-runtime";',
    'export { AcpRuntimeError } from "alisio/plugin-sdk/acp-runtime";',
    'export { buildTokenChannelStatusSummary, clearAccountEntryFields, DEFAULT_ACCOUNT_ID, normalizeAccountId, PAIRING_APPROVED_MESSAGE, parseTelegramTopicConversation, projectCredentialSnapshotFields, resolveConfiguredFromCredentialStatuses, resolveTelegramPollVisibility } from "alisio/plugin-sdk/telegram-core";',
    'export { buildChannelConfigSchema, getChatChannelMeta, jsonResult, readNumberParam, readReactionParams, readStringArrayParam, readStringOrNumberParam, readStringParam, resolvePollMaxSelections, TelegramConfigSchema } from "alisio/plugin-sdk/telegram-core";',
