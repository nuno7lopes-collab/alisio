...
export function isCommandEnabled(cfg: AlisioConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: AlisioConfig): {
  provider: string;
  model: string;
} {
...
export function getCommandDetection(_cfg?: AlisioConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: AlisioConfig) {
