...
const { telegramCommandTestPlugin } = loadBundledPluginTestApiSync<{
  telegramCommandTestPlugin: ChannelPlugin;
}>("telegram");
...
vi.mock("../../agents/model-catalog.js", () => ({
  loadModelCatalog: vi.fn(async () => mockModelCatalogEntries),
  loadMergedRuntimeModelCatalog: vi.fn(async () => mockModelCatalogEntries),
}));
...
function buildParams(commandBody: string, cfg: AlisioConfig, ctxOverrides?: Partial<MsgContext>) {
      ...
      },
      {
        ...
        run: async () => {
          await withTempConfigPath({ messages: { ackReaction: ":)" } }, async (configPath) => {
            ...
            const written = await readJsonFile<AlisioConfig>(configPath);
