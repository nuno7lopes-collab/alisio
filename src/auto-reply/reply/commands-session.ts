...
import { scheduleGatewaySigusr1Restart, triggerAlisioRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Alisio in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerAlisioRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Alisio via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
