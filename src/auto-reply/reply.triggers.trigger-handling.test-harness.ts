...
const piEmbeddedMocks = getSharedMocks("alisio.trigger-handling.pi-embedded-mocks", () => ({
...
const modelCatalogMocks = getSharedMocks("alisio.trigger-handling.model-catalog-mocks", () => ({
...
const modelFallbackMocks = getSharedMocks("alisio.trigger-handling.model-fallback-mocks", () => ({
...
const webSessionMocks = getSharedMocks("alisio.trigger-handling.web-session-mocks", () => ({
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("ALISIO_STATE_DIR", snapshot.stateDir);
}
...
function setTempHomeEnv(home: string): void {
  ...
  process.env.ALISIO_STATE_DIR = join(home, ".alisio");
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "alisio-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".alisio", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
...
export function makeCfg(home: string): AlisioConfig {
  return {
    agents: {
      defaults: {
        model: { primary: "anthropic/claude-opus-4-5" },
        workspace: join(home, "alisio"),
