...
type HomeEnvSnapshot = {
  ...
  ALISIO_STATE_DIR: string | undefined;
  ALISIO_AGENT_DIR: string | undefined;
...
function snapshotHomeEnv(): HomeEnvSnapshot {
  return {
    ...
    ALISIO_AGENT_DIR: process.env.ALISIO_AGENT_DIR,
...
export function createTempHomeHarness(options: { prefix: string; beforeEachCase?: () => void }) {
  ...
  async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    await fs.mkdir(path.join(home, ".alisio", "agents", "main", "sessions"), { recursive: true });
    const envSnapshot = snapshotHomeEnv();
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.ALISIO_STATE_DIR = path.join(home, ".alisio");
    process.env.ALISIO_AGENT_DIR = path.join(home, ".alisio", "agent");
    process.env.PI_CODING_AGENT_DIR = path.join(home, ".alisio", "agent");
