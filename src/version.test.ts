...
async function withTempDir<T>(run: (dir: string) => Promise<T>): Promise<T> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "alisio-version-"));
  try {
...
describe("version resolution", () => {
  it("resolves package version from nested dist/plugin-sdk module URL", async () => {
    await withTempDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "alisio", version: "1.2.3" });
  ...
  it("ignores non-alisio package and blank build-info versions", async () => {
    await withTempDir(async (root) => {
  ...
  it("prefers ALISIO_VERSION over service and package versions", () => {
    expect(
      resolveRuntimeServiceVersion({
        ALISIO_VERSION: "9.9.9",
        ALISIO_SERVICE_VERSION: "2.2.2",
        npm_package_version: "1.1.1",
      }),
    ).toBe("9.9.9");
  });

  it("prefers runtime VERSION over stale ALISIO_VERSION for compatibility checks", () => {
    const previous = process.env.ALISIO_VERSION;
    const previousService = process.env.ALISIO_SERVICE_VERSION;
    const previousPackage = process.env.npm_package_version;
    try {
      process.env.ALISIO_VERSION = "2026.3.25";
      process.env.ALISIO_SERVICE_VERSION = "2026.3.25-service";
      process.env.npm_package_version = "2026.3.25-package";
      expect(resolveCompatibilityHostVersion()).toBe(VERSION);
    } finally {
      process.env.ALISIO_VERSION = previous;
      process.env.ALISIO_SERVICE_VERSION = previousService;
  ...
  it("keeps explicit env-object overrides for compatibility checks in tests", () => {
    expect(
      resolveCompatibilityHostVersion({
        ...
        ALISIO_SERVICE_VERSION: "2026.3.98",
