...
function listAgents(cfg: AlisioConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: AlisioConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<AlisioConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: AlisioConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: AlisioConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: AlisioConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<AlisioConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  AlisioConfig,
  {
    bindingsRef: AlisioConfig["bindings"];
    agentsRef: AlisioConfig["agents"];
    sessionRef: AlisioConfig["session"];
...
function resolveRouteCacheForConfig(cfg: AlisioConfig): Map<string, ResolvedAgentRoute> {
...
export function resolveAgentRoute(input: ResolveAgentRouteInput): ResolvedAgentRoute {
  ...
  const dmScope = resolveDmScope(input.cfg.session?.dmScope);
