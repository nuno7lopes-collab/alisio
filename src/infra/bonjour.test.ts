...
function enableAdvertiserUnitMode(hostname = "test-host") {
  ...
  process.env.ALISIO_MDNS_HOSTNAME = hostname;
}
...
describe("gateway bonjour advertiser", () => {
  ...
  it("suppresses ciao self-probe retry console noise while advertising", async () => {
    ...
    try {
      ...
      console.log(
        "[test._alisio-gw._tcp.local.] failed probing with reason: Error: Can't probe for a service which is announced already. Received announcing for service test._alisio-gw._tcp.local.. Trying again in 2 seconds!",
      );
  ...
  it("normalizes hostnames with domains for service names", async () => {
    ...
    expect(gatewayCall?.[0]?.name).toBe("alisio (Alisio)");
    expect(gatewayCall?.[0]?.domain).toBe("local");
    expect(gatewayCall?.[0]?.hostname).toBe("alisio");
    expect((gatewayCall?.[0]?.txt as Record<string, string>)?.lanHost).toBe("alisio.local");
