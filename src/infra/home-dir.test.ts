...
describe("resolveEffectiveHomeDir", () => {
  it.each([
    {
      name: "prefers ALISIO_HOME over HOME and USERPROFILE",
      env: {
...
describe("resolveRequiredHomeDir", () => {
  it.each([
    ...
    {
      name: "returns a fully resolved path for ALISIO_HOME",
      env: { ALISIO_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      homedir: () => "/fallback",
      expected: path.resolve("/custom/home"),
    },
    {
      name: "returns cwd when ALISIO_HOME is tilde-only and no fallback home exists",
      env: { ALISIO_HOME: "~" } as NodeJS.ProcessEnv,
      homedir: () => {
        throw new Error("no home");
      },
      expected: process.cwd(),
...
describe("resolveOsHomeDir", () => {
  it("ignores ALISIO_HOME and uses HOME", () => {
    expect(
      resolveOsHomeDir(
        {
          ALISIO_HOME: "/srv/alisio-home",
  ...
  it.each([
    {
      ...
      expected: `${path.resolve("/srv/alisio-home")}/x`,
    },
    {
      name: "expands exact ~ using explicit home",
      input: "~",
      opts: { home: " /srv/alisio-home " },
      expected: "/srv/alisio-home",
    },
    {
...
describe("resolveHomeRelativePath", () => {
  it.each([
    ...
    },
    {
      ...
      expected: path.resolve("/srv/alisio-home/docs"),
    },
    {
...
describe("resolveOsHomeRelativePath", () => {
  it("expands tilde paths using the OS home instead of ALISIO_HOME", () => {
    expect(
