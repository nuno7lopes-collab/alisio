...
describe("runMessageAction media behavior", () => {
  ...
  describe("sandboxed media validation", () => {
    ...
    it("allows media paths under preferred Alisio tmp root", async () => {
      ...
      try {
        ...
        const hostTmpOutsideAlisio = path.join(os.tmpdir(), "outside-alisio", "test-media.png");
        await expect(
          runMessageAction({
            cfg: slackConfig,
            action: "send",
