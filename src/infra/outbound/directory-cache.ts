...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: AlisioConfig | null = null;
  ...
  get(key: string, cfg: AlisioConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: AlisioConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: AlisioConfig): void {
