...
describe("generic current-conversation bindings", () => {
  ...
  beforeEach(async () => {
    previousStateDir = process.env.ALISIO_STATE_DIR;
    testStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "alisio-current-bindings-"));
    process.env.ALISIO_STATE_DIR = testStateDir;
