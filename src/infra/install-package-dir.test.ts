...
describe("installPackageDir", () => {
  ...
  it("keeps the existing install in place when staged validation fails", async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "alisio-install-package-dir-"));
    ...
    await expect(listMatchingDirs(installBaseDir, ".alisio-install-stage-")).resolves.toHaveLength(
      0,
    );
    await expect(listMatchingDirs(installBaseDir, ".alisio-install-backups")).resolves.toHaveLength(
      0,
    );
  });
  ...
  it("restores the original install if publish rename fails", async () => {
    ...
    const backupRoot = path.join(installBaseDir, ".alisio-install-backups");
  ...
  it("warns and leaves the backup in place when the install base changes before backup cleanup", async () => {
    ...
    const backupRoot = path.join(preservedInstallRoot, ".alisio-install-backups");
  ...
  it("installs peer dependencies for isolated plugin package installs", async () => {
    ...
    expect(vi.mocked(runCommandWithTimeout)).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        cwd: expect.stringContaining(".alisio-install-stage-"),
      }),
    );
  });
  ...
  it("hides the staged project .npmrc while npm install runs and restores it afterward", async () => {
    ...
    vi.mocked(runCommandWithTimeout).mockImplementation(async (_argv, optionsOrTimeout) => {
      ...
      await expect(
        listMatchingEntries(cwd ?? "", ".alisio-install-hidden-npmrc-"),
    ...
    await expect(
      listMatchingEntries(targetDir, ".alisio-install-hidden-npmrc-"),
