...
async function makeTempDir(label: string): Promise<string> {
  return fs.mkdtemp(path.join(os.tmpdir(), `alisio-${label}-`));
}
...
describe("git commit resolution", () => {
  ...
  it("does not walk out of the alisio package into a host repo", async () => {
    ...
    const packageRoot = path.join(hostRepo, "node_modules", "alisio");
    await fs.mkdir(path.join(packageRoot, "dist"), { recursive: true });
    await fs.writeFile(
      path.join(packageRoot, "package.json"),
      JSON.stringify({ name: "alisio", version: "2026.3.10" }),
      "utf-8",
    );
