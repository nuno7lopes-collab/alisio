...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __alisioDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__alisioDiagnosticEventsState) {
    globalStore.__alisioDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__alisioDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: AlisioConfig): boolean {
