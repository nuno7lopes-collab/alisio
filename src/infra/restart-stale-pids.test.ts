...
const mockResolveGatewayPort = vi.hoisted(() => vi.fn(() => 40705));
...
function createAlisioBusyResult(pid: number, overrides: Partial<MockLsofResult> = {}) {
  return createLsofResult({
    stdout: lsofOutput([{ pid, cmd: "alisio-gateway" }]),
    ...overrides,
  });
}
...
function installInitialBusyPoll(
  ...
  mockSpawnSync.mockImplementation(() => {
    call += 1;
    if (call === 1) {
      return createAlisioBusyResult(stalePid);
    }
...
describe.skipIf(isWindows)("restart-stale-pids", () => {
  ...
  beforeEach(async () => {
    ...
    mockResolveGatewayPort.mockReturnValue(40705);
  ...
  describe("findGatewayPidsOnPortSync", () => {
    ...
    it("parses alisio-gateway pids and excludes the current process", () => {
      const stalePid = process.pid + 1;
      mockSpawnSync.mockReturnValue({
        error: null,
        status: 0,
        stdout: lsofOutput([
          { pid: stalePid, cmd: "alisio-gateway" },
          { pid: process.pid, cmd: "alisio-gateway" },
        ]),
        stderr: "",
      });
    ...
    it("excludes pids whose command does not include 'alisio'", () => {
    ...
    it("forwards the spawnTimeoutMs argument to spawnSync", () => {
      ...
      findGatewayPidsOnPortSync(40705, 400);
    ...
    it("deduplicates pids from dual-stack listeners (IPv4+IPv6 emit same pid twice)", () => {
      ...
      mockSpawnSync.mockReturnValue({ error: null, status: 0, stdout, stderr: "" });
      const result = findGatewayPidsOnPortSync(40705);
  ...
  describe("parsePidsFromLsofOutput (via findGatewayPidsOnPortSync stdout path)", () => {
    ...
    it("parses multiple alisio pids from a single lsof output block", () => {
      ...
      mockSpawnSync.mockReturnValue({
        ...
        status: 0,
        stdout: lsofOutput([
          { pid: pid1, cmd: "alisio-gateway" },
          { pid: pid2, cmd: "alisio-gateway" },
        ]),
        stderr: "",
      });
      const result = findGatewayPidsOnPortSync(40705);
    ...
    it("returns [] when status 0 but only non-alisio pids present", () => {
      // Port may be bound by an unrelated process. findGatewayPidsOnPortSync
      // only tracks alisio processes — non-alisio listeners are ignored.
  ...
  describe("pollPortOnce — no second lsof spawn (Codex P1 regression)", () => {
    ...
    it("lsof status 1 with non-empty alisio stdout is treated as busy, not free (Linux container edge case)", () => {
      ...
      // status 1 + non-empty alisio stdout must not be treated as port-free.
      const stalePid = process.pid + 601;
      const getCallCount = installInitialBusyPoll(stalePid, (call) => {
        if (call === 2) {
          // status 1 + alisio pid in stdout — container-restricted lsof reports partial results
          return createAlisioBusyResult(stalePid, {
            status: 1,
    ...
    it("pollPortOnce outer catch returns { free: null, permanent: false } when resolveLsofCommandSync throws", () => {
      ...
      mockSpawnSync.mockImplementationOnce(() => {
        ...
        return {
          error: null,
          status: 0,
          stdout: lsofOutput([{ pid: stalePid, cmd: "alisio-gateway" }]),
          stderr: "",
        };
      });
  ...
  describe("parsePidsFromLsofOutput — branch coverage (lines 67-69)", () => {
    it("skips a mid-loop entry when the command does not include 'alisio'", () => {
      // Exercises the false branch of currentCmd.toLowerCase().includes("alisio")
      // inside the mid-loop flush: a non-alisio cmd between two entries must not
      // be pushed, but the following alisio entry still must be.
      const stalePid = process.pid + 700;
      // Mixed output: non-alisio entry first, then alisio entry
    ...
    it("ignores a p-line with an invalid (non-positive) PID — ternary false branch", () => {
      ...
      // p0 is invalid (not > 0); the following valid alisio entry must still be found.
      const stdout = `p0\ncalisio-gateway\np${stalePid}\ncalisio-gateway\n`;
      mockSpawnSync.mockReturnValue({ error: null, status: 0, stdout, stderr: "" });
      const result = findGatewayPidsOnPortSync(40705);
      expect(result).toContain(stalePid);
  ...
  // -------------------------------------------------------------------------
  // pollPortOnce branch — status 1 + non-empty stdout with zero alisio pids
  // -------------------------------------------------------------------------
  describe("pollPortOnce — status 1 + non-empty non-alisio stdout (line 145)", () => {
    it("treats status 1 + non-alisio stdout as port-free (not an alisio process)", () => {
      // status 1 + non-empty stdout where no alisio pids are present:
      // the port may be held by an unrelated process. From our perspective
      // (we only kill alisio pids) it is effectively free.
      const stalePid = process.pid + 800;
      const getCallCount = installInitialBusyPoll(stalePid, () => {
        // status 1 + non-alisio output — should be treated as free:true for our purposes
        return createLsofResult({
          status: 1,
      ...
      vi.spyOn(process, "kill").mockReturnValue(true);
      // Should complete cleanly — no alisio pids in status-1 output → free
      expect(() => cleanStaleGatewayProcessesSync()).not.toThrow();
