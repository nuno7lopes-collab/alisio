...
describe("ports helpers", () => {
  ...
  it("prints an Alisio-specific hint when port details look like another Alisio instance", async () => {
    const runtime = {
      error: vi.fn(),
      log: vi.fn(),
      exit: vi.fn() as unknown as (code: number) => never,
    ...
    const messages = runtime.error.mock.calls.map((call) => stripAnsi(String(call[0] ?? "")));
    expect(messages.join("\n")).toContain("another Alisio instance is already running");
  });
});
...
describeUnix("inspectPortUsage", () => {
  ...
  it("falls back to ss when lsof is unavailable", async () => {
    ...
    try {
      ...
      expect(result.listeners[0]?.commandLine).toContain("alisio");
