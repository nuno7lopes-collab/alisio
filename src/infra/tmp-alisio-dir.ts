...
export const POSIX_ALISIO_TMP_DIR = "/tmp/alisio";
const TMP_DIR_ACCESS_MODE = fs.constants.W_OK | fs.constants.X_OK;

type ResolvePreferredAlisioTmpDirOptions = {
...
export function resolvePreferredAlisioTmpDir(
  options: ResolvePreferredAlisioTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "alisio" : `alisio-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[alisio] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback Alisio temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback Alisio temp dir: ${fallbackPath}`);
    }
  ...
  const existingPreferredState = resolveDirState(POSIX_ALISIO_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_ALISIO_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_ALISIO_TMP_DIR)) {
      return POSIX_ALISIO_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_ALISIO_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_ALISIO_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_ALISIO_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_ALISIO_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_ALISIO_TMP_DIR;
