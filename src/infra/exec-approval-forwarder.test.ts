...
function makeTargetsCfg(targets: Array<{ channel: string; to: string }>): AlisioConfig {
  return {
    approvals: {
      exec: {
        enabled: true,
...
function makeSessionCfg(options: { discordExecApprovalsEnabled?: boolean } = {}): AlisioConfig {
  return {
