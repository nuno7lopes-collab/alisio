...
describe("package-json helpers", () => {
  it("reads package version and trims package name", async () => {
    await withTempDir({ prefix: "alisio-package-json-" }, async (root) => {
      await fs.writeFile(
        path.join(root, "package.json"),
        JSON.stringify({ version: " 1.2.3 ", name: "  @alisio/demo  " }),
        "utf8",
      );
  ...
  it.each([
    ...
    },
    {
      ...
      writePackageJson: async (root: string) => {
        await fs.writeFile(
          path.join(root, "package.json"),
          JSON.stringify({ version: "   ", name: "@alisio/demo" }),
          "utf8",
        );
      },
      expectedVersion: null,
