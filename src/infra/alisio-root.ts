...
import { isKnownDistributionPackageName } from "./distribution-profile.js";
...
async function findPackageRoot(startDir: string, maxDepth = 12): Promise<string | null> {
  for (const current of iterAncestorDirs(startDir, maxDepth)) {
    ...
    if (isKnownDistributionPackageName(name)) {
      return current;
    }
  }
  return null;
...
export async function resolveAlisioPackageRoot(opts: {
  cwd?: string;
  argv1?: string;
  moduleUrl?: string;
}): Promise<string | null> {
...
export function resolveAlisioPackageRootSync(opts: {
  cwd?: string;
  argv1?: string;
  moduleUrl?: string;
}): string | null {
