...
const STAGED_NPM_PROJECT_CONFIG_PREFIX = ".alisio-install-hidden-npmrc-";
...
export async function installPackageDir(params: {
  ...
  try {
    ...
    stageDir = await fs.mkdtemp(path.join(installBaseRealPath, ".alisio-install-stage-"));
  ...
  if (params.mode === "update" && (await fileExists(canonicalTargetDir))) {
    const backupRoot = path.join(installBaseRealPath, ".alisio-install-backups");
