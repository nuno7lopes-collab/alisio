...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/alisio-state", "devices")).toEqual({
      dir: path.join("/tmp/alisio-state", "devices"),
      pendingPath: path.join("/tmp/alisio-state", "devices", "pending.json"),
      pairedPath: path.join("/tmp/alisio-state", "devices", "paired.json"),
    });
  });
