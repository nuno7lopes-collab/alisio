...
const createTempDir = () => tempDirs.make("alisio-state-migrations-test-");
...
describe("state migrations", () => {
  it("detects legacy sessions, agent files, whatsapp auth, and telegram allowFrom copies", async () => {
    ...
    expect(detected.preview).toEqual([
      ...
      `- WhatsApp auth: ${path.join(stateDir, "credentials")} → ${path.join(stateDir, "credentials", "whatsapp", "default")}`,
