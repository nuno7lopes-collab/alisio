...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__alisio_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "alisio-root");
...
function setPackageRoot(root: string, name = "alisio") {
  setFile(path.join(root, "package.json"), JSON.stringify({ name }));
}

function expectResolvedPackageRoot(
  syncResolver: typeof import("./alisio-root.js").resolveAlisioPackageRootSync,
  asyncResolver: typeof import("./alisio-root.js").resolveAlisioPackageRoot,
  opts: Parameters<typeof import("./alisio-root.js").resolveAlisioPackageRootSync>[0],
...
describe("resolveAlisioPackageRoot", () => {
  let resolveAlisioPackageRoot: typeof import("./alisio-root.js").resolveAlisioPackageRoot;
  let resolveAlisioPackageRootSync: typeof import("./alisio-root.js").resolveAlisioPackageRootSync;

  beforeEach(async () => {
    vi.resetModules();
    ({ resolveAlisioPackageRoot, resolveAlisioPackageRootSync } = await import("./alisio-root.js"));
    state.entries.clear();
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const argv1 = path.join(project, "node_modules", ".bin", "alisio");
        const pkgRoot = path.join(project, "node_modules", "alisio");
    ...
    },
    {
      ...
      setup: () => {
        ...
        const bin = path.join(project, "bin", "alisio");
        const realPkg = path.join(project, "real-pkg");
        state.realpaths.set(abs(bin), abs(path.join(realPkg, "alisio.mjs")));
    ...
    {
      name: "falls through from a non-alisio moduleUrl candidate to cwd",
      setup: () => {
        const wrongPkgRoot = fx("moduleurl-fallthrough", "wrong");
        const cwdPkgRoot = fx("moduleurl-fallthrough", "cwd");
        setPackageRoot(wrongPkgRoot, "not-alisio");
    ...
    {
      name: "returns null for non-alisio package roots",
      setup: () => {
        const pkgRoot = fx("not-alisio");
        setPackageRoot(pkgRoot, "not-alisio");
        return { opts: { cwd: pkgRoot }, expected: null };
      },
    },
    {
      name: "accepts alisio package roots",
      setup: () => {
        const pkgRoot = fx("alisio-package");
        setPackageRoot(pkgRoot, "alisio");
    ...
    },
    {
      ...
      setup: () => {
        ...
        state.realpaths.set(
          abs(argv1),
          abs(path.join(project, "versions", "current", "alisio.mjs")),
        );
        const pkgRoot = path.join(project, "node_modules", "alisio");
        setPackageRoot(pkgRoot);
        return { opts: { argv1 }, expected: pkgRoot };
