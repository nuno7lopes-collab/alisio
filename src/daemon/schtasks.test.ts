...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "alisio-schtasks-test-"));
    try {
  ...
  it("reads script from ALISIO_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ ALISIO_STATE_DIR: path.join(tmpDir, "custom-state") }),
