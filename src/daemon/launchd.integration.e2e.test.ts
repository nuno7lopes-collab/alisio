...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `alisio-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      ALISIO_LAUNCHD_LABEL: `ai.alisio.launchd-int-${testId}`,
      ALISIO_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
