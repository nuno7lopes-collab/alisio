...
function withFakeRuntimeBin<T>(params: { binName: string; run: () => T }): T {
  return withFakeRuntimeBins({
    ...
    tmpPrefix: `alisio-${params.binName}-bin-`,
    run: params.run,
  });
}
...
function withFakeRuntimeBins<T>(params: {
  ...
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), params.tmpPrefix ?? "alisio-runtime-bins-"));
...
const unsafeRuntimeInvocationCases: UnsafeRuntimeInvocationCase[] = [
  ...
  },
  {
    ...
    binName: "ruby",
    tmpPrefix: "alisio-ruby-load-path-",
  ...
  },
  {
    ...
    binName: "perl",
    tmpPrefix: "alisio-perl-module-preload-",
  ...
  },
  {
    ...
    binName: "perl",
    tmpPrefix: "alisio-perl-load-path-",
  ...
  },
  {
    ...
    binName: "perl",
    tmpPrefix: "alisio-perl-preload-load-path-",
  ...
  },
  {
    ...
    binName: "node",
    tmpPrefix: "alisio-inline-shell-node-",
...
describe("hardenApprovedExecutionPaths", () => {
  ...
  it.runIf(process.platform !== "win32").each(cases)("$name", (testCase) => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "alisio-approval-hardening-"));
    const oldPath = process.env.PATH;
      ...
      withFakeRuntimeBins({
        ...
        run: () => {
          withScriptOperandPlanFixture(
            {
              tmpPrefix: "alisio-approval-script-plan-",
              fixture: runtimeCase,
  ...
  it("captures the real shell script operand after value-taking shell flags", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "alisio-shell-option-value-"));
    try {
